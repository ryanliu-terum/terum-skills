...
const opt = (name, bugRisk, fit) => ({ name, change: 'change for ' + name, bugRisk, fit, fitCitation: fit >= 3 ? 'ledger D3: "does X"' : 'silent', effort: 'a paragraph', winsIf: 'wins if ' + name })
...
const TRIAGE = {
  ...
  'clear-one':    { ...base, options: [opt('A', 1, 3), opt('B', 1, 1)], difficulty: 'moderate', scope: 'pattern', patternDetail: '§5.4, §6' },
  'fork-one':     { ...base, options: [opt('A', 1, 1), opt('B', 1, 1)], oneClearlyWins: false, whyOneOrFork: 'depends on which orphan predicate the product wants' },
