...
const manifest = await agent(
  ...
  (MODE_KIND === 'branch'
    ? '1. Run `git -C /Users/ryanliu/Documents/Terum/terum-codex/refactor-b7 rev-parse --is-inside-work-tree`; if not a repo, return diffAvailable:false. baseRef = "' + BASE_REF + '". Run `git -C /Users/ryanliu/Documents/Terum/terum-codex/refactor-b7 diff --numstat ' + BASE_REF + '...HEAD` and `git -C /Users/ryanliu/Documents/Terum/terum-codex/refactor-b7 diff --name-status ' + BASE_REF + '...HEAD`. target = `git -C /Users/ryanliu/Documents/Terum/terum-codex/refactor-b7 rev-parse --abbrev-ref HEAD`. filesOnDisk = true.\n'
    : MODE_KIND === 'working'
...
const DIFF_HOWTO = () => CWD_NOTE +
  (MODE_KIND === 'pr'
    ...
    : 'Get changed lines via `git -C /Users/ryanliu/Documents/Terum/terum-codex/refactor-b7 diff ' + manifest.baseRef + (MODE_KIND === 'branch' ? '... ' : ' ') + '-- <file>`') +
  (filesOnDisk
...
const VERIFY_PROMPT = (f, v) =>
  ...
  '## Checklist -- FIRST re-read the actual file at the cited line (Read; ' + (MODE_KIND === 'pr' ? '`gh pr diff ' + (PR || '') + '`' : '`git -C /Users/ryanliu/Documents/Terum/terum-codex/refactor-b7 diff`') + ' for the changed lines). Refute (refuted=true) if ANY of:\n' +
...
const codexArgs = "-C /Users/ryanliu/Documents/Terum/terum-codex/refactor-b7 -s read-only --ephemeral -m " + CODEX_SLUG +
...
const CODEX_RELAY_PROMPT = (f, v) =>
  ...
  'Changed lines for this target: ' + (MODE_KIND === 'pr' ? 'gh pr diff ' + (PR || '') + ' -- ' + f.file : 'git -C /Users/ryanliu/Documents/Terum/terum-codex/refactor-b7 diff ' + manifest.baseRef + (MODE_KIND === 'branch' ? '... ' : ' ') + '-- ' + f.file) + '\n' +
  'TERUM_EOF\n' +
