...
const TRIAGE_SCHEMA = {
  ...
  properties: {
    ...
    options: { type: 'array', items: {
      type: 'object', required: ['name', 'change', 'bugRisk', 'fit', 'effort', 'winsIf'],
      properties: { name: { type: 'string' }, change: { type: 'string' }, bugRisk: { type: 'number' }, fit: { type: 'number' }, fitCitation: { type: 'string' }, effort: { type: 'string' }, winsIf: { type: 'string' } },
    }},
...
//               behaviour (a product/design decision the code cannot settle), or the highest-Fit
//               option carries Bug risk 3-4 while a lower-Fit one sits at 0-1. Effort NEVER makes a fork. Goes to /decision-walk with its
...
const TRIAGE_PROMPT = (f) =>
  ...
  'Each option: name; change (what and where, concretely); fit 0-4 = how exactly the fixed behaviour is the behaviour the governing spec describes and the North Star asks for (0 contradicts a spec sentence or the North Star -- cite it; 1 both are silent and the option guesses at intent; 2 the spec is silent but the North Star or a root CLAUDE.md invariant implies this behaviour -- cite it; 3 matches a cited spec sentence; 4 matches a cited spec sentence AND it is a behaviour the North Star names as the point); fitCitation = the section + quoted sentence the fit score rests on, or "silent" -- a fit without a citation reads as 1; bugRisk 0-4 = how likely the change itself is to introduce a new defect, lower is better (0 a constant/copy/config swap the tests pin exactly; 1 mechanical code with a fallback to today\'s behaviour, worst case the old bug; 2 new logic on a tested path, a mistake is a wrong value or a red test; 3 a boundary the tests cannot exercise -- native call, second language, startup path, threading rule -- where a mistake is a crash or hang only a launched build reveals; 4 the write path, a migration or a shared invariant with no fallback) -- name the concrete failure, not the category; cause-versus-symptom is judged in fit, a symptom mask reads fit 1 at most; effort = ONE line of fact (hours, files, migration or multi-repo coordination, revert path) -- reported so the reader knows what they are buying, never a reason to prefer a less correct option (Ryan, 2026-09-06: implementation time and rework surface do not outrank correctness); winsIf = the specific condition under which THIS option beats the recommended one (for the recommended option itself: the condition under which it wins, one line). Never add or average fit and bugRisk.\n' +
  '`recommended` = 0-based index into options: the highest fit; at equal fit the lowest bugRisk; at equal both, say so and let effort break the tie out loud in whyOneOrFork. `oneClearlyWins` = true when the recommended option has the highest fit and no alternative beats it on bugRisk at equal fit (or every alternative is strictly worse). false when (a) the spec and North Star are silent and the options differ in user-visible behaviour -- a product/design decision the code cannot settle -- or (b) the highest-fit option carries bugRisk 3-4 while a lower-fit option sits at 0-1 (the spec\'s exact behaviour against the chance of a new defect is the human\'s call). Effort differences NEVER make a fork. Say which in whyOneOrFork, in plain English a non-engineer could decide from.\n\n' +
...
const optLine = (o, i, rec) => '  - ' + (i === rec ? '**' : '') + 'Option ' + (i + 1) + ': ' + o.name + (i === rec ? ' (recommended)**' : '') + ' — Fit ' + o.fit + '/4' + (nonEmpty(o.fitCitation) ? ' (' + o.fitCitation + ')' : '') + ' · Bug risk ' + o.bugRisk + '/4. ' + o.change + (nonEmpty(o.effort) ? ' *Effort (not a score):* ' + o.effort : '') + ' *Wins if:* ' + o.winsIf
