...
const WT = '/Users/ryanliu/Documents/Terum/terum-codex/rulings-walk'
const CWD_NOTE = 'IMPORTANT -- the code under review lives in a sibling git worktree at /Users/ryanliu/Documents/Terum/terum-codex/rulings-walk (branch fix/phase1-rulings), NOT in the current working directory. Run EVERY git command as `git -C /Users/ryanliu/Documents/Terum/terum-codex/rulings-walk ...` and read/grep files under /Users/ryanliu/Documents/Terum/terum-codex/rulings-walk/ (absolute paths). The diff under review is `git -C /Users/ryanliu/Documents/Terum/terum-codex/rulings-walk diff d294401...HEAD` (everything on the branch since d294401). Do not review .planning/**, .claude/**, docs, or the spec move; review code and tests. Full pass over the rulings fix branch: the twelve LOCKED rulings of .planning/decisions/2026-09-06-phase1-rulings-decision-walk.md (R1-R9, R11, R12, R14) applied one commit each — share\'s generated metadata block with the misc default (R1), publish\'s fresh branch per run with a create-only push and the open-endorsement note (R2), the vendored target lock\'s 60 s window and compromise reporting from release (R3), every sync taking the team mutex (R4), place() returning a landed swap with a notice (R5), reconcileShared\'s exits deferring (R6), describeClone shared by setup and team join with setup\'s self-heal re-clone (R8/R9), ls\'s per-row degrade (R11), search\'s unresolved flag (R12), the README link-label escape (R14) — plus the M3 walkthroughs following the minted branch. Each ruling\'s Technical section in that ledger is its spec. Treat the verdicts in that ledger and the entries in .planning/debug/harden/*.deferred.md as settled: do not re-raise the fan-out family (R10, GATED), the vendored fingerprint encoding (R13, DEFERRED), the stamp\'s meaning (R7), or R3\'s accepted rule that a lock theft\'s report replaces the work\'s error in a double failure. The docs commits at the bottom of the branch (the batch, the ledger, the register) are out of scope.\n\n'
...
const manifest = await agent(
  CWD_NOTE + '## Diff Manifest Builder (ultrareview)\n\n' +
...
for (const dim of DIMENSIONS) {
  batches.forEach((batch, bi) => {
    reviewThunks.push(() =>
      agent(CWD_NOTE + REVIEW_PROMPT(dim, batch, bi), withModel({ label: dim.key + ':' + (bi + 1), phase: 'Review', schema: FINDINGS_SCHEMA, agentType: 'Explore' }, REVIEW_MODEL))
...
if (allFindings.length >= 2) {
  ...
  const deduped = await agent(
    CWD_NOTE + '## Duplicate-Finding Merger (ultrareview)\n\n' +
...
const codexArgs = "-C " + WT + " -s read-only --ephemeral -m " + CODEX_SLUG +
  " -c model_reasoning_effort=\"" + CODEX_EFFORT + "\"" +
  (CODEX_FAST ? " -c service_tier=\"fast\"" : "") +
  " --output-schema " + WT + "/.claude/workflows/codex-verdict.schema.json"
...
const CODEX_RELAY_PROMPT = (f, v) =>
  ...
  'P=$(mktemp) ; V=$(mktemp)\n' +
  "cat > \"$P\" <<'TERUM_EOF'\n" +
  'Follow the rules in ' + WT + '/.claude/workflows/codex-verify-rules.md -- read that file FIRST, then do what it says. The repository under review is the git worktree at ' + WT + ' (your working directory); run git there and read files under it.\n\n' +
  'Review target: ' + manifest.target + ' (mode ' + MODE_KIND + ').\n' +
  ...
  toVerify.map((finding) => () =>
    parallel(
      Array.from({ length: vp.votes }, (_, v) => () =>
        agent(
          CWD_NOTE + (CODEX_VERIFY ? CODEX_RELAY_PROMPT(finding, v) : VERIFY_PROMPT(finding, v)),
      ...
      const afterRetry = await Promise.all(votes.map(async (x, v) => {
        ...
        const retry = await agent(
          CWD_NOTE + CODEX_RELAY_PROMPT(finding, v) +
...
if (triageSkip) {
  ...
  const results = await parallel(confirmed.map((f) => () =>
    agent(CWD_NOTE + TRIAGE_PROMPT(f), withModel({ label: 't:' + f.title.slice(0, 30), phase: 'Triage', schema: TRIAGE_SCHEMA, agentType: 'Explore' }, REVIEW_MODEL))
  ))
...
const report = await agent(
  CWD_NOTE + '## Synthesis: code-review report\n\n' +
