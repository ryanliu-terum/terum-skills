...
const CWD_NOTE = "\n## CWD NOTE — the review target is a SIBLING WORKTREE\\nEvery git command in this run MUST be scoped with `git -C /Users/ryanliu/Documents/Terum/terum-codex/refactor-frames` (the repo root CLAUDE.md forbids `cd`), and every file path you Read is `/Users/ryanliu/Documents/Terum/terum-codex/refactor-frames/<path>` — the paths in the manifest are repo-relative to THAT worktree, not to this process cwd. A `git diff` run without `-C` describes a different branch entirely and the review would be silently clean.\\n"
...
const manifest = await agent(
  ...
  (MODE_KIND === 'branch'
    ? '1. Run `git -C /Users/ryanliu/Documents/Terum/terum-codex/refactor-frames rev-parse --is-inside-work-tree`; if not a repo, return diffAvailable:false. baseRef = "' + BASE_REF + '". Run `git -C /Users/ryanliu/Documents/Terum/terum-codex/refactor-frames diff --numstat ' + BASE_REF + '...HEAD` and `git -C /Users/ryanliu/Documents/Terum/terum-codex/refactor-frames diff --name-status ' + BASE_REF + '...HEAD`. target = `git -C /Users/ryanliu/Documents/Terum/terum-codex/refactor-frames rev-parse --abbrev-ref HEAD`. filesOnDisk = true.\n'
    : MODE_KIND === 'working'
...
const DIFF_HOWTO = () => CWD_NOTE +
  (MODE_KIND === 'pr'
    ? 'Get changed lines via `gh pr diff ' + (PR || '') + ' -- <file>`'
    : 'Get changed lines via `git -C /Users/ryanliu/Documents/Terum/terum-codex/refactor-frames diff ' + manifest.baseRef + (MODE_KIND === 'branch' ? '... ' : ' ') + '-- <file>`') +
  (filesOnDisk
...
const VERIFY_PROMPT = (f, v) =>
  ...
  '## Checklist -- FIRST re-read the actual file at the cited line (Read; ' + (MODE_KIND === 'pr' ? '`gh pr diff ' + (PR || '') + '`' : '`git -C /Users/ryanliu/Documents/Terum/terum-codex/refactor-frames diff`') + ' for the changed lines). Refute (refuted=true) if ANY of:\n' +
...
const codexArgs = "-C /Users/ryanliu/Documents/Terum/terum-codex/refactor-frames -s read-only --ephemeral -m " + CODEX_SLUG +
...
const CODEX_RELAY_PROMPT = (f, v) =>
  ...
  'Changed lines for this target: ' + (MODE_KIND === 'pr' ? 'gh pr diff ' + (PR || '') + ' -- ' + f.file : 'git -C /Users/ryanliu/Documents/Terum/terum-codex/refactor-frames diff ' + manifest.baseRef + (MODE_KIND === 'branch' ? '... ' : ' ') + '-- ' + f.file) + '\n' +
  'TERUM_EOF\n' +
