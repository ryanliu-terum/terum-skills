...
// resolution options with Fit/Bug-risk/Wins-if (effort is a footnote, never a score -- Ryan 2026-09-06), and RATES the recommended one; the bucket is
...
const TRIAGE_PROMPT = (f) =>
  ...
  'Each option: name; change (what text changes, in which sections, concretely); fit 0-4 = how exactly the resolved text is what the decision ledger and the ratified North Star (the `north_star:` frontmatter of the newest .planning/decisions/*-decision-walk.md for this spec, or the spec\'s own "North Star check" line) already ask for (0 contradicts a ledger row or the North Star — cite it; 1 neither decides it and the option guesses; 2 the North Star implies it — cite it; 3 matches a cited ledger row or locked decision; 4 matches a cited row AND it is a behaviour the North Star names as the point); fitCitation = the row/section + quoted sentence, or "silent" — a fit without a citation reads as 1; bugRisk 0-4 = how likely the edit itself is to introduce a new defect in the spec, lower is better (0 a number, name or cross-reference the finding fully determines; 1 a wording edit in one place with the old text as the obvious revert; 2 new text in one section that other sections do not restate; 3 a rule restated elsewhere, where editing one place and not the others leaves the spec contradicting itself; 4 a contract a sibling spec, an implementer or a ledger row depends on) -- name the concrete failure, not the category; effort = ONE line of fact (a wording edit, a new sub-section, a contract change a sibling spec depends on) — reported, never a reason to prefer a less correct resolution (Ryan, 2026-09-06); winsIf = the specific condition under which THIS option beats the recommended one. Never add or average fit and bugRisk.\n' +
  '`recommended` = 0-based index: the highest fit; at equal fit the lowest bugRisk; at equal both say so and let effort break the tie out loud in whyOneOrFork. `oneClearlyWins` = true when the recommended option has the highest fit and no alternative beats it on bugRisk at equal fit. false when the ledger and North Star are silent and the options differ in what the product does (which predicate defines "orphaned", whether a feature stays, who is allowed to do what), or when the highest-fit option carries bugRisk 3-4 while a lower-fit option sits at 0-1 — those go to a human decision walk, not to you. Effort differences NEVER make a fork. Say which in whyOneOrFork, in plain English a non-engineer could decide from.\n\n' +
