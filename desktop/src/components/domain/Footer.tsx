...
export function Footer({machine,me,settings=false,team}:{team?:TeamStatus|undefined;machine:StatusResult['machine']|undefined;me?:StatusResult['me']|undefined;settings?:boolean}){
 ...
 const identity=<><div className="avatar">{me?.initials||'—'}</div><div className="footer-lines"><span className="footer-login">{me?.footerLabel||'—'}</span><span className="footer-machine" title={machine?.os||copy}>{showMachine?(copy??(machine?.name||'—')):''}</span></div></>;
