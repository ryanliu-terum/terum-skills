import { useCapabilities, usePreference, cloneStateCopy } from '../../backend';
...
export function Footer({machine,me,settings=false,team}:{team?:TeamStatus|undefined;machine:StatusResult['machine']|undefined;me?:StatusResult['me']|undefined;settings?:boolean}){
 const showMachine=usePreference('appearance:machine',true);const capabilities=useCapabilities();const copy=team?.cloneState?cloneStateCopy(team.cloneState,team.clone??'—',team.remote??'—',team.readable??true):undefined;
 const identity=<><div className="avatar">{me?.initials||'—'}</div><div className="footer-lines"><span className="footer-login">{me?.footerLabel||'—'}</span><span className="footer-machine" title={machine?.os||copy}>{showMachine?(copy??(capabilities?.machineRegistry?machine?.name||'—':'—')):''}</span></div></>;
