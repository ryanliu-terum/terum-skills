 ...
 */
function ProjectRow({root,selected,showCount}:{root:Root;selected:boolean;showCount:boolean}) {
 return <NavRow label={root.label} icon="box" href={'#/library/checkout?root='+encodeURIComponent(root.id)} selected={selected} nested count={!showCount?undefined:root.rootState==='absent'||root.rootState==='unreadable'?'—':root.count}/>;
}
