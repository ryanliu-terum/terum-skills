...
function NavRow({label,icon,href,selected=false,nested=false,expandable=false,collapsed=false,onToggle,count,trailing,menu=null}:{label:string;icon:IconName;href?:string;selected?:boolean;nested?:boolean;expandable?:boolean;collapsed?:boolean;onToggle?:()=>void;count?:string|undefined;trailing?:ReactNode;menu?:ContextMenuBuilder|null}){
 ...
 const style={paddingLeft:nested?32:8,background:selected?'var(--tk-bg3)':'transparent',color:selected?'var(--tk-text1)':'var(--tk-text2)'};
...
export function Sidebar({selected,counts,me,surfaces,roots,collapsedSections=[],onToggleSection,onHide}:{collapsedSections?:readonly string[];onToggleSection?:(section:string)=>void;onHide?:()=>void;selected:string;counts:Record<string,string>|null;me?:StatusResult['me']|undefined;surfaces?:Surfaces|undefined;roots?:readonly Root[]|undefined}){
 ...
 <NavRow label="Global" icon="globe" href="#/library/global" selected={selected==='Global'} count={displayedCounts?.Global} menu={globalRoot?()=>folderMenu(globalRoot.root,false):null}/>{surfaces?.library!==false?<><NavRow label="Projects" icon="folder" expandable collapsed={collapsedSections.includes('projects')} onToggle={()=>onToggleSection?.('projects')}/>
