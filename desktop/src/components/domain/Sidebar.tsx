...
export function Sidebar({selected,counts,machine,surfaces,projects}:{selected:string;counts:Record<string,string>|null;machine:StatusResult['machine']|undefined;surfaces?:Surfaces|undefined;projects?:readonly string[]|undefined}){
 ...
 <NavRow label="Global" icon="globe" href="#/library/global" selected={selected==='Global'} count={counts?.Global}/>{surfaces?.library!==false&&projects?.length!==0?<><NavRow label="Projects" icon="folder" href="#/marketplace/projects" expandable/>
 {(projects??['Terum','SSM','MRF']).map(name=><NavRow key={name} label={name} icon="box" href={'#/library/project/'+encodeURIComponent(projects?name:name.toLowerCase())} selected={selected.toLowerCase()===name.toLowerCase()} nested count={counts?.[name]}/>)}</>:null}
