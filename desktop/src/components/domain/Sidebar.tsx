...
export function Sidebar({selected,counts,machine,me,surfaces,team,projects}:{team?:TeamStatus|undefined;selected:string;counts:Record<string,string>|null;machine:StatusResult['machine']|undefined;me?:StatusResult['me']|undefined;surfaces?:Surfaces|undefined;projects?:readonly string[]|undefined}){
 ...
 <NavRow label="Global" icon="globe" href="#/library/global" selected={selected==='Global'} count={displayedCounts?.Global}/>{surfaces?.library!==false&&projects&&projects.length?<><NavRow label="Projects" icon="folder" href="#/marketplace/projects" expandable/>
 {projects.map(name=><NavRow key={name} label={name} icon="box" href={'#/library/project/'+encodeURIComponent(name)} selected={selected.toLowerCase()===name.toLowerCase()} nested count={displayedCounts?.[name]}/>)}</>:null}
 {surfaces?.inbox===true?<><NavRow label="Inbox" icon="inbox" href="#/inbox" expandable selected={selected==='Inbox'}/><NavRow label="Pushes" icon="arrow-down-to-line" href="#/inbox?tab=pushes" nested count={displayedCounts?.Pushes}/><NavRow label="Updates" icon="refresh" href="#/inbox?tab=updates" nested count={displayedCounts?.Updates}/><NavRow label="Alerts" icon="alert" href="#/inbox?tab=alerts" nested count={displayedCounts?.Alerts}/></>:null}
