...
export function Sidebar({selected,counts,machine}:{selected:string;counts:Record<string,string>|null;machine:StatusResult['machine']|undefined}){
 ...
 <NavRow label="Global" icon="globe" href="#/library/global" selected={selected==='Global'} count={counts?.Global}/><NavRow label="Projects" icon="folder" href="#/marketplace/projects" expandable/>
 {['Terum','SSM','MRF'].map(name=><NavRow key={name} label={name} icon="box" href={'#/library/project/'+name.toLowerCase()} selected={selected.toLowerCase()===name.toLowerCase()} nested count={counts?.[name]}/>)}
 <NavRow label="Inbox" icon="inbox" href="#/inbox" expandable selected={selected==='Inbox'}/><NavRow label="Pushes" icon="arrow-down-to-line" href="#/inbox?tab=pushes" nested count={counts?.Pushes}/><NavRow label="Updates" icon="refresh" href="#/inbox?tab=updates" nested count={counts?.Updates}/><NavRow label="Alerts" icon="alert" href="#/inbox?tab=alerts" nested count={counts?.Alerts}/>
 </div><div className="nav-group"><SectionHeader label="Team"/><NavRow label="Marketplace" icon="store" href="#/marketplace" selected={selected==='Marketplace'}/><NavRow label="Share" icon="users" href="#/share" selected={selected==='Share'}/></div></nav><Footer machine={machine} settings={selected==='Settings'}/></aside>;
}
