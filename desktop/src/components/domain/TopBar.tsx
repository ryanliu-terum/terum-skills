...
export function TopBar({mode,inboxAvailable=true}:{mode:Capabilities['windowChrome'];inboxAvailable?:boolean}){
 ...
 <div className="topbar-right" {...drag}>{inboxAvailable?<a data-badge={badge||undefined} className="shell-link icon-button bell-button" aria-label="Inbox" href="#/inbox"><Icon name="bell"/></a>:null}{error?<span role="alert">{error}</span>:null}</div></header>;
}
