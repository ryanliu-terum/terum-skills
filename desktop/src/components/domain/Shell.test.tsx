...
it('renders backend failures as visible alerts',async()=>{location.hash='#/library/global?__mock=error';render(<Providers><App/></Providers>);expect(await screen.findByRole('alert')).toHaveTextContent("ENOENT: no such file or directory, scandir '~/.terum/skills'");expect(document.querySelector('[data-error-boundary]')).toBeNull();});
