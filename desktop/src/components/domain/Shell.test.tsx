...
it('renders the frame shell, selected Global, sidebar and the footer name',async()=>{location.hash='#/frame';render(<Providers><App/></Providers>);expect(screen.getByRole('main')).toBeInTheDocument();expect(await screen.findByText('teniroo')).toBeInTheDocument();expect(screen.queryByText('teddy-mbp')).toBeNull();for(const name of ['Global','Terum','SSM','MRF','Pushes','Updates','Alerts','Marketplace','Members'])expect(screen.getByRole('link',{name:new RegExp(name)})).toBeInTheDocument();expect(screen.getByRole('button',{name:'Collapse Projects'})).toBeInTheDocument();const global=screen.getByRole('link',{name:/Global/});expect(global).toHaveAttribute('aria-current','page');expect(global.style.background).toBe('');/* the selected look is CSS on aria-current, never inline, so :hover can apply */expect(document.documentElement.dataset.appReady).toBe('true');cleanup();expect(document.documentElement.dataset.appReady).toBeUndefined();});
it('supports all capability-driven chrome modes',()=>{const view=render(<TopBar mode="cosmetic"/>);expect(view.container.querySelectorAll('[aria-hidden="true"] div')).toHaveLength(3);view.rerender(<TopBar mode="mac-overlay"/>);expect(view.container.querySelector('.topbar-left')?.firstElementChild).toHaveStyle({width:'52px'});view.rerender(<TopBar mode="mac-overlay" controlsEnd={76}/>);expect(view.container.querySelector('.topbar-left')?.firstElementChild).toHaveStyle({width:'60px'});view.rerender(<TopBar mode="native"/>);expect(view.container.querySelectorAll('.window-controls span')).toHaveLength(0);});
...
it('retains the footer name and its identity link while Library error counts are hidden',async()=>{
 location.hash='#/library/global?__mock=error';render(<Providers><App/></Providers>);
 expect(await screen.findByText('teniroo')).toBeInTheDocument();
