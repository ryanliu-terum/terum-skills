...
export function WorkflowDialog({ title, body, primary, command, children, close, submit, busy = false, error, width = 440, danger = false, aside }: PropsWithChildren<{title:string;body:string;primary:string;command:string;close:()=>void;submit:()=>void;busy?:boolean;error?:string|null;width?:number;danger?:boolean;aside?:ReactNode}>) {
  return <Dialog open onOpenChange={open=>{if(!open&&!busy)close();}}><WorkflowPopup style={{width,maxHeight:'calc(100vh - 32px)',overflowY:'auto'}}><div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:8}}><DialogTitle>{title}</DialogTitle>{aside}</div><DialogDescription><RichText text={body}/></DialogDescription>{children}<TerminalHint command={command}/>{error && <div role="alert" style={{fontSize:12,color:'var(--tk-bad)'}}>{error}</div>}<div style={{display:'flex',justifyContent:'flex-end',gap:8,marginTop:4}}><Button onClick={close} disabled={busy}>Cancel</Button><Button kind={danger?'danger':'primary'} onClick={submit} disabled={busy}>{primary}</Button></div></WorkflowPopup></Dialog>;
}
