...
import { CollapsibleCommand, RichText, TerminalHint } from './Primitives';
...
export function WorkflowDialog({ title, body, primary, command, children, close, submit, busy = false, error, width = 440, danger = false, aside, onStop, dismissKeepsRunning = false, lines, status, closeLabel = 'Cancel', hideBusyClose = false, commandSummary = null }: PropsWithChildren<{title:string;body:string;primary:string|null;command:string;/** UI policy §3: the short form drawn in place of a long `command`; the full line stays one click and one copy away. */commandSummary?:string|null;close:()=>void;submit:()=>void;busy?:boolean;error?:string|null;width?:number;danger?:boolean;aside?:ReactNode;onStop?:()=>void;dismissKeepsRunning?:boolean;lines?:readonly string[];status?:ReactNode;closeLabel?:string;hideBusyClose?:boolean}>) {
  // UI policy §1: the log pane and the error line copy on right-click; the pane copies what is on screen at click time.
  const linesMenu = useCopyMenu(() => (lines ?? []).join('\n'), 'output'), errorMenu = useCopyMenu(() => error ?? '', 'error');
  return <Dialog open onOpenChange={open=>{if(!open&&(!busy||dismissKeepsRunning))close();}}><WorkflowPopup style={{width,maxHeight:'calc(100vh - 32px)',overflowY:'auto'}}><div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:8}}><DialogTitle>{title}</DialogTitle>{aside}</div><DialogDescription><RichText text={body}/></DialogDescription>{lines?.length?<pre ref={linesMenu} className="board-mono workflow-lines" role="log" aria-live="polite">{lines.join('\n')}</pre>:null}{status?<div role="status">{status}</div>:null}{children}{commandSummary!==null?<CollapsibleCommand command={command} summary={commandSummary}/>:<TerminalHint command={command}/>}{error && <div ref={errorMenu} role="alert" className="workflow-error">{error}</div>}<div style={{display:'flex',justifyContent:'flex-end',gap:8,marginTop:4}}><>{busy&&onStop?<>{dismissKeepsRunning?<Button onClick={close}>Keep running</Button>:null}<Button kind="danger" onClick={onStop}>Stop</Button></>:busy&&hideBusyClose?null:<Button onClick={close} disabled={busy}>{closeLabel}</Button>}</>{primary!==null?<Button kind={danger?'danger':'primary'} onClick={submit} disabled={busy}>{primary}</Button>:null}</div></WorkflowPopup></Dialog>;
}
