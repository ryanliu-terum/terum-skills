...
export function WorkflowDialog({ title, body, primary, command, children, close, submit, busy = false, error, width = 440, danger = false, aside, onStop, dismissKeepsRunning = false, lines, status, closeLabel = 'Cancel' }: PropsWithChildren<{title:string;body:string;primary:string|null;command:string;close:()=>void;submit:()=>void;busy?:boolean;error?:string|null;width?:number;danger?:boolean;aside?:ReactNode;onStop?:()=>void;dismissKeepsRunning?:boolean;lines?:readonly string[];status?:ReactNode;closeLabel?:string}>) {
  return <Dialog open onOpenChange={open=>{if(!open&&(!busy||dismissKeepsRunning))close();}}><WorkflowPopup style={{width,maxHeight:'calc(100vh - 32px)',overflowY:'auto'}}><div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:8}}><DialogTitle>{title}</DialogTitle>{aside}</div><DialogDescription><RichText text={body}/></DialogDescription>{lines?.length?<pre className="board-mono workflow-lines" role="log" aria-live="polite">{lines.join('\n')}</pre>:null}{status?<div role="status">{status}</div>:null}{children}<TerminalHint command={command}/>{error && <div role="alert" style={{fontSize:12,color:'var(--tk-bad)'}}>{error}</div>}<div style={{display:'flex',justifyContent:'flex-end',gap:8,marginTop:4}}><>{busy&&onStop?<Button kind="danger" onClick={onStop}>Stop</Button>:<Button onClick={close} disabled={busy}>{closeLabel}</Button>}</>{primary!==null?<Button kind={danger?'danger':'primary'} onClick={submit} disabled={busy}>{primary}</Button>:null}</div></WorkflowPopup></Dialog>;
}
