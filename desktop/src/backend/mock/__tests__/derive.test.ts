...
it('uses receipt cost maxima for the ROI bars',()=>{expect(f.score_fractions(d.DETAIL.receipt)).toEqual({roi:[.38/.51,1]});expect(f.score_fractions(null)).toEqual({roi:null});});
it('rev 20: derives per-rep case rows that agree with the strip, null verdicts for holes, and a tally over decidable reps only',()=>{for(const s of [d.DETAIL,d.DETAIL_PARTIAL]){const rc=s.receipt!;const rows=f.case_rows_from_per_case(rc);expect(rows).toHaveLength(f.expected_rows(s));expect(rows.map(r=>r.outcomes['candidate-vs-baseline']?.[0]!.toUpperCase()??'-').join('')).toBe(s.history[0]?.rows);for(const row of rows)for(const arm of Object.values(row.arms))expect(arm.passed===null?arm.checks:arm.checks.length).toEqual(arm.passed===null?[]:2);}const partial=f.case_rows_from_per_case(d.DETAIL_PARTIAL.receipt!);expect(partial.filter(r=>r.arms['candidate']?.passed===null&&r.arms['baseline']?.passed===null)).toHaveLength(2);expect(f.case_runs_from_rows(partial)).toEqual({candidate:{passed:expect.any(Number),total:7},baseline:{passed:expect.any(Number),total:7}});const full=f.with_case_runs(d.DETAIL.receipt!);expect(full.case_runs).toEqual({candidate:{passed:14,total:18},baseline:{passed:10,total:18}});expect(f.with_case_runs(full)).toBe(full);});
