...
export type MockScenario='default'|'error'|'empty'|'loading'|'slow'|'disabled'|'not-installed'|'no-team'|'invalid-newest'|'version-mismatch'|'on-disk-only'|'partial'|'missing-root'|'no-projects'|'raw-md';
export function readScenario(hash=location.hash):MockScenario {
 ...
 switch(value){case 'error':case 'empty':case 'loading':case 'slow':case 'disabled':case 'not-installed':case 'no-team':case 'invalid-newest':case 'version-mismatch':case 'on-disk-only':case 'partial':case 'missing-root':case 'no-projects':case 'raw-md':return value;default:return 'default';}
}
