...
export function createMockBackend(opts:{latencyMs?:number}={}):Backend & {readonly quitRequested:boolean;readonly appUpdateCalls:readonly (readonly ['armOnClose',string] | readonly ['disarmOnClose'])[]} {
 ...
 const backend:Backend & {readonly quitRequested:boolean;readonly appUpdateCalls:readonly (readonly ['armOnClose',string] | readonly ['disarmOnClose'])[]} = {
  ...
  publish:args=>long('share',async ctx=>{const skill=skillByRef(args.ref);if(!skill.ok)return fail(skill.error);ctx.print(`Publishing ${args.ref}…`);if(args.project){endorsed.push({project:args.project,name:args.ref});for(const listener of listeners)listener('clone');}return ok({name:args.ref,version:skill.value.version??design.DETAIL.version,changed:true,prUrl:args.project?`https://github.com/terum/team-skills/pull/${140+endorsed.length}`:null});}),
  ...
  setup:args=>long('share',async ctx=>{ctx.print('Setting up terum-skills…');const choice=await ctx.ask('select','Create a team or join one?',{choices:['Create a new team','Join an existing team'],descriptions:['Creates a private GitHub repository under your account.','Uses an invitation from the team owner.']});const role=choice==='Create a new team'?'creator' as const:'joiner' as const;const team=args.target??design.TEAM_REPO;const steps:NonNullable<SetupResult['steps']>={role:'done',team:'done'};ctx.print('Looking for skill folders on this machine…');if(await ctx.ask('confirm','Look for skill folders on this machine and add them to your library?')){await ctx.ask('text','Look under which folder?',{default:'~'});ctx.print('No skill folders found under ~.');steps.discover='done';}else steps.discover='skipped';steps.evals=await replaySetupEvals(ctx);return ok({team,role,steps});}),
  eval:args=>long('library',async ctx=>{const detail=skillByRef(args.ref);if(!detail.ok)return fail(detail.error);ctx.print(`Evaluating ${args.ref}…`);ctx.progress(1,1,'Complete');return ok({name:args.ref,runDir:`~/.terum/skills/evals/terum/${args.ref}/20260906T120000Z`,executionStatus:'complete' as const});}),
