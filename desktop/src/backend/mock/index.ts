...
export function createMockBackend(opts:{latencyMs?:number}={}):Backend {
 ...
 async function connectPicker(ctx:RunContext,team:string):Promise<Result<ConnectOutcome>>{
  ...
  while(true){
   ...
   if(answer==='Skip')return fail('Connect was declined.');
 ...
 const backend:Backend = {
  ...
  search:args=>read('marketplace',()=>{const hits:SearchHit[]=[...design.CATALOG.map(s=>({kind:'skill' as const,ref:s.name,name:s.name,description:s.desc})),...design.PEOPLE.map(p=>({kind:'member' as const,ref:p.handle,name:p.name,description:p.role})),...design.PROJECTS.map(p=>({kind:'project' as const,ref:p.key,name:p.name,description:p.desc}))];return ok(hits.filter(h=>(!args.kinds||args.kinds.includes(h.kind))&&(h.name+' '+h.description).toLowerCase().includes(args.q.toLowerCase())));}),
  install:args=>long('library',async ctx=>{ctx.print(`Installing ${args.ref}…`);if(!args.ref.trim())return fail('A ref is required.');const detail=skillByRef(args.ref);if((args.kind??'skill')==='skill'&&!detail.ok)return fail(detail.error);if(!await ctx.ask('confirm',`Approve these tools for ${args.ref}?`))return fail('Install was declined.');ctx.progress(1,1,'Installed');return ok([{id:args.ref,name:args.ref,scope:args.scope??'Global'}]);}),
  uninstallSkill:args=>long('library',async ctx=>{const skill=skillByRef(args.ref);if(!skill.ok)return fail(skill.error);ctx.print(`Removing ${args.ref}…`);if(!await ctx.ask('confirm',`Remove ${args.ref}?`))return fail('Remove was declined.');return ok([{id:args.ref,name:args.ref}]);}),
  uninstallMachine:()=>long('settings',async ctx=>{ctx.print('Removing terum-skills…');return await ctx.ask('confirm','Remove terum-skills from this machine?')?ok({removed:design.SKILLS.map(s=>s.name)}):fail('Remove was declined.');}),
  connect:args=>long<ConnectOutcome|undefined>('share',async ctx=>{ctx.print('Connecting local skills…');if(Object.values(args).some(v=>v!==undefined)){const name=(args.path??design.LOCAL_UNSHARED[0]??'local-skill').replace(/\/$/,'').split('/').pop();if(!name)return fail('A skill folder is required.');return await ctx.ask('confirm',`Connect ${name}?`)?ok({id:name,name}):fail('Connect was declined.');}return connectPicker(ctx,design.TEAMS[0]?.key??'terum');}),
  publish:args=>long('share',async ctx=>{const skill=skillByRef(args.ref);if(!skill.ok)return fail(skill.error);ctx.print(`Publishing ${args.ref}…`);return ok({name:args.ref,version:skill.value.version??design.DETAIL.version});}),
  sync:args=>long('onboarding',async ctx=>{ctx.print('Syncing team skills…');const names=design.QUARANTINE.map(q=>q[1]??'');if(args.prune&&!await ctx.ask('confirm',`Delete ${names.length} quarantined item(s)?`))return fail('Prune was declined.');return ok({placed:design.SKILLS.map(s=>s.name),removed:args.prune?names:[]});}),
  ...
  team:args=>long('share',async ctx=>{const name=args.team??args.name??design.TEAMS[0]?.name??'Terum';ctx.print(`${args.kind}: ${name}`);if(args.kind==='leave'&&!await ctx.ask('confirm',`Leave ${name}? This removes ${design.PLACEMENTS_N} placed skill(s) from this machine.`))return fail('Leave was declined.');return ok({name,kind:args.kind});}),
