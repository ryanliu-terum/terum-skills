...
export function score_fractions(rc:Receipt|null):{roi:[number,number]|null}{if(!rc)return {roi:null};const c=Number.parseFloat(rc.eff.candidate[2]!.replace('$','')),b=Number.parseFloat(rc.eff.baseline[2]!.replace('$',''));return {roi:roiFractions(c,b)};}
export type CaseRow=NonNullable<Receipt['case_rows']>[number];
export type CaseRuns=NonNullable<Receipt['case_runs']>;
/** Rev 20 (eval-engine §5.3) on a fixture that predates it: per-rep rows laid out to agree with every number the canvas does carry — the strip's W/L/T-then-holes order per case (`rows_from_per_case`), an arm passing the reps its mean check share says it did, and a hole (`both-arms-failed`) as a null verdict for both arms. Two synthetic checks per case; a real receipt carries the case file's own check names. The fixture has no incumbent columns, so no incumbent rows. */
export function case_rows_from_per_case(rc:Pick<Receipt,'per_case'|'k'>):CaseRow[]{return per_case_rows(rc).flatMap(([name,candidate,baseline,[w,l,t],holes])=>{const strip=[...'W'.repeat(w)+'L'.repeat(l)+'T'.repeat(t)+'-'.repeat(holes)];const checks=[`file_exists:${name}.md`,`transcript_mentions:${name}`];const arm=(share:number,rep:number)=>{const passed=rep<Math.round(share*rc.k);return {passed,checks:checks.map((check,i):[string,boolean]=>[check,passed||i===1])};};return strip.map((outcome,rep)=>outcome==='-'?{case:name,rep,arms:{candidate:{passed:null,checks:[]},baseline:{passed:null,checks:[]}},outcomes:{}}:{case:name,rep,arms:{candidate:arm(candidate,rep),baseline:arm(baseline,rep)},outcomes:{'candidate-vs-baseline':({W:'win',L:'loss',T:'tie'} as const)[outcome as 'W'|'L'|'T']}});});}
/** Rev 20: passed over decidable case-runs per arm — a null verdict is neither passed nor counted. Mirrors `caseRunTally` in the engine's results.ts. */
export function case_runs_from_rows(rows:readonly CaseRow[]):CaseRuns{const tally:CaseRuns={};for(const row of rows)for(const [arm,{passed}] of Object.entries(row.arms)){const t=tally[arm]??(tally[arm]={passed:0,total:0});if(passed!==null){t.total+=1;if(passed)t.passed+=1;}}return tally;}
/** A fixture receipt with the rev-20 fields the real adapter maps from the receipt itself. Idempotent. */
export function with_case_runs<R extends Receipt>(rc:R):R{if(rc.case_rows&&rc.case_runs)return rc;const case_rows=case_rows_from_per_case(rc);return {...rc,case_rows,case_runs:case_runs_from_rows(case_rows)};}
