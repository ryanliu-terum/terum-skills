...
export function person_buckets(handle:string):[string,Card[]][]{return ['Global',...d.PROJECTS.map(q=>q.name)].map((name):[string,Card[]]=>[name,skills_by(handle).filter(s=>lists[s.name]?.[0]===name)]).filter(([,rows])=>rows.length>0);}
...
export function skill_ref(s:{repo?:string;origin:string;name:string}):string{return `${s.repo||s.origin+'/team-skills'}/${s.name}`;}
export function share_command(s:{repo?:string;origin:string;name:string;version:string}):string{return cli(`install ${skill_ref(s)}@${s.version}`);}
