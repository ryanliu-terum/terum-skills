...
const cliPublish = z.object({ name: z.string(), branch: z.string().nullable(), prUrl: z.string().nullable(), compareUrl: z.string().nullable().optional(), changed: z.boolean().optional(), outcomes: z.array(z.object({ name: z.string(), outcome: z.string(), detail: z.string().optional() })).optional() }).passthrough();
...
const cliLocalRow = z.object({ frontmatter: z.string().nullish(), name: z.string(), path: z.string(), state: z.string(), tracked: z.boolean(), shared: z.array(z.strictObject({ id: z.string(), team: z.string() })), placement: z.strictObject({ id: z.string(), team: z.string(), version: z.string().length(40).nullable() }).nullable(), health: cliLocalHealth, category: z.string().nullish().transform(v=>v??null), description: z.string().nullish().transform(value => value ?? null), characters: z.number().nullish().transform(value => value ?? null), otherAuthor: z.string().nullish().transform(value => value ?? null), problem: z.string().optional(), skillId: z.string().nullable().optional(), placed: z.boolean().optional(), connected: z.boolean().optional() }).strict();
...
function localCard(row:LocalRow,section:LocalSection,home:string):SkillCard {
  ...
  return {teamed:false,path:row.path,name:row.name,desc:row.description??'',project:labelOf(section),category:row.category??'—',installs:local?'0 installs':'—',installsN:0,installed:'placed',placed,onDiskOnly:!placed,teamState:placed?'unknown':row.connected||row.shared.length?'shared':'unshared',paths:[[abbreviateHome(row.path,home),section.scope]],connectedSources:row.connected||row.shared.length?[row.path]:[],otherAuthor:row.otherAuthor,flags:row.problem!==undefined?['broken']:local?['local']:[],flagText:row.problem!==undefined?{broken:row.problem}:local?{local:'Local · not shared with a team'}:{},grants:null,normalizedGrants:null,grantsHash:null,...tokenLabel(row.characters),wlt:null,summary:null,favorite:false,favorites:null,enabled:true,updated:null,indicators:{broken:{icon:'alert',token:'bad',text:'The skill version could not be resolved.'},update:{icon:'arrow-up-circle',token:'warn',text:''},local:{icon:'pencil',token:'text3',text:''}}};}
function notOfferedCard(entry:NotOffered,section:LocalSection,home:string):SkillCard {
  return localCard({name:entry.name,path:entry.path,state:'',tracked:false,shared:[],placement:null,health:'unknown',category:entry.category,description:entry.description,characters:entry.characters,otherAuthor:null,problem:'Not connectable · '+(entry.detail??entry.reason)},section,home);
}
...
export function createTauriBackend(bridge: Bridge = tauriBridge()): Backend {
  ...
  const backend: Backend = {
    ...
    publish: (args: PublishArgs) => run(['publish', ...(args.team ? ['--team', args.team] : []), ...(args.project ? ['--project', args.project] : []), '--', ...(args.refs ?? (args.ref === undefined ? [] : [args.ref]))], cliPublish, (value): PublishResult => ({ name: value.name, version: value.prUrl ?? value.branch ?? null, changed: value.changed ?? true, prUrl: value.prUrl ?? null, ...(value.compareUrl === undefined ? {} : { compareUrl: value.compareUrl }), ...(value.outcomes === undefined ? {} : { outcomes: value.outcomes as NonNullable<PublishResult['outcomes']> }) }), ['clone']),
