...
export const cliLocalRow = z.object({ body: z.string().nullish(), frontmatter: z.string().nullish(), name: z.string(), path: z.string(), state: z.string(), tracked: z.boolean(), placement: z.strictObject({ id: z.string(), team: z.string(), version: z.string().nullable() }).nullable(), health: cliLocalHealth, edited:z.boolean().optional(), localEval:cliEvalReport.shape.latest.optional(), localEvalStale:z.boolean().optional(), category: z.string().nullish().transform(v=>v??null), description: z.string().nullish().transform(value => value ?? null), characters: z.number().nullish().transform(value => value ?? null), updated: z.string().nullish().transform(value => value ?? null), problem: z.string().optional(), skillId: z.string().nullable().optional(), placed: z.boolean().optional() }).strict();
...
export const cliSkillFile=z.object({kind:z.enum(['move','rename','delete']),path:z.string(),destination:z.string().nullable(),quarantined:z.string().nullable(),installed:z.boolean(),notices:z.array(z.string())});
...
function localCard(row:LocalRow,section:LocalSection,home:string):SkillCard {
  ...
  return {edited:row.edited??row.health==='local-changed',localEval:summary?{...summary,runnerHandle:row.localEval?.version?row.localEval.provenance.runner_handle:null,version:row.localEval?.version??null}:null,localEvalStale:row.localEvalStale??false,installedVersion:null,latestVersion:null,evalVersion:null,evalStale:false,latestEvalState:null,profileVersion:null,teamed:false,path:row.path,name:row.name,desc:row.problem!==undefined?row.path+' · '+row.problem:cardSummary(row.body,row.description),project:labelOf(section),category:row.category??'—',installs:'—',installsN:0,installed:'placed',placed,onDiskOnly:!placed,teamState:'unknown',paths:[[abbreviateHome(row.path,home),section.scope]],projectRoots:section.repoRoot?[abbreviateHome(section.repoRoot,home)]:[],flags:row.problem!==undefined?['broken']:local?['local']:[],flagText:row.problem!==undefined?{broken:row.problem}:local?{local:'Local'}:{},grants:null,normalizedGrants:null,grantsHash:null,...tokenLabel(row.characters),wlt:summary?[summary.w,summary.l,summary.t]:null,summary,provenance:row.localEval?{model:row.localEval.provenance.model,k:row.localEval.provenance.k,ccVersion:row.localEval.provenance.cc_version,runner:row.localEval.provenance.runner_handle,when:row.localEval.provenance.timestamp.slice(0,10)}:null,favorite:false,favorites:null,enabled:true,updated:row.updated ?? null,indicators:{broken:{icon:'alert',token:'bad',text:'The skill version could not be resolved.'},update:{icon:'arrow-up-circle',token:'warn',text:''},local:{icon:'pencil',token:'text3',text:''}}};}
function notOfferedCard(entry:NotOffered,section:LocalSection,home:string):SkillCard {
  return localCard({name:entry.name,path:entry.path,state:'',tracked:false,placement:null,health:'unknown',category:entry.category,description:entry.description,characters:entry.characters,updated:null,problem:entry.detail??entry.reason},section,home);
}
