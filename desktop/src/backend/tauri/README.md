...
- `library` and `skill` are served from `ls`, `ls --local`, `status` and `validate` (S7f); `status` and `settings` from `status`, `ls --local` and `host_platform()` (S7k, a failing status with a value still serves its teams beside the error): every field the CLI does not return is `null` or `—`, never a design constant. `onboarding`, `receipts`, `inbox`, `catalog`, `roster`, `update` still fail with a GAPS.md message and are hidden by `surfaces()`; each closes in the batch that first serves it (S7b roster/catalog, S7n receipts, S7e update; S7r serves setup Boot separately, while the tour and Inbox still lack read models).
