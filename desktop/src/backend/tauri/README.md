...
- `install.scope` is passed as `--into <root>` after the adapter resolves the label to a registered checkout root (the CLI does not echo it back, so the result's `scope` is the label the app sent); `connect.keepSource`/`keepRepo`/`relocate`/`forget` map onto the CLI's `--keep-source`/`--keep-repo`/`--relocate`/`--forget`. `invite.scope`/`role` are still not passed: the CLI has no such options (GAPS.md).
