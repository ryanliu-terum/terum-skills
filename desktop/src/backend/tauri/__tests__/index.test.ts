...
it('advertises status and settings while the other eight remain typed gaps', async () => {
  const f = replay(undefined);
  const b = createTauriBackend(f.bridge);
  expect(await b.surfaces()).toEqual({ status: true, settings: true, onboarding: false, library: false, skill: false, receipts: false, inbox: false, catalog: false, roster: false, update: false });
  for (const result of await Promise.all([b.onboarding(), b.library({ scope: 'Global' }), b.skill({ ref: 'a' }), b.receipts({ skillId: 'a', version: 'abc' }), b.inbox(), b.catalog(), b.roster(), b.update()])) {
