...
it('replays the rebuilt fixture through Library, project and installed scopes and Skill detail',async()=>{
  ...
  const project=await backend.library({scope:'terum',team:'acme'});expect(project).toMatchObject({ok:true,value:{title:'1 of 3 skills · 0 in ~/.claude/skills · 0 in seed',skills:[{name:'tdd',project:'terum'}]}});
