...
it('replays the rebuilt fixture through Library, project and installed scopes and Skill detail',async()=>{
  ...
  expect(library).toMatchObject({ok:true,value:{title:'3 skills',projects:[{name:'terum',skills:['22222222-2222-4222-8222-222222222222'],remotes:['github.com/acme/terum']}],skills:expect.arrayContaining([expect.objectContaining({name:'deploy-check',desc:'a deploy needs a pre-flight checklist.',normalizedGrants:'none',installed:true,installsN:2})])}});
  ...
  const project=await backend.library({scope:'terum',team:'acme'});expect(project).toMatchObject({ok:true,value:{title:'1 skills',skills:[{name:'tdd',project:'terum'}]}});
...
it('serves the two scoped endorsement notes from recorded inventory rows',async()=>{
 ...
 expect(global).toMatchObject({ok:true,value:{title:'3 skills',overview:{skills_note:'1 endorsed to Global',installs:'3',evaluated:'—',attention:'—'}}});
 expect(project).toMatchObject({ok:true,value:{title:'1 skills',overview:{skills_note:'0 also on Global',installs:'1',evaluated:'—',attention:'—'}}});
});
