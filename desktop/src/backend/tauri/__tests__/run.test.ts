...
describe('createTauriBackend — argv and result mapping per verb', () => {
  ...
  it('capabilities: mac-overlay on macOS, every flagged gap false, editor and clipboard true', async () => {
    ...
    expect(await backend.capabilities()).toEqual({ appVersion: import.meta.env.VITE_APP_VERSION, windowChrome: 'mac-overlay', windowControlsEnd: 76, disablePerMachine: false, inboxEventLog: false, offtargetKind: false, machineRegistry: false, perCaseEvalTables: false, openInEditor: true, clipboard: true });
  });
  ...
  it('install builds the three argv shapes and maps the CLI rows to the seam', async () => {
    const f = fakeBridge((args,emit) => {
      ...
      else ok('install', [{ id: 'deploy-check', team: 'terum', path: '/p', version: 'v1', profiled: true }])(args,emit);
    });
    const backend = createTauriBackend(f.bridge);
