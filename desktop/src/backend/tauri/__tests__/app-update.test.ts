...
it('reads a marker for the running version that never reached launched as the update it evidently was', async () => {
 ...
 const stuck = { ...check, lastApply: { schema: 1, version: '0.15.0', phase: 'installing', at: '2026-09-13T23:12:14Z', error: null } };
 const h = harness(stuck);
 const first = await h.backend.appUpdate.check(); expect(first).toMatchObject({ ok: true, value: { lastApply: { version: '0.15.0', phase: 'launched', error: null } } });
