...
export type Result<T> = {ok:true;value:T}|{ok:false;error:string;value?:T};
...
export type SkillDetail=Omit<Design['DETAIL'],keyof SkillCard|'root'|'history'> & SkillCard & {root:'Global'|'Marketplace';history:(Design['DETAIL']['history'][number]&{summary:ReceiptSummary|null})[];skillMd:{frontmatter:string;body:SkillMdBlock[]};evalEstimate:EvalEstimate|null;evalEstimateText:string;evalEstimateTip:string;evalCommand:string;shareCommand:string;incumbentLift:[number,string]|null;reportNumbers:ReportNumbers;scoreFractions:{routesExpected:number;roi:[number,number]|null;quality:[number,number]|null};method:string};
export type InboxKind='share'|'update'|'alert'|'eval'|'review'|'author'|'team';
export type InboxItem=Omit<Design['INBOX'][number],'kind'> & {id:string;kind:InboxKind;summary:ReceiptSummary|null;incumbentLift:[number,string]|null;reportNumbers?:ReportNumbers};
