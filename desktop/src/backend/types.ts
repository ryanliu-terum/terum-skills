...
/** `installed` is how many distinct skills this member's committed people file records — a fact about them, not about this machine; null when the source reports no count. */
export type Member=Omit<Design['ROSTER'][number], 'followers'|'role'|'joined'> & {role:string|null;followers:number|null;joined:string|null;installed:number|null;status:string;projects:string[];lastSeen:string;lastPublish:string};
/** `invited`, `joined` and `installed` are null when the source does not report them — the screen omits the claim rather than asserting zero or a date. */
