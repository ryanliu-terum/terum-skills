...
it('models the discover and evals questions and reports both step outcomes',async()=>{
 ...
 for(const accepted of [true,false]){
  ...
  const result=await answerAll(b.setup({offerConnect:false}),frame=>{questions.push(frame.question);return frame.kind==='confirm'?accepted:frame.kind==='select'?'Join an existing team':'';});
  ...
  expect(questions).toContain('Evaluate the 1 shared skill that has no receipt yet? This runs Claude on each one and commits each receipt to the team repo.');
