...
it('assigns eleven unique kind/sub/name ids',()=>{const items=inboxItems();expect(items).toHaveLength(11);expect(new Set(items.map(it=>it.id)).size).toBe(11);for(const item of items)expect(item.id).toBe([item.kind,item.sub,item.name].filter(Boolean).join('-'));});
