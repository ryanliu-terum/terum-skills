 ...
 * appended without touching the screen (batch E, 2026-09-13). `selected` counts the DRAWN selection (what the count and
 * the Publish button promise); `held` counts every selected card, drawn or hidden by the query, so Clear can always empty
 * a selection the grid is not showing.
 */
export function LibrarySelectionBar({ selected, drawn, held, onSelectAll, onClear, onPublish, children }: { selected: number; drawn: number; held: number; onSelectAll: () => void; onClear: () => void; onPublish: () => void; children?: ReactNode }) {
  return <div className="library-selection-bar" role="toolbar" aria-label="Selection">
    ...
    <Button kind="ghost" onClick={onClear} disabled={held === 0}>Clear</Button>
