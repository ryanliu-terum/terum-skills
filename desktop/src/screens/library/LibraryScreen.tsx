...
export function LibraryScreen(){const location=useLocation(),state=useUrlState(),backend=useBackend(),ask=useContext(PromptContext),print=useContext(PrintContext),features=useFeatures(),projectAdd=useAddLibraryProject(),navigate=useNavigate(),[search,setSearch]=useSearchParams(),[actionError,setActionError]=useState<string|null>(null),[checking,setChecking]=useState(false),[reconcile,setReconcile]=useState<{result:ReconcileResult;note:string|null}|null>(null),queryClient=useQueryClient();const root=search.get('root')??'',scope:LibraryScope=location.pathname==='/library/checkout'?{kind:'checkout',root}:{kind:'global'},missing=scope.kind==='checkout'&&!root,scopeKey=scope.kind==='global'?'global':scope.root;
