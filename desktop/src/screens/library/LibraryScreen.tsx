...
import type { LibraryScope, ReconcileResult, Root } from '../../backend/types';
...
export function LibraryScreen(){const location=useLocation(),state=useUrlState(),backend=useBackend(),features=useFeatures(),projectAdd=useAddLibraryProject(),navigate=useNavigate(),[search,setSearch]=useSearchParams(),[actionError,setActionError]=useState<string|null>(null),[checking,setChecking]=useState(false),[reconcile,setReconcile]=useState<ReconcileResult|null>(null);const root=search.get('root')??'',scope:LibraryScope=location.pathname==='/library/checkout'?{kind:'checkout',root}:{kind:'global'},missing=scope.kind==='checkout'&&!root;
