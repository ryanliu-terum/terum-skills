...
export function ProgressCard({rows,placed,total,failed=false,label='Skills placed'}:{rows:Onboarding['bootRows'];placed:number|null;total:number|null;failed?:boolean;label?:string}){return <div className="onboarding-progress">{rows.map(([kind,text,right],index)=><div className="onboarding-progress-row" key={index} data-state={kind}>{kind==='pending'?<span className="onboarding-pending"/>:<Icon name={kind==='done'?'check-circle':kind==='failed'?'alert':'refresh'} size={16} stroke="1.75" color={`var(--tk-${kind==='done'?'good':kind==='failed'?'bad':'brand'})`}/>}<span><RichText text={text}/></span><span>{right}</span></div>)}<div className="onboarding-progress-track-slot"><div className="onboarding-progress-track" role="progressbar" aria-label={label} aria-valuemin={0} {...(total!==null?{'aria-valuemax':total}:{})} {...(placed!==null?{'aria-valuenow':placed}:{})} data-failed={failed||undefined}>{placed!==null&&placed>0&&total!==null&&total>0&&<span style={{width:`calc(100% * ${placed} / ${total})`}}/>}</div></div></div>;}
