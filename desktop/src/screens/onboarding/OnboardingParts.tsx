...
export function OnboardingFrame({children,steps,current,skipped}:{children:ReactNode;steps:readonly string[];current:string|null;skipped:readonly string[]}){
  ...
  return <div className="onboarding-frame"><div className="onboarding-bar">{mode==='cosmetic'?<TrafficLights/>:mode==='mac-overlay'?<div style={{width:78,flexShrink:0}}/>:null}<div className="mark-slot"><TerumMark/></div>{mode==='drawn-controls'&&<div className="window-controls" style={{marginLeft:'auto'}} aria-hidden="true"><span>−</span><span>□</span><span>×</span></div>}</div><div className="onboarding-map-band">{current&&<div className="onboarding-map" aria-label="Onboarding progress">{steps.map((name,index)=><span key={name} aria-label={name} aria-current={name===current?'step':undefined} data-skipped={skipped.includes(name)||undefined} data-state={skipped.includes(name)?'skipped':index<reached?'done':index===reached?'current':'pending'} title={name+(skipped.includes(name)?' (skipped)':'')}/>)}</div>}</div><main className="onboarding-main">{children}</main></div>;
}
