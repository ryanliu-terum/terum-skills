...
export function OnboardingBasics({data:d,tab}:{data:Onboarding;tab:string}){
  if(tab==='Manage')return <div inert aria-label="Skill card preview" style={{width:440}}><SkillCard skill={d.skill}/></div>;
  ...
  if(tab==='Share')return <div className="onboarding-share"><ShareBlock command={d.shareCommand} width={440}/><Facepile initials={d.used_by} total={d.installs_n} label={`${d.installs_n} teammates have installed this`}/></div>;
