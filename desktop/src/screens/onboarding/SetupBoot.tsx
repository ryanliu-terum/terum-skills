...
import type { Onboarding, SetupStep } from '../../backend/types';
...
export function SetupBoot({launch}:{launch:{target:string;writtenAt:string}}) {
 ...
 useEffect(()=>{void session.start(ask);},[session,ask]);
 ...
 return <ScreenFrame><OnboardingFrame steps={[]} current={null} skipped={[]}><OnboardingColumn>
  ...
  <OnboardingTitle>{failed?"Couldn't finish setup":result?.ok?'Setup finished':'Setting up your workspace'}</OnboardingTitle>
  <OnboardingPara>{launch.target}</OnboardingPara>
  ...
  <div role="status" aria-live="polite">{result?(result.ok?'Setup finished.':result.error):state.progress?.label??current??'Setup is running.'}</div>
  <div aria-label="Setup output" className="onboarding-error-line">{state.lines.map((line,index)=><div key={index}>{line}</div>)}</div>
  ...
  {result&&<OnboardingActions primary="Open the Library" onPrimary={()=>navigate('/library/global')}/>}
