...
function OnboardingFlow({step,data:d,status}:{step:string;data:Onboarding;status:StatusResult|undefined}){
  ...
  function primary(){
    ...
    if(step==='feedback'){void action.perform(()=>backend.openUrl(d.ONBOARD_COMMUNITY.invite),next);return;}
  ...
  let body:ReactNode;
  if(step==='boot')body=<><Tile icon="refresh"/><Title>Setting up {status?.teams.length===1?status.teams[0]?.name:'your workspace'}</Title><Para>{`Fetching ${d.TEAM_REPO} and placing the team's Global set on this machine.`}</Para><ProgressCard rows={d.bootRows} placed={3} total={d.BOOT_STEPS}/><TerminalHint command="npx -y terum-skills@latest sync" prefix="Same as"/></>;
