...
export function OnboardingScreen(){
  ...
  const query=useQuery({queryKey:['onboarding',state.mock],queryFn:()=>backend.onboarding()});
  const status=useQuery({queryKey:['status',state.mock],queryFn:()=>backend.status()});
  const library=useQuery({queryKey:['library','Global',state.mock],queryFn:()=>backend.library({scope:'Global'}),enabled:step==='done'});
...
function OnboardingFlow({step,data:d}:{step:string;data:Onboarding}){
  ...
  let body:ReactNode;
  if(step==='boot')body=<><Tile icon="refresh"/><Title>Setting up {d.team.name}</Title><Para>{`Fetching ${d.TEAM_REPO} and placing the team's Global set on this machine.`}</Para><ProgressCard rows={d.bootRows} placed={d.ONBOARD_PLACED} total={d.GLOBAL_SET.length}/><TerminalHint command="npx -y terum-skills@latest sync" prefix="Same as"/></>;
...
function BootError({error,data,status,retry}:{error:string;data:Onboarding|undefined;status:StatusResult|undefined;retry:()=>void}){
  ...
  return <Frame current={null} steps={[]} skipped={[]}><Column><Tile icon="alert"/><Title>Couldn't sync {data?.team.name??team?.name??'your workspace'}</Title><Para>GitHub could not be reached; your clone is still here, so the Library can open with what synced last.</Para><ProgressCard rows={rows} placed={0} total={data?.GLOBAL_SET.length??null} failed/><div className="onboarding-error-line" role="alert"><RichText text={action.error??error}/></div><Actions primary="Retry" secondary="Continue offline" icon="refresh" onPrimary={again} onSecondary={offline} busy={action.busy}/><Keys pairs={[["↵","to retry"],["esc","to continue offline"]]}/><TerminalHint command="npx -y terum-skills@latest sync" prefix="Same as"/></Column></Frame>;
}
