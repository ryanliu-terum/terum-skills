...
it('replays the default overnight choice with estimate, four descriptions and a completed Queued row',async()=>{
 ...
 expect(dialog.querySelectorAll('.prompt-option-description')).toHaveLength(4);expect(within(dialog).getByRole('radio',{name:'Overnight'})).toBeChecked();
...
it('replays Now with both progress frames and success and failure output',async()=>{
 const dialog=await reachEvals();fireEvent.click(within(dialog).getByRole('radio',{name:'Now'}));fireEvent.click(within(dialog).getByRole('button',{name:'Continue'}));
 const row=await screen.findByText('Evaluating shared skills');await waitFor(()=>expect(row.parentElement).toHaveTextContent('1 of 2'));
 ...
 await waitFor(()=>expect(row.parentElement).toHaveTextContent('2 of 2'));expect(screen.getByRole('progressbar')).toHaveAttribute('aria-valuenow','2');
 expect(screen.getByText('✗ release-notes: Hygiene failed for release-notes')).toHaveClass('setup-output-bad');
 await screen.findByRole('heading',{name:'Setup finished'});expect(row.parentElement).toHaveTextContent('Done');expect(row.parentElement).not.toHaveTextContent('2 of 2');
});
