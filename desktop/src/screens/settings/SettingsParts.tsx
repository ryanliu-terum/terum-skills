...
export function Value({children,mono=false,quiet=false,copy,title}:PropsWithChildren<{mono?:boolean;quiet?:boolean;copy?:{what:string;text?:string}|null;/** A plain-value tooltip (copyable values already say "Click to copy"). */title?:string}>){
 ...
 return copying?<CopyValue what={copying.what} {...(copying.text!==undefined?{text:copying.text}:{})} className={className} style={style}>{children}</CopyValue>:<span className={className} style={style} {...(title!==undefined?{title}:{})}>{children}</span>;
}
