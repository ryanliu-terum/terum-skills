...
it('surfaces failed editor results',async()=>{vi.spyOn(backend,'openInEditor').mockResolvedValue({ok:false,error:'Editor unavailable.'});open('#/settings/advanced');fireEvent.click(await screen.findByRole('button',{name:'Show in Finder'}));expect(await screen.findByRole('alert')).toHaveTextContent('Editor unavailable.');});
