...
import { appUpdatePolicy, stagedAppUpdate, type AppUpdateErrors } from '../../lib/app-update';
...
export function AppUpdateRows({mode,appVersion,report}:{mode:'notice'|'full';appVersion:string|null;report:UpdateAdvice|null}):ReactNode {
 ...
 else if(s.lastApply && s.lastApply.phase!=='launched'){const last=s.lastApply;desc=<>{`${v} · installing ${last.version} did not finish.`} <span role="alert">{last.error??'The installer did not report completion. Try again.'}</span></>;control=<Button onClick={()=>setRelaunch(last.version)}>Try again</Button>;}
