...
export function SettingsDialogs({section,data:d,status}:{section:string;data:Settings;status:StatusResult}){
  ...
  if(section==='machine'&&name==='prune'&&d.QUARANTINE!==null&&rows.QUARANTINE!==null)return <WorkflowDialog title={`Delete ${d.QUARANTINE.length} quarantined folder${d.QUARANTINE.length===1?'':'s'}?`} body="Prune deletes only inside ~/.terum/skills/quarantine, and only what is listed here. Nothing else on this machine is touched." primary="Delete" danger command="npx -y terum-skills@latest prune" close={close} busy={action.busy} error={action.error} submit={()=>void action.run(()=>backend.prune(),{},close)}><div className="prune-list">{rows.QUARANTINE.map(([when,name,,size])=><div key={when+name}><span>quarantine/{when}/{name}</span><span>{size}</span></div>)}</div></WorkflowDialog>;
