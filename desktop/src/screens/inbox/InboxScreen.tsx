...
export function InboxScreen() {
  ...
  const query = useQuery({ queryKey: ['inbox', state.mock], queryFn: () => backend.inbox() });
  ...
  const detail = useQuery({ queryKey: ['inbox-report-detail', item?.name, state.mock], queryFn: () => backend.skill({ ref: item?.name ?? '' }), enabled: needsDetail });
