...
export function RunEvalDialog({skill:s,open,onClose}:{skill:SkillDetail;open:boolean;onClose:()=>void}){
 ...
 function start(){try{evalRun.start({ref:s.name,name:s.name,...(s.team?{team:s.team}:{}),commit:active?.commit??commit});onClose();}catch(reason){setError(reason instanceof Error?reason.message:String(reason));}}
