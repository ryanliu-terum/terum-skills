...
/** The four D6 dialogs share the CLI's typed-name confirmation. The workflow is the PAGE's (SkillScreen),
 *  the way SettingsDialogs owns its dialogs' workflow: a cancelled or refused result lands in
...
export function SkillFileDialog({kind,skill,workflow,onClose,onDone}:{kind:'move'|'copy'|'rename'|'delete';skill:SkillDetail;workflow:ReturnType<typeof useWorkflow>;onClose:()=>void;onDone:(value:SkillFileResult)=>void}) {
 ...
 const roots=useQuery({queryKey:['library','file-destinations'],enabled:kind==='move'||kind==='copy',queryFn:()=>backend.library({scope:{kind:'global'}})});
 ...
 async function submit(){if(!skill.path)return;const path=skill.path;const outcome=await workflow.run(()=>kind==='move'?backend.skillFile.move({path,to}):kind==='copy'?backend.skillFile.copy({path,to}):kind==='rename'?backend.skillFile.rename({path,to}):backend.skillFile.delete({path}),{[`Type ${skill.name} to ${kind} this folder`]:typed});if(outcome?.ok)setResult(outcome.value);}
 ...
 {kind==='move'||kind==='copy'?<><label>{title} to<select aria-label={title+' to'} value={to} onChange={e=>setTo(e.target.value)}><option value="">Choose destination</option>{destinations.map(root=><option key={root.id} value={root.kind==='global'?'global':root.root}>{root.label}</option>)}</select></label>{roots.data?.ok===false?<ErrorLine>{roots.data.error}</ErrorLine>:null}</>:null}
