...
function InviteForm({close,data,catalog}:{close:()=>void;data:Onboarding;catalog:Catalog}){
  ...
  function submit(){const values=logins.split(/[\s,]+/).filter(Boolean);if(!values.length||values.some(login=>! /^[a-z\d](?:[a-z\d-]{0,37}[a-z\d])?$/i.test(login))){setInvalid('Enter valid GitHub logins, separated by commas or spaces.');return;}if(scope==='project'&&!selected){setInvalid('Choose a project for this invitation.');return;}setInvalid(null);void action.run(()=>backend.invite({logins:values,scope:scope==='team'?'Global':project,role:role.toLowerCase()}),{},close);}
  return <WorkflowDialog title="Invite members" body="Adds each login as a collaborator on the team repo and lists the invitation on the roster until they join." primary="Invite" command={'npx -y terum-skills@latest invite '+(logins||'<github-login>')} width={520} close={close} submit={submit} busy={action.busy} error={action.error??invalid} aside={<span className="invite-help" onMouseEnter={()=>setHelp(true)} onMouseLeave={()=>setHelp(false)}><IconButton icon="help-circle" label="Invitation help" onClick={()=>setHelp(!help)}/>{help&&<HoverTip>{<RichText text={data.INVITE_TIP}/>}</HoverTip>}</span>}>
    ...
    <div className="invite-field"><SectionLabel>Invite scoping</SectionLabel><RadioGroup value={scope} onValueChange={value=>{if(typeof value==='string')setScope(value);}} aria-label="Invite scoping" className="invite-scopes">
    ...
    <div className="invite-field"><SectionLabel>Role</SectionLabel><div className="invite-role"><Small>Admins invite and remove members.</Small><InlineChoice label="Role" value={role} options={['Member','Admin']} onChange={setRole}/></div></div>
    <div className="invite-field"><SectionLabel>Send them this</SectionLabel><CliBox command={data.joinBlock}/><Small>GitHub emails the invitation; this runs the joiner's wizard, which accepts it and offers the Global set.</Small></div>
  </WorkflowDialog>;
}
