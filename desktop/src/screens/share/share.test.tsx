...
// Review r1 HIGH (the Global sentinel's sibling): the Project radio defaulted to projects[0], and a layout-3 catalog lists Global as an
// ordinary project — first, in the re-recorded mock-vs-real ls.jsonl — so the "Project" choice silently duplicated "Whole team", which
// submit already maps to 'Global'. The default is the first project that is not Global.
it('defaults the project scope past a real Global project',async()=>{const before=await backend.catalog();if(!before.ok)throw new Error(before.error);const terum=before.value.projects.find(p=>p.key==='terum')!;vi.spyOn(backend,'catalog').mockResolvedValue({...before,value:{...before.value,projects:[{...terum,key:'Global',name:'Global'},...before.value.projects]}});const invite=vi.spyOn(backend,'invite');open('#/share?dialog=invite');await screen.findByRole('textbox',{name:'GitHub logins'});fireEvent.click(within(screen.getByRole('dialog')).getByRole('button',{name:'Invite'}));await waitFor(()=>expect(invite).toHaveBeenCalled());expect(invite.mock.calls[0]?.[0]).toMatchObject({scope:'Terum'});});
...
it('does not launch duplicate invitation runs while one is pending',async()=>{let finish:(()=>void)|undefined;const pending=new Promise<void>(resolve=>{finish=resolve;});const invite=vi.spyOn(backend,'invite').mockImplementation(()=>createRun(async()=>{await pending;return {ok:true,value:{invited:['sortiz'],already:[],failed:[]}};}));open('#/share?dialog=invite');await screen.findByRole('textbox',{name:'GitHub logins'});const button=within(screen.getByRole('dialog')).getByRole('button',{name:'Invite'});fireEvent.click(button);fireEvent.click(button);expect(invite).toHaveBeenCalledTimes(1);expect(button).toBeDisabled();finish?.();await waitFor(()=>expect(screen.queryByRole('dialog')).toBeNull());});
it('cancels an active invitation run when its screen unmounts',async()=>{const run=createRun(async ctx=>{await ctx.sleep(60_000);return {ok:true,value:{invited:['sortiz'],already:[],failed:[]}};});const cancel=vi.spyOn(run,'cancel');vi.spyOn(backend,'invite').mockReturnValue(run);const view=open('#/share?dialog=invite');await screen.findByRole('textbox',{name:'GitHub logins'});fireEvent.click(within(screen.getByRole('dialog')).getByRole('button',{name:'Invite'}));view.unmount();await waitFor(()=>expect(cancel).toHaveBeenCalled());expect((await run.done).ok).toBe(false);});
