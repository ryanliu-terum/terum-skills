...
export function ShareScreen() {
  ...
  const matches=(row:{name:string;handle:string},status:string)=>`${row.name} ${row.handle}`.toLowerCase().includes(q)&&(filter==='All'||status===filter.toLowerCase());
  return <Shell selected="Share" counts={loading||error?null:empty?{Global:'30',Terum:'22',SSM:'15',MRF:'32',Pushes:'3',Updates:'3',Alerts:'8'}:undefined}><ScreenFrame ready={(!query.isPending||state.mock==='loading')&&(state.dialog!=='invite'||!invitation.isPending||state.mock==='loading')}>
    <WorkflowHeader title="Members" icon="users" subtitle={data?empty?'1 member':`${data.members.length} members · ${data.invited.length} invitation${data.invited.length===1?'':'s'}`:undefined}>{!error&&<Button icon="user-plus" state={state.dialog==='invite'?'pressed':'default'} onClick={invite}>Invite</Button>}</WorkflowHeader>
    {error?<CenteredState alert icon="alert" title="Couldn't read the roster" body="terum-skills could not read the people files in the team clone, so this page shows nothing rather than a stale roster. Sync again, or check the clone in Settings." primary="Sync now" secondary="Open settings" onPrimary={()=>void action.run(()=>backend.sync({}),{},()=>{void query.refetch();})} onSecondary={()=>navigate('/settings/teams')}><ErrorLine>{error}</ErrorLine></CenteredState>:<>
      <div className="share-tools">{loading?<BoardSkeleton width="100%" height={32} radius={8}/>:<div className="share-search"><Icon name="search" size={16}/><input aria-label="Find members" placeholder="Find members" value={search.get('q')??''} onChange={e=>setSearch(p=>{if(e.target.value)p.set('q',e.target.value);else p.delete('q');return p;})}/>{q&&<IconButton label="Clear member search" icon="x" size={20} iconSize={14} onClick={()=>setSearch(p=>{p.delete('q');return p;})}/>}<BaseMenu.Root><BaseMenu.Trigger className="icon-button" aria-label="Filter members"><Icon name="filter" size={16}/></BaseMenu.Trigger><BaseMenu.Portal><BaseMenu.Positioner align="end" sideOffset={4}><BaseMenu.Popup className="floating-panel">{['All','Admin','Member','Invited'].map(label=><BaseMenu.Item key={label} className="menu-item" onClick={()=>setFilter(label)}>{label}</BaseMenu.Item>)}</BaseMenu.Popup></BaseMenu.Positioner></BaseMenu.Portal></BaseMenu.Root></div>}</div>
      <div className={'share-table-wrap'+(empty?' is-empty':'')}><div role="table" aria-label="Members" className="share-table"><MembersHead/>{loading?Array.from({length:13},(_,i)=><MemberSkeleton key={i}/>):<>{data?.members.map((member,index)=>matches(member,member.status)?<MemberRow key={member.handle} member={member} index={index}/>:null)}{data?.invited.map(member=>matches(member,'invited')?<InvitedRow key={member.handle} member={member}/>:null)}</>}</div>{data&&!data.members.some(row=>matches(row,row.status))&&!data.invited.some(row=>matches(row,'invited'))&&<p className="share-no-results">No members match your search.</p>}</div>
      {empty&&<CenteredState icon="users" title="Just you so far" body="Invite teammates by their GitHub login. GitHub emails each one, and the join block runs the wizard that offers them the team's Global set." primary="Invite" secondary="Copy join block" onPrimary={invite} onSecondary={()=>void action.perform(()=>backend.onboarding(),value=>{void backend.copyToClipboard(value.joinBlock).then(result=>{if(!result.ok)action.fail(result.error);},action.fail);})}><TerminalHint command="npx -y terum-skills@latest invite <github-login>" prefix="From the terminal"/></CenteredState>}
    </>}{action.error&&<div role="alert" className="share-action-error">{action.error}</div>}
    {state.dialog==='invite'&&!loading&&!error&&<ShareInvite close={close} data={invitation.data?.ok?invitation.data.value:undefined} error={invitation.data?.ok===false?invitation.data.error:invitation.isError?invitation.error.message:null} retry={()=>{void invitation.refetch();}}/>}
  </ScreenFrame></Shell>;
}
