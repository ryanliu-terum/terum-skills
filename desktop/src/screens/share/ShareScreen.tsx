...
export function ShareScreen() {
  ...
  // Which member row owns the displayed action error; null routes it to the screen-level slot.
  // The error itself lives in the screen's one workflow instance, so the next action anywhere
  // on the screen replaces it, opening the invite dismisses it, and navigating away drops it.
  ...
  const reason=query.data?.ok===false?query.data.reason:undefined;
  const loading=!data&&!error;
  const empty=data?.members.length===1&&(data.invited?.length??0)===0;
  function invite(){setNotice(null);action.clear();setSearch(p=>{p.set('dialog','invite');return p;});}
  function removeMember(handle:string){setErrorAt(handle);void action.run(()=>backend.team({kind:'remove',handle}));}
  ...
  return <Shell selected="Share" counts={loading||error?null:undefined}><ScreenFrame ready={(!query.isPending||state.mock==='loading')&&(state.dialog!=='invite'||(!settings.isPending&&(!catalogNeeded||!catalog.isPending))||state.mock==='loading')}>
    <WorkflowHeader title="Members" icon="users" subtitle={data?empty?'1 member':`${data.members.length} members`+(data.invited===null?'':` · ${data.invited.length} invitation${data.invited.length===1?'':'s'}`):undefined}>{<Button icon="user-plus" state={state.dialog==='invite'?'pressed':'default'} onClick={invite}>Invite</Button>}</WorkflowHeader>
    ...
    :<CenteredState alert icon="alert" title="Couldn't read the roster" body="terum-skills could not read the team roster, so this page shows nothing rather than a stale one. The message below is the CLI's own." primary="Sync now" secondary="Open settings" onPrimary={syncAction.open} onSecondary={()=>navigate('/settings/teams')}><ErrorLine>{error}</ErrorLine></CenteredState>:<>
      ...
      <div className={'share-table-wrap'+(empty?' is-empty':'')}><div role="table" aria-label="Members" className="share-table"><MembersHead/>{loading?Array.from({length:13},(_,i)=><MemberSkeleton key={i}/>):<>{data?.members.map((member,index)=>matches(member)?<MemberRow key={member.handle} member={member} index={index} error={errorAt===member.handle?action.error:null} onRemove={()=>removeMember(member.handle)}/>:null)}{data?.invited?.map(member=>matches(member)?<InvitedRow key={member.handle} member={member}/>:null)}</>}</div>{data&&!data.members.some(row=>matches(row))&&!data.invited?.some(row=>matches(row))&&<p className="share-no-results">No members match your search.</p>}</div>
      {empty&&<CenteredState icon="users" title="Just you so far" body="Invite teammates by their GitHub login. GitHub emails each one; the join block runs the wizard, which accepts the pending invitation with gh signed in and otherwise asks them to accept it in the browser." primary="Invite" secondary="Copy join block" onPrimary={invite} onSecondary={()=>{setErrorAt(null);void action.perform(()=>backend.settings(),value=>{
    ...
    </>}{action.error&&errorAt===null&&<div role="alert" className="share-action-error">{action.error}</div>}{notice&&<div role="status" className="share-action-notice">{notice}</div>}
