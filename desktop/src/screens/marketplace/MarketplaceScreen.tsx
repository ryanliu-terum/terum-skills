...
import type { Catalog, Features, Person, Project, Root, Run, SkillCard } from '../../backend/types';
...
import { activeFacets, plural, pluralWord, rawGrants, personStatus } from './market-data';
import { facetPredicate, parseFacets, stripFacets } from './market-facets';
...
function skillNames(catalog: Catalog, names: string[], query: string, facetMatch?: (card: SkillCard) => boolean) {
  return names.filter(name => {
    const skill = [...catalog.skills, ...catalog.extras].find(s => s.name === name);
    if (query && !(skill && matches(skill.name + ' ' + skill.desc, query))) return false;
    // A name with no catalog card passes when unqueried, as before; committed facets also leave it alone — with no card there is no datum to filter on (graceful degradation, see facetMatches).
    if (facetMatch && skill && !facetMatch(skill)) return false;
    return true;
  });
}
/**
...
function InstallDialog({ project, person, catalog, roots, features, scope, onScope, onAddFolder, error, busy, onClose, onInstall }: { project?: Project; person?: Person; catalog: Catalog; roots: Root[]; features: Features | undefined; scope: string | null; onScope: (scope: string) => void; onAddFolder: () => void; error: string | null; busy: boolean; onClose: () => void; onInstall: () => void }) {
  const projectCounts = project ? catalog.bulkInstall[project.key] : undefined;
  const asking = project ? (projectCounts ? project.skillsIn.map(name => [...catalog.skills, ...catalog.extras].find(s => s.name === name)).filter(s => s !== undefined).filter(s => rawGrants(s).length > 0) : []) : (person?.installable ?? []).map(name => [...catalog.skills, ...catalog.extras].find(s => s.name === name)).filter(s => s !== undefined).filter(s => rawGrants(s).length > 0);
  const counts = project ? projectCounts : { total: person?.installable.length ?? 0, asking: asking.length };
  const total = project?.skills ?? person?.installable.length ?? 0;
  const destinations = destinationsFor(roots, project?.remoteSlugs ?? [], { checkouts: features?.checkouts ?? false });
  return <Dialog open onOpenChange={open => { if (!open && !busy) onClose(); }}><DialogPopup>
    <DialogTitle>{project ? `Install project ${project.name}` : `Install ${person?.handle}'s skills`}</DialogTitle>
    {project ? <DialogDescription>Adds the project's {project.skills}{` ${pluralWord(project.skills, 'skill')} to your people file and places them into this repo's .claude/skills. Run it inside a checkout of `}{project.remote} — from anywhere else it exits and copies nothing.</DialogDescription> : <DialogDescription>{`Adds ${person?.handle}'s ${total} ${pluralWord(total, 'skill')} to your people file and places them into the destination you pick below.`}</DialogDescription>}
    <div className="board-column" style={{ gap: 6 }}><SectionLabel>Install to</SectionLabel>
      {features?.installScope ? <><RadioGroup aria-label="Install to" className="market-install-scopes" value={scope} disabled={busy} onValueChange={value => onScope(String(value))}>{destinations.rows.map(([label, caption]) => <RadioRow key={label} value={label} label={label} caption={caption}/>)}</RadioGroup>
        {destinations.addRow && <button type="button" className="market-install-add-folder" disabled={busy} onClick={onAddFolder}><Icon name="plus" size={14}/>Add a project folder…</button>}
        {scope === null && destinations.preselected === null && <Small>Two registered folders point at this repository — pick one.</Small>}
      </> : <div className="market-install-scopes" style={{ height: 123 }}>Global</div>}
    </div>
    {counts && <div className="market-install-grants"><SectionLabel>Tool grants to approve · {counts.asking} of {counts.total} ask</SectionLabel><div>{asking.map(s => <div className="market-install-grant-row" key={s.name}><span>{s.name}</span><div>{rawGrants(s).map(g => <Chip key={g}>{g}</Chip>)}</div></div>)}</div><Small>Approved once per machine; the others grant no tools and place without asking.</Small></div>}
    {error && <div role="alert"><ErrorLine>{error}</ErrorLine></div>}
    <TerminalHint command={project ? `npx -y terum-skills@latest install project ${project.key}` : `npx -y terum-skills@latest install member ${person?.handle}`}/><div className="market-dialog-actions"><Button disabled={busy} onClick={onClose}>Cancel</Button><Button kind="primary" disabled={busy || scope === null} onClick={onInstall}><span>{`Install ${plural(total, 'skill')}`}</span></Button></div>
  </DialogPopup></Dialog>;
}
function DetailPage({ catalog: c, project, person, busy, onRun, onError }: { catalog: Catalog; project?: Project; person?: Person; busy: boolean; onRun: (start: () => Run<unknown>, answers?: Record<string, string | boolean>, done?: () => void) => Promise<boolean>; onError: (message: string) => void }) {
  ...
  const q = project, p = person, name = q?.name ?? p?.handle ?? '', total = q?.skills ?? p?.installable.length ?? 0, ref = q?.key ?? p?.handle ?? '', names = q?.skillsIn ?? p?.skills ?? [], query = state.q ?? '', facetMatch = facetPredicate(c, parseFacets(params));
  ...
  const client = useQueryClient(), folderAction = useWorkflow(), ask = useContext(PromptContext), print = useContext(PrintContext);
  const dialogOpen = state.dialog === 'install';
  const [selection, setSelection] = useState<{ open: boolean; scope: string | null | undefined }>({ open: dialogOpen, scope: undefined });
  // Seed only once after roots arrive, and discard the choice when the URL closes the dialog.
  if (selection.open !== dialogOpen) setSelection({ open: dialogOpen, scope: undefined });
  else if (dialogOpen && selection.scope === undefined && features && (status.data?.ok || !features.installScope)) {
    setSelection({ open: true, scope: features.installScope ? destinationsFor(roots, q?.remoteSlugs ?? [], features).preselected : 'Global' });
  }
  const scope = features?.installScope === false ? 'Global' : selection.scope ?? null;
  function selectScope(scope: string) { setSelection({ open: true, scope }); }
  function addFolder() {
    if (!features?.installScope || !features.checkouts || busy) return;
    void folderAction.perform<string | null>(async () => {
      const picked = await backend.pickFolder();
      if (!picked.ok || picked.value === null) return picked;
      const added = await driveRun(backend.checkouts.add(picked.value), {}, ask, print);
      if (!added.ok) return { ok: false, error: added.error, ...(added.cancelled ? { cancelled: added.cancelled } : {}), ...(added.refused ? { refused: added.refused } : {}) };
      await client.invalidateQueries({ queryKey: ['status'], refetchType: 'none' });
      const refreshed = await status.refetch();
      if (!refreshed.data?.ok) return { ok: false, error: refreshed.data?.error ?? 'Could not read registered folders.' };
      const root = refreshed.data.value.roots.find(root => root.kind === 'checkout' && root.registered && root.id === added.value.path);
      return root ? { ok: true, value: root.label } : { ok: false, error: 'The registered folder is missing from the destination list.' };
    }, label => { if (label !== null) selectScope(label); });
  }
  const removable = q ? q.skillsIn : p?.installable ?? [];
  const installed = q ? q.installed : !!p && personStatus(p)[0] === 'Installed';
  function param(key: string, value?: string) { if (key === 'dialog') folderAction.clear(); const next = new URLSearchParams(params); if (value) next.set(key, value); else next.delete(key); setParams(next); }
  function remove() { return onRun(() => backend.uninstallSkill(q ? { ref, kind: 'project', project: ref } : { ref, kind: 'member', member: ref }), {}, () => {}); }
  function install() { if (scope === null || busy || folderAction.busy) return; return onRun(() => backend.install(q ? { ref, kind: 'project', project: ref, scope } : { ref, kind: 'member', member: ref, scope }), {}, () => { param('dialog'); }); }
...
function ListPage({ catalog: c, type, categoryKey, rosterOrder }: { catalog: Catalog; type: string; categoryKey?: string; rosterOrder: string[] }) {
  const { q = '' } = useUrlState(), [listParams, setListParams] = useSearchParams(), facetMatch = facetPredicate(c, parseFacets(listParams)); let title: string, subtitle: string, search: string, sort: string, icon = 'folder', trail: string[], body: ReactNode;
  ...
  if (type === 'skills') { title = 'Top rated'; subtitle = `${c.skills.length} of ${c.catalogN} skills · by installs, from people files`; search = `Search ${c.catalogN} skills`; sort = 'Most installed'; icon = 'store'; trail = ['Top rated']; body = <CardRow catalog={c} names={order(skillNames(c, c.topRated, q, facetMatch), name => name)}/>; }
  else if (type === 'projects') { title = 'Teams / Projects'; subtitle = `${plural(c.projects.length, 'project')} · their skills place when you sync inside the repo`; search = `Search ${plural(c.projects.length, 'project')}`; sort = 'Most members'; trail = ['Projects']; body = <div className="market-grid two">{order(c.projectsByMembers.map(name => c.projects.find(p => p.name === name)).filter(p => p !== undefined).filter(p => matches(p.name + ' ' + p.desc, q)), p => p.name).map(p => <ProjectCard key={p.key} project={p} expanded/>)}</div>; }
...
export function MarketplaceScreen() {
  ...
  const error = actionError ?? (query.data?.ok === false ? query.data.error : query.isError ? query.error.message : isPeopleList ? roster.data?.ok === false ? roster.data.error : roster.isError ? roster.error.message : null : null);
  const pending = query.isPending || (isPeopleList && roster.isPending), ready = state.mock === 'loading' || !pending;
  async function run(start: () => Run<unknown>, answers: Record<string, string | boolean> = {}, done?: () => void) { if (busy) return false; setBusy(true); try { const result = await driveRun(start(), answers, prompt, print); if (!result.ok) { if (result.cancelled) { if (state.dialog === 'install') clear('dialog'); } else setActionError(result.error); return false; } done?.(); return true; } catch (e) { setActionError(e instanceof Error ? e.message : 'Marketplace operation failed.'); return false; } finally { setBusy(false); } }
  function clear(key: string) { let next = new URLSearchParams(search); next.delete(key); if (key === 'active') { next.delete('filters'); next = stripFacets(next); } setSearch(next); }
  const active = activeFacets(state.active, state.filters === 'open' ? 4 : 0), names = catalog ? skillNames(catalog, catalog.topRated, state.q ?? '', facetPredicate(catalog, parseFacets(search))) : [];
  return <Shell selected="Marketplace" counts={pending && !error ? null : undefined}><ScreenFrame ready={ready}>{error ? <div className="market-home"><Hero {...(catalog ? { catalog } : {})}/><CenteredState alert icon="alert" title="Couldn't read the marketplace" body="terum-skills could not read the team catalog, so this page shows nothing rather than a stale catalog. The message below is the CLI's own." primary="Sync now" secondary="Open settings" onPrimary={syncAction.open} onSecondary={() => navigate('/settings/account')}><ErrorLine>{error}</ErrorLine></CenteredState></div> : unknown ? <CenteredState icon="search" title="Not found" body={unknown} primary="Back to marketplace" onPrimary={() => navigate('/marketplace')}/> : pending || !catalog ? <Loading/> : project || person ? <DetailPage key={project?.key ?? person?.handle} catalog={catalog} {...(project ? { project } : person ? { person } : {})} busy={busy} onRun={run} onError={setActionError}/> : type ? <ListPage catalog={catalog} type={type} {...(params.key ? { categoryKey: params.key } : {})} rosterOrder={roster.data?.ok ? catalog.peopleByAdoption : []}/> : <div className="market-home"><Hero catalog={catalog}/>{state.q && !names.length ? <CenteredState icon="search" title={`No skills match “${state.q}”${active ? ` with ${plural(active, 'filter')} on` : ''}`} body="Try a shorter query, or clear the filters to search the whole catalog." primary="Clear filters" secondary="Clear search" onPrimary={() => clear('active')} onSecondary={() => clear('q')}/> : state.q ? <div className="market-search-results"><CardRow catalog={catalog} names={names}/></div> : <Home catalog={catalog}/>}</div>}{catalog && state.dialog === 'new-project' ? <NewProjectDialog catalog={catalog} onClose={() => clear('dialog')} onCreated={name => { clear('dialog'); navigate('/marketplace/projects/' + encodeURIComponent(name)); }}/> : null}{syncAction.popup}</ScreenFrame></Shell>;
}
