...
import { CardRow, CategoryRow, CategoryTile, Crumbs, FilterButton, Follow, Mark, MarketRail, MarketSearch, PersonCard, ProjectCard, Section } from './market-components';
import { activeFacets, plural, pluralWord, rawGrants, personStatus } from './market-data';
import { activeFacetCount, defaultFacets, facetPredicate, parseFacets, stripFacets } from './market-facets';
import { destinationsFor } from './install-destinations';
import { RadioGroup, RadioRow } from '../../components/ui/RadioGroup';
import './marketplace.css';
// The hero row is the field plus the filter button. With no catalog (the error board reads one that failed) there are no facets to
// filter on and `MarketSearch` has no popover to render, so the button is left out rather than drawn dead (data honesty, AGENTS 7).
function Hero({ catalog }: { catalog?: Catalog }) { return <div className="market-hero"><h1>Marketplace</h1><span>Your team's best skills, all in one place.</span><div>{catalog ? <><MarketSearch hero catalog={catalog}/><FilterButton catalog={catalog}/></> : <MarketSearch hero/>}</div></div>; }
...
function DetailPage({ catalog: c, project, person, busy, onRun, onError }: { catalog: Catalog; project?: Project; person?: Person; busy: boolean; onRun: (start: () => Run<unknown>, answers?: Record<string, string | boolean>, done?: () => void) => Promise<boolean>; onError: (message: string) => void }) {
  ...
  function install(scope?: Scope) { return onRun(() => backend.install(q ? { ref, kind: 'project', project: ref, ...(scope === undefined ? {} : { scope }) } : { ref, kind: 'member', member: ref }), {}, () => { param('dialog'); }); }
...
export function MarketplaceScreen() {
  ...
  // The no-results line counts the committed facets and, with the popover open and nothing committed yet, the selection the
  // popover seeds itself with — the same number the Filter button beside it shows. The literal 4 this replaces was the
  // MOCK's default selection: on the real adapter, whose filterDefault is all-neutral, it claimed "with 4 filters on" over
  // an unfiltered catalog (review C1/C6). defaultFacets(catalog) counts 4 on the mock, so no drawn board moves.
  const active = activeFacets(state.active, state.filters === 'open' && catalog ? activeFacetCount(defaultFacets(catalog)) : 0), names = catalog ? skillNames(catalog, catalog.topRated, state.q ?? '', facetPredicate(catalog, parseFacets(search))) : [];
