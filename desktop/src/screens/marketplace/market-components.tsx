...
export function PersonCard({ person: q }: { person: Person }) { const features=useFeatures();return <article className="market-person-card" data-testid={'person-card-' + q.handle}><div className="market-card-head"><div><Avatar initials={q.initials} size={32}/></div><div className="market-person-ident"><Link to={'/marketplace/people/' + q.handle}>{q.name}</Link><span>{features?.memberRole?`${q.role} · `:''}{q.handle}</span></div><Follow person={q} compact/></div><div className="market-person-lines"><span>{q.publishLine}</span>{features?.memberRole?<span>{q.teamsLine}</span>:null}</div><div className="market-inline"><Chip>{plural(q.skills.length, 'skill')}</Chip><Chip>{plural(q.adoption, 'install')}</Chip>{features?.follow?<Chip>{q.followers===null?'—':plural(q.followers, 'follower')}</Chip>:null}</div></article>; }
