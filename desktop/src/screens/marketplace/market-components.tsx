...
function MarketplaceSkillCard({ skill }: { skill: Card }) {
  ...
  return <div className="market-card-wrap"><SkillCard skill={skill}/>{skill.installed !== 'placed' && <button className="market-card-install" type="button" title={skill.installed === 'recorded' ? 'Installed · not on this machine' : undefined} onClick={() => navigate('/skill/' + skill.name + '?dialog=install&root=marketplace')}><Icon name="arrow-down-to-line" size={14} stroke="1.75"/><span>{skill.installed === 'recorded' ? 'Reinstall' : 'Install'}</span></button>}</div>;
}
