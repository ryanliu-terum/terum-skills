...
export function MarketRail({ project, person }: { project?: Project; person?: Person }) {
  ...
  const status = q ? q.installed ? ['Installed', `${q.skills} skills placed in ${q.path}`] : ['Not installed', 'Placed when you sync in the repo'] : p ? p.onDisk[0] === p.onDisk[1] ? ['Installed', `${p.onDisk[0]} of ${p.onDisk[1]} skills on this machine`] : ['Not installed', p.placeNote] : [];
