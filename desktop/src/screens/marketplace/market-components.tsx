import { personStatus } from './market-data';
...
export function ProjectCard({ project: q, expanded = false }: { project: Project; expanded?: boolean }) { const features=useFeatures();return <Link to={'/marketplace/projects/' + q.key} className={'market-project-card' + (expanded ? ' expanded' : '')} data-testid={'project-card-' + q.key}><div className="market-card-head"><Mark name={q.ico}/><span>{q.name}</span>{expanded && <InstallMark installed={q.installed}/>}</div><span className="market-project-desc">{q.desc}</span><div className="market-card-chips"><div><Chip>{q.skills} skills</Chip>{features?.projectMembers?<Chip>{q.members} members</Chip>:null}{expanded && <Chip>{q.evaluated??'—'} of {q.skills} evaluated</Chip>}</div>{!expanded && <InstallMark installed={q.installed}/>}</div>{expanded && <div className="market-project-foot"><div><Avatar initials={q.admin.initials} size={20}/><span>{q.admin.name}</span><Small>admin · updated {q.updated}</Small></div><Small><span className="board-mono">{q.remote}</span></Small></div>}</Link>; }
export function PersonCard({ person: q }: { person: Person }) { const features=useFeatures();return <article className="market-person-card" data-testid={'person-card-' + q.handle}><div className="market-card-head"><Link to={'/marketplace/people/' + q.handle} aria-label={q.name}><Avatar initials={q.initials} size={32}/></Link><div className="market-person-ident"><Link to={'/marketplace/people/' + q.handle}>{q.name}</Link><span>{features?.memberRole?`${q.role} · `:''}{q.handle}</span></div><Follow person={q} compact/></div><div className="market-person-lines"><span>{q.publishLine}</span>{features?.memberRole?<span>{q.teamsLine}</span>:null}</div><div className="market-inline"><Chip>{plural(q.skills.length, 'skill')}</Chip><Chip>{plural(q.adoption, 'install')}</Chip>{features?.follow?<Chip>{q.followers===null?'—':plural(q.followers, 'follower')}</Chip>:null}</div></article>; }
...
export function MarketRail({ project, person, repository }: { project?: Project; person?: Person; repository:string|null }) {
  ...
  const status = q ? q.installed ? ['Installed', `${q.skills} skills placed in ${q.path}`] : ['Not installed', 'Placed by install project, inside a checkout of this repo'] : p ? personStatus(p) : [];
