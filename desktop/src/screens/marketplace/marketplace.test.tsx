...
it('renders profile and installed buckets separately, with the curated version on its card', async () => {
 const backend = pickBackend(), result = await backend.catalog(); if (!result.ok) throw new Error(result.error);
...
it('opens project install from its primary and closes after successful run', async () => { const install = vi.spyOn(pickBackend(), 'install'); open('#/marketplace/projects/docs'); fireEvent.click(await screen.findByRole('button', { name: 'Install 3 skills' })); const dialog = await screen.findByRole('dialog'); await waitFor(() => expect(within(dialog).getByRole('button', { name: 'Install 3 skills' })).toBeEnabled()); fireEvent.click(within(dialog).getByRole('button', { name: 'Install 3 skills' })); const consent = await screen.findByRole('dialog', { name: 'Approve these tools for docs?' }); fireEvent.click(within(consent).getByRole('button', { name: 'Yes' })); await waitFor(() => expect(screen.queryByRole('dialog')).toBeNull()); expect(install).toHaveBeenCalledWith({ ref: 'docs', kind: 'project', project: 'docs', scope: 'Global' }); expect(location.hash).toBe('#/marketplace/projects/docs'); });
it('selects the newly added folder as the install destination', async () => { const install = vi.spyOn(pickBackend(), 'install'); open('#/marketplace/projects/docs?dialog=install'); const dialog = await screen.findByRole('dialog'); await waitFor(() => expect(within(dialog).getByRole('radio', { name: /Global/ })).toBeChecked()); fireEvent.click(within(dialog).getByRole('button', { name: 'Add project…' })); await waitFor(() => expect(within(dialog).queryByRole('radio', { checked: true })).toBeNull()); fireEvent.click(within(dialog).getByRole('button', { name: 'Install 3 skills' })); const consent = await screen.findByRole('dialog', { name: 'Approve these tools for docs?' }); fireEvent.click(within(consent).getByRole('button', { name: 'Yes' })); await waitFor(() => expect(screen.queryByRole('dialog')).toBeNull()); expect(install).toHaveBeenCalledWith({ ref: 'docs', kind: 'project', project: 'docs', scope: 'new-project' }); });
// Review r1 HIGH (bulk-install-destination spec §3): the dialog must match a destination on ANY of the project's remotes, from
// `Project.remoteSlugs`, never a single slug re-derived from the display `remote`. The mock SSM root's origin is ryanliu-terum/ssm.
it('preselects the checkout whose origin is the project\'s second remote', async () => { const backend = pickBackend(); const before = await backend.catalog(); if (!before.ok) throw new Error(before.error); vi.spyOn(backend, 'catalog').mockResolvedValue({ ...before, value: { ...before.value, projects: before.value.projects.map(p => p.key === 'docs' ? { ...p, remoteSlugs: ['terum/docs', 'ryanliu-terum/ssm'] } : p) } }); open('#/marketplace/projects/docs?dialog=install'); const dialog = await screen.findByRole('dialog'); await waitFor(() => expect(within(dialog).getByRole('radio', { name: /SSM/ })).toBeChecked()); expect(within(dialog).getByRole('radio', { name: /Global/ })).not.toBeChecked(); });
// Spec §4.4: the mock's own fixture pre-selects a checkout on at least one project — the SSM team project carries the SSM checkout's
// origin (MOCK_ORIGIN) as a second slug beside the canvas's `github.com/terum/ssm` — while Docs, the locked board, stays on Global.
it('pre-selects the SSM checkout for the mock SSM project from the fixture alone', async () => { open('#/marketplace/projects/ssm?dialog=install'); const dialog = await screen.findByRole('dialog'); await waitFor(() => expect(within(dialog).getByRole('radio', { name: /SSM/ })).toBeChecked()); expect(within(dialog).getByRole('radio', { name: /Global/ })).not.toBeChecked(); });
