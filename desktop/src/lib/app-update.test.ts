...
import { attemptedAppUpdates, recordAppUpdateAttempt, recordAppUpdateError, stagedAppUpdate } from './app-update';
...
it('records each version attempted this session once, for the launch hook and the one-shot alike',()=>{
 const client=new QueryClient();expect(attemptedAppUpdates(client)).toEqual([]);
 recordAppUpdateAttempt(client,'0.19.0');recordAppUpdateAttempt(client,'0.19.0');recordAppUpdateAttempt(client,'0.20.0');
 expect(attemptedAppUpdates(client)).toEqual(['0.19.0','0.20.0']);client.clear();
});
