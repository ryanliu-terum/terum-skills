...
export function Providers({children}:PropsWithChildren){
 ...
 return <BackendContext value={backend}><QueryClientProvider client={client}><Tooltip.Provider><PromptProvider><EvalRunProvider>{children}</EvalRunProvider></PromptProvider></Tooltip.Provider></QueryClientProvider></BackendContext>;
}
...
function PromptDialog({question,answer,cancel}:{question:PromptQuestion;answer:(value:string|boolean)=>void;cancel:()=>void}){
 ...
 return <Dialog open onOpenChange={open=>{if(!open&&question.kind!=='confirm')cancel();}}><DialogPopup><DialogTitle>{question.question}</DialogTitle>{question.detail?.length?<DialogDescription>{question.detail.map((line,i)=><div key={i}>{line}</div>)}</DialogDescription>:null}<form onSubmit={event=>{event.preventDefault();answer(question.kind==='confirm'?true:value);}}>{question.kind==='text'?<input aria-label={question.question} value={value} onChange={event=>setValue(event.target.value)} className="prompt-field"/>:question.kind==='select'?<select aria-label={question.question} value={value} onChange={event=>setValue(event.target.value)} className="prompt-field"><option value="" disabled>Choose…</option>{question.choices?.map(choice=><option key={choice} value={choice}>{choice}</option>)}</select>:null}<div className="prompt-actions"><Button onClick={cancel}>{question.kind==='confirm'?'No':'Cancel'}</Button><Button kind="primary" type="submit" disabled={question.kind==='select'&&!question.choices?.includes(value)}>{question.kind==='confirm'?'Yes':'Continue'}</Button></div></form></DialogPopup></Dialog>;
}
