...
export type EvalRunValue = EvalResult | (EvalQueueResult & { commit?: never });
export interface EvalRunState { ref:string;name:string;team:string|undefined;queue?:boolean;run:Run<EvalRunValue>;lines:string[];startedAt:number;state:'running'|'done'|'failed'|'stopped';result?:Result<EvalRunValue>;commit:boolean;progress?:Extract<Frame,{t:'progress'}> }
