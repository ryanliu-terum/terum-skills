...
describe('board manifest',()=>{
 it('matches all 88 fidelity rows in both directions',()=>{expect(BOARDS).toHaveLength(88);expect(new Set(BOARDS.map(b=>b.name)).size).toBe(88);expect([...readFidelity().keys()].sort()).toEqual(BOARDS.map(b=>b.name).sort());});
