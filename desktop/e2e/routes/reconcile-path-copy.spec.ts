...
// UI policy §2 (2026-09-14 review): a path in "Check against the team" is click-to-copy and sits beside a checkbox.
...
test('copying a path in Check against the team never toggles its row', async ({ page, context }) => {
  await context.grantPermissions(['clipboard-read', 'clipboard-write']);
  await prepare(page, { name: 'Reconcile path copy', route: '#/library/global', klass: 'dialog', width: 1440, height: 900 });
  await page.getByRole('button', { name: 'Check against the team' }).click();
  const dialog = page.getByRole('dialog', { name: 'Check against the team' });
