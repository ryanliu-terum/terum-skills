...
describe('CLI wiring (§3: commander wiring only)', () => {
  const harness = () => {
    ...
    const program = buildProgram(execute, {
      ...
      publish: async (args) => { calls.push({ verb: 'publish', ...args }); return args.ref === 'fail' ? failure('nope') : success({ team: 't', id: 'id', name: args.ref, project: 'p', version: 'v1', created: true, identicalTo: null, attachedEvals: 0, evalAssets: 0, profileAdded: false, projectAdded: false }); },
  ...
  it('wires validate with its target and team selection', async () => {
    ...
    const program = buildProgram(async (invoke) => { await invoke(new ScriptedPrompter()); }, {
      login: async () => success({ gh: { installed: true, authenticated: true }, handle: 'me', updated: [], notice: null }), team: async () => success({ team: 't', remote: 'r' }),
      validate: async (args) => { calls.push(args); return success({ name: args.target, findings: 0, warnings: 0, repairable: 0 }); },
    });
    program.configureOutput({ writeErr: () => undefined, writeOut: () => undefined });
