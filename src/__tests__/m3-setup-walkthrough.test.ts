...
describe('M3 setup walkthrough', () => {
  ...
  it('takes Alice and Bob through setup, then delivers Alice’s published skill on Bob’s next sync', async () => {
    ...
    // The share picker is share's own (issue 9): it offers `Share <name>` labels and only on an interactive channel.
    const aliceIo = new ScriptedPrompter(['Create a new team', 'team', '', '', 'Alice', 'alice@example.com', 'team', 'bob', 'Share sample'], [true, true], true);
