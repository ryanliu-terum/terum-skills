...
const measured = z.object({ efficiency: z.object({ cost_usd: z.number().nonnegative(), duration_ms: z.number().nonnegative() }) });
...
/** Only explicit run totals qualify. Older per-arm averages cannot honestly estimate a whole run. */
  ...
  async function walk(directory: string, depth: number): Promise<void> {
    ...
    for (const entry of entries) {
      if (entry.isDirectory() && depth < 2) await walk(join(directory, entry.name), depth + 1);
        ...
        const parsed = measured.safeParse(value);
        if (!parsed.success) continue;
        costs.push(parsed.data.efficiency.cost_usd); durations.push(parsed.data.efficiency.duration_ms);
      }
    }
  }
...
export function estimateLine(count: number, estimate: EvalEstimate | null, parallel = EVAL_PARALLEL_DEFAULT): string {
  return estimate === null
    ? `Evaluating ${count} skills, ${parallel} at a time: no earlier runs to estimate from; each eval runs the skill's cases against a baseline on this machine and bills your Claude account.`
    : `Evaluating ${count} skills, ${parallel} at a time: about $${(count * estimate.costUsd).toFixed(2)} and ${duration(Math.ceil(count / parallel) * estimate.durationMs)} on this machine, from ${estimate.runs} earlier runs (median $${estimate.costUsd.toFixed(2)} · ${duration(estimate.durationMs)} each).`;
}
