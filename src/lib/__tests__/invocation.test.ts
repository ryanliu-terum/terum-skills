...
describe('runtime hint evidence (independent of provenance)', () => {
  ...
  it('ignores empty entries and skips non-files; no candidate is npx', async () => {
    // The walk joins with the host's separator, so the candidates are spelled with `join` and matched by prefix either way.
    const stat = vi.fn(async (p: string) => ({ isFile: () => p.startsWith(join('/prefix')) }));
    expect(await resolveInvocationForm(evidence({ pathEntries: ['', '/directory', '/prefix/bin'], stat }))).toBe('bare');
    expect(stat.mock.calls.map(([p]) => p)).toEqual([join('/directory', 'terum-skills'), join('/prefix/bin', 'terum-skills')]);
  ...
  it('walks past PATH entries without the executable (ENOENT, ENOTDIR) to a later match; a walk with no match is npx', async () => {
    ...
    const stat = vi.fn(async (p: string) => { if (p.startsWith(join('/prefix'))) return { isFile: () => true }; throw missing(p.startsWith(join('/file')) ? 'ENOTDIR' : 'ENOENT'); });
    expect(await resolveInvocationForm(evidence({ pathEntries: ['/usr/bin', '/file/bin', '/prefix/bin'], stat }))).toBe('bare');
    expect(stat.mock.calls.map(([p]) => p)).toEqual([join('/usr/bin', 'terum-skills'), join('/file/bin', 'terum-skills'), join('/prefix/bin', 'terum-skills')]);
