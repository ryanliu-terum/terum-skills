...
import { bareTeam, cloneWithIdentity, git, NonInteractivePrompter, pushFromSeed, ScriptedPrompter } from '../../lib/__tests__/fixtures.js';
...
describe('eval (§6 / IE2)', () => {
  ...
  it('keeps the rev-1 generated --commit refusal for a non-author', async () => {
    const fixture = await bareTeam(); await pushFromSeed(fixture.seed, 'skills/sample/SKILL.md', skill());
    const store = createConfigStore(join(fixture.root, 'state')); await cloneWithIdentity(fixture.bare, store.teamClone('team'));
    await store.update((config) => { config.teams.team = { remote: fixture.bare, handle: 'seed' }; });
    const io = new ScriptedPrompter([], [true], true);
    await expect(run({ ref: 'sample', commit: true, config: store, agent: generationAgent([]), preflight: async () => success({ ccVersion: 'stub' }) }, io)).resolves.toMatchObject({ ok: false, error: expect.stringContaining('--commit is refused for generated eval assets') });
    expect(io.asked).toEqual([]);
  });

  it('lets the author confirm generated assets into one sync commit, then receipts the new pinned tree', async () => {
    const fixture = await bareTeam(); await pushFromSeed(fixture.seed, 'skills/sample/SKILL.md', skill());
    const store = createConfigStore(join(fixture.root, 'state')); const clone = await cloneWithIdentity(fixture.bare, store.teamClone('team'));
    ...
    const io = new ScriptedPrompter([], [true], true);
    const result = await run({ ref: 'sample', commit: true, config: store, agent: generationAgent([]), k: 1, now: () => new Date('2026-09-08T12:34:56Z'), preflight: async () => success({ ccVersion: 'stub' }) }, io);
  ...
  it('treats an author declining confirmation as a run-tree-only generated run', async () => {
    const fixture = await bareTeam(); await pushFromSeed(fixture.seed, 'skills/sample/SKILL.md', skill());
    const store = createConfigStore(join(fixture.root, 'state')); const clone = await cloneWithIdentity(fixture.bare, store.teamClone('team'));
    const source = join(fixture.root, 'source'); await mkdir(source); await writeFile(join(source, 'SKILL.md'), skill());
    const baseline = await canonicalDigest(source);
  ...
  it('treats a non-interactive author as declining without asking', async () => {
    const fixture = await bareTeam(); await pushFromSeed(fixture.seed, 'skills/sample/SKILL.md', skill());
    const store = createConfigStore(join(fixture.root, 'state')); const clone = await cloneWithIdentity(fixture.bare, store.teamClone('team'));
    const source = join(fixture.root, 'source'); await mkdir(source); await writeFile(join(source, 'SKILL.md'), skill());
    const baseline = await canonicalDigest(source);
  ...
  it('never prompts an author when all eval assets are authored', async () => {
    const fixture = await bareTeam(); await pushFromSeed(fixture.seed, 'skills/sample/SKILL.md', skill());
    await pushFromSeed(fixture.seed, 'skills/sample/evals/cases/happy.yaml', 'task: deploy\nchecks:\n  - transcript_mentions: deployed\n');
    await pushFromSeed(fixture.seed, 'skills/sample/evals/triggers.yaml', 'should_trigger: [deploy]\nshould_not_trigger: [chat]\n');
    const store = createConfigStore(join(fixture.root, 'state')); await cloneWithIdentity(fixture.bare, store.teamClone('team'));
  ...
  it('keeps --gen --commit refused without prompting or replacing authored assets', async () => {
    const fixture = await bareTeam(); await pushFromSeed(fixture.seed, 'skills/sample/SKILL.md', skill());
    await pushFromSeed(fixture.seed, 'skills/sample/evals/cases/happy.yaml', 'task: deploy\nchecks:\n  - transcript_mentions: deployed\n');
    await pushFromSeed(fixture.seed, 'skills/sample/evals/triggers.yaml', 'should_trigger: [deploy]\nshould_not_trigger: [chat]\n');
    const store = createConfigStore(join(fixture.root, 'state')); await cloneWithIdentity(fixture.bare, store.teamClone('team'));
    ...
    await store.update((config) => { config.teams.team = { remote: fixture.bare, handle: 'seed' }; config.shared[ID] = { source, team: 'team', baseline }; });
    const io = new ScriptedPrompter([], [true], true);
    await expect(run({ ref: 'sample', gen: true, commit: true, config: store, agent: generationAgent([]), k: 1, preflight: async () => success({ ccVersion: 'stub' }) }, io)).resolves.toMatchObject({ ok: false, error: expect.stringContaining('--commit is refused for generated eval assets') });
    expect(io.asked).toEqual([]); expect(existsSync(join(source, 'evals'))).toBe(false);
  });
