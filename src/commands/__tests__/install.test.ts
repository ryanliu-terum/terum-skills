...
describe('install (§6 refs)', () => {
  ...
  it('a three-part ref on a machine that never joined bootstraps through setup — quiet, every prompt still asked — and then installs: one command (§6, M4 exit)', async () => {
    ...
    // Identity form: GitHub login and handle default to gh's login and are confirmed as a block after name and email; the one Claude Code question is declined as a block and then piece by piece.
    const io = new ScriptedPrompter(['Bob', 'bob@example.com'], [true, false, false, false, false]);
    ...
    expect(io.countAsked('Install these?')).toBe(1);
    expect(io.countAsked('Auto-syncs')).toBe(1);
    expect(io.countAsked('/terum-skills skill')).toBe(1);
