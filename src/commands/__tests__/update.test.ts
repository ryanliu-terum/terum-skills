...
describe('print-only update', () => {
  it.each(['global', 'local', 'npx', 'source', 'unknown'] as const)('prints the exact %s template', async (kind) => {
    ...
    const expected = {
      global: ['terum-skills 0.1.0', 'This copy: /opt/homebrew/lib/node_modules/terum-skills', `Latest advertised release: 0.1.1 (observed ${at})`, 'If installed globally with npm, run:', '  npm install -g terum-skills@latest', 'Otherwise, update it with the tool that installed this copy.', 'Then run terum-skills setup once if the session hook or /terum-skills skill names a version: it re-points them at this copy.'],
      ...
      npx: ['terum-skills 0.1.0', 'This copy: /cache/_npx/hash/node_modules/terum-skills', `Latest advertised release: 0.1.1 (observed ${at})`, 'Cache request recorded as: terum-skills@latest', "To request the registry's latest release, run:", '  npx -y terum-skills@latest <command>', 'This does not update other local or global installations.', 'The session hook and /terum-skills skill keep the version that set them up until you re-run:', '  npx -y terum-skills@latest setup'],
  ...
  it('uses nobody wording and version unknown without executing anything', async () => {
    ...
    expect(io.lines).toEqual(['terum-skills version unknown', 'This copy: /opt/homebrew/lib/node_modules/terum-skills', 'Release advertisements are not checked on this machine.', 'If installed globally with npm, run:', '  npm install -g terum-skills@latest', 'Otherwise, update it with the tool that installed this copy.', 'Then run terum-skills setup once if the session hook or /terum-skills skill names a version: it re-points them at this copy.']);
  });
