...
describe('local ls author credit', () => {
  const withAuthor = (name: string, author: string) => `---\nname: ${name}\ndescription: local\nlicense: UNLICENSED\nmetadata:\n  id: ${ID}\n  author: ${author}\n  terum-category: docs\n---\n`;
  it('credits a folder whose SKILL.md names someone else, and stays silent for the viewer\'s own', async () => {
    const home = await temporaryDirectory(); const store = createConfigStore(join(home, 'state'));
    await store.update((config) => { config.display_name = 'Ryan Liu'; config.email = 'ryan@example.com'; });
    await localSource(home, 'theirs', withAuthor('theirs', 'Mira Chen <mira@example.com>'));
    await localSource(home, 'mine', withAuthor('mine', 'Ryan Liu <ryan@example.com>'));
    // Spacing and case are not a different person: otherAuthor uses the guard's own normalizeAuthor,
    // so a folder the viewer wrote is never reported as somebody else's work.
    await localSource(home, 'sloppy', withAuthor('sloppy', '  RYAN LIU   <Ryan@Example.com> '));
    const io = new ScriptedPrompter();
    const result = await run({ local: true, home, config: store, runner: { run: async () => { throw new Error('must not run commands'); } } }, io);
    if (!result.ok) throw new Error(result.error);
    const rows = Object.fromEntries((result.value.local ?? []).flatMap(section => section.rows).map(row => [row.name, row.otherAuthor]));
    expect(rows).toEqual({ theirs: 'Mira Chen <mira@example.com>', mine: null, sloppy: null });
  });
});
