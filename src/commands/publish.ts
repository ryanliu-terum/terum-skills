...
async function notInTeam(args: PublishArgs, config: Config, team: string, name: string, stateRoot: string): Promise<string> {
  ...
  if (found.length) {
    return found.map(({ path, label }) => `No skill ${args.ref} in team ${team}. Found a local folder at ${path}${found.length > 1 ? ` (${label})` : ''} that is not tracked as a connected source or placement on this machine. To connect it to ${team}, run \`${invocation(args.form, 'connect', path)}\`, then retry \`${retry}\`.`).join('\n') + note;
  }
