...
=================== THE SPEC: .planning/specs/m7-S7ad.md ===================
...
- **AD-19:** `appStateSchema` gains `path: z.string().nullable().optional()` and `target: z.string().optional()`, and `writtenAt` becomes REQUIRED (the consume-once key); `spawn` passes `path: state.path ?? null` to `cli_spawn`. The onboarding route's consume-once rule (read `target`, compare `writtenAt` with the pref, route to the wizard once) is S7r's: here only expose `target` and `writtenAt` on the parsed state and add a seam method `Backend.launchTarget(): Promise<{ target: string; writtenAt: string } | null>` (mock: `null`).
