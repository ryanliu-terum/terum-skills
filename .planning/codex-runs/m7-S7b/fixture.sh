...
set -euo pipefail
FX=${1:?fixture root}; CLI=/Users/ryanliu/Documents/Terum/terum-codex/m7-S7b/dist/index.js
...
p=Path('people/mira.json');d=json.loads(p.read_text());d.update(role='Platform',projects=['terum']);p.write_text(json.dumps(d)+'\n');assert json.loads(p.read_text())==d
PYMETA
cat > team.json <<J
...
# Route the fixture's public-looking remote to its local bare host; writes remain offline.
...
{"default_handle":"seed","email":"seed@example.com","display_name":"Seed","github":"seed","teams":{"acme":{"remote":"$FX/repo/team.git","handle":"seed"}},"shared":{"$ID_TDD":{"source":"$FX/repo/seed/skills/tdd","team":"acme"}},"approvals":{},"pending":[],"placements":{"$HOME/.claude/skills/deploy-check":{"id":"$ID_DEPLOY","team":"acme","version":"$VER","scope":{"kind":"global"},"placed_at":"2026-09-01T00:00:00Z","fingerprint":"fixture"}}}
J
