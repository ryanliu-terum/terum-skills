...
# The state the installed-state captures show: the m7-S7b team (acme: mira with role Platform in project terum,
# seed with role Platform, ravi; deploy-check/tdd/diagnose at the 1111/2222/3333 ids) and, for the `ls member`
# frames, mira's installed[] exactly as ls-member-installed.jsonl records it (deploy-check and tdd, both global,
# since 2026-09-01). Seeded directly in layout 3 (skills/<name>/v1/, team.json layout_version 3, projects.Global).
# This is the "placed" local state: seed's deploy-check placed in the global root and recorded in the ledger; the
...
# the value the committed frames carry keeps every recording byte-identical run to run.
...
person() { # handle display email installedJson extraJson — a seeded installed[].version of null stays null under layout 3
  cat > "$FX/repo/seed/people/$1.json" <<J
{"handle":"$1","display_name":"$2","email":"$3","github":"$1","bio":"","installed":$4,"declined":[]$5}
J
}
person seed "Seed" seed@example.com "[{\"id\":\"$ID_DEPLOY\",\"version\":null,\"scope\":{\"kind\":\"global\"},\"since\":\"2026-08-20T00:00:00Z\"}]" ',"role":"Platform"'
person mira "Mira Chen" mira@example.com "[{\"id\":\"$ID_DEPLOY\",\"version\":null,\"scope\":{\"kind\":\"global\"},\"since\":\"2026-09-01\"},{\"id\":\"$ID_TDD\",\"version\":null,\"scope\":{\"kind\":\"global\"},\"since\":\"2026-09-01\"}]" ',"role":"Platform","projects":["terum"]'
