...
set -euo pipefail
FX=${1:?fixture root}; CLI=/Users/ryanliu/Documents/Terum/terum-codex/m7-S7e/dist/index.js
...
skill() { # name id author category description
  mkdir -p "$FX/repo/seed/skills/$1"; cat > "$FX/repo/seed/skills/$1/SKILL.md" <<MD
---
name: $1
description: $5
license: UNLICENSED
...
mkdir -p "$HOME/.claude/skills/deploy-check"; cp "$FX/repo/seed/skills/deploy-check/SKILL.md" "$HOME/.claude/skills/deploy-check/SKILL.md"
...
# S7e offline recording: a local git team disables release probing under github-teams policy.
