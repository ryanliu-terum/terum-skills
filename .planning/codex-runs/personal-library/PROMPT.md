Implement the spec below. Read AGENTS.md at the repo root FIRST, then desktop/AGENTS.md (the eight invariants of the desktop app) and follow both exactly. Read every per-directory CLAUDE.md / README for a subtree you touch before your first edit there.
...
- Scope: desktop only. Do not edit anything under `src/` (the CLI), `docs/`, `desktop/GAPS.md`, `desktop/FIDELITY.md`, `desktop/AGENTS.md`, `desktop/README.md`, `desktop/package.json`, `desktop/src/fixtures/design.json`, `desktop/src/styles/tokens.css`, or `.planning/` (the recorded frames there are read-only inputs). Do not create a Marketplace ▸ Team section.
- The `.planning/codex-runs/personal-library/frames/*.jsonl` recordings named in §4 already exist in the worktree; read them with `readFileSync(resolve('../.planning/codex-runs/personal-library/frames', name + '.jsonl'))` from the desktop test cwd, exactly like `desktop/src/backend/tauri/__tests__/replay.test.ts` does for S7g.
- `node_modules` are already installed (root is a symlink, `desktop/node_modules` is a real directory). No network, no `npm install`, no git commands of any kind (not even `git status` or `git diff`), no commits, no staging; leave every change in the working tree.
- Gates you must run yourself from the `desktop/` directory and report with real counts: `npm run typecheck`, `npm run lint` (it runs with `--max-warnings 0`), and `NODE_OPTIONS=--no-experimental-webstorage npm test`. Do not run Playwright (no browser in the sandbox) and do not run the root suite (`src/` is untouched); the orchestrator runs those. If a gate cannot run in the sandbox, say so in the report; never fake a number.
- Tests: never delete or weaken a test to make the suite pass; every test you change or add goes in `testsModified`. Every rewritten assertion must state the new contract (the roots model), not merely drop the old one. New tests must pass on this tree and must fail on a tree without the roots model (the orchestrator checks this on a scratch copy of main).
- No `eslint-disable` without a reason on the same line; no test-only branches in components; every board state reachable from the URL and the `?__mock=` scenario switch.
- The design fixture and tokens are generated; never hand-edit them. Pixel geometry of every existing board must not change: the mock's sidebar rows, counts, labels and order, the Library Global page, the Settings ▸ This machine page (the Checkouts group is hidden on the mock through `surfaces.checkouts:false`) and every dialog render exactly as before.
- If the spec is ambiguous, implement the most conservative reading and record the question in `openQuestions`; never resolve a design fork yourself. Record any deliberate departure in `deviations`.
- Verify every write by reading the file back. Report `invariantsTouched` per desktop/AGENTS.md.
