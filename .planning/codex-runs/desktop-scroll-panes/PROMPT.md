Implement the spec below. Read AGENTS.md at the repo root FIRST, then desktop/AGENTS.md (the desktop app's own invariants; follow both exactly). All work is under desktop/.
...
- Work only inside `desktop/`. Read `desktop/AGENTS.md` before your first edit; its invariant 5 (pixels come from the boards) and 7 (no shortcuts in tests) govern this change.
- Gates you run, from `desktop/`: `npm run typecheck && npm run lint && npm test`. Report real counts (files / tests). Playwright cannot launch inside the sandbox: write `e2e/routes/scroll.spec.ts` carefully in the exact style of `e2e/routes/card-click.spec.ts`, say in the report that you could not run it, and do not fake a result. The orchestrator runs the route battery and the 88-board fidelity gate outside the sandbox.
- Never edit `GAPS.md`, `FIDELITY.md`, `AGENTS.md`, `README.md`, `package.json`, `src/styles/tokens.css`, `src/fixtures/design.json`, or anything under `e2e/fidelity/`. Describe anything those would need in `openQuestions`.
- Never delete or weaken an existing test; declare every test change in `testsModified`.
- No git commands of any kind (not even `git status`): this is a worktree and git metadata is outside the writable root. Verify every write by reading the file back. Do not commit, stage or push; leave changes in the working tree.
