...
| **B9 — auto-category** (`.planning/specs/2026-09-10-auto-category.md`, D28) | that spec §1–§7, amended: the suggestion happens at **first publish**, not connect | new `src/lib/categorize.ts`; `askJson` gains `settingSources`; `--category` on `publish`; `src/lib/evals/hygiene.ts` gains `HYG7` and `assessHygiene`'s trailing `categories?` parameter (its sixth; the fifth is `managedFieldsAbsent`); publish passes `team.categories`; `cli.ts` help gains HYG7 | `categorize.ts` grading fixture; HYG7 warns on an off-list declared category and no-ops without a list | Split out of B3 by D28: it is a feature with a model call in it, serving its own North Star ("browse actually works"), and B3 is the keystone every other batch waits on. **Its §6 caller list names `connect.ts`, `publish.ts:64/:118`, `eval.ts:98` and `validate.ts:42` — all pre-B1 — was rewritten to publish-only on 2026-09-13 (refactor spec rev 9).** Runs after B5 | M |
