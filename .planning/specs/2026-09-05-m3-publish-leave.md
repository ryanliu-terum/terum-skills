...
export interface PublishResult {
  ...
  branch: string | null;       // the fresh 'publish/<name>-<handle>-<id8>' under "pr"; null under "push", when unchanged, or when an open endorsement was kept (R2)
...
2. **Bring the clone current.** `refreshClone(runner, clone, { label: team })` from `teamRepo.ts` — `git fetch origin` then `git reset --hard origin/main`, safeWrite's own recovery, shared with `sync` (D5b, 2026-09-05 close-out walk; `pull --ff-only` wedged permanently on a clone whose local `main` had drifted). Failure → `Could not refresh <team>: <git's message>`. Reason: the publish policy and the endorsed lists are read from the clone next, and a stale clone must not let a member publish under a policy the team has since changed.
3. **Read policy and skill.** `readTeam(clone)` → `policy.publish` (`'pr' | 'push'`; the schema already restricts it). `findSkill(clone, team, reference.name)` → the `SkillRecord` (`id`, `name`); Missing → return failure with the complete recovery message specified for issue 7. A readable, eligible local candidate under either the global or nearest-repository project Claude Code skills root supplies an absolute-path `connect` command (both paths, scope-labelled, for a duplicate name; unreadable counts summed across roots; build spec §6, Ryan 2026-09-06); otherwise supply team-qualified `ls` and explicit-path `connect` guidance. Preserve the resolved team and requested project in recovery commands. Discovery is best-effort and never imports a skill, edits a source, records tracking, or prompts.
...
8. **The write.** `openTeamRepo(clone, binding.remote, runner).safeWrite(mutate, options)` with `options = { action: 'publish', handle, message: '<handle>: publish <name>', ...(policy === 'pr' ? { branch: 'publish/<name>-<handle>-<id8>' } : {}), ...args.safeWrite }` — the branch name is minted fresh for every run (R2). **No `token` field.** Under `'push'` the write lands on `main` and the command prints `Published <name> to <team> (<global|project p>).` and returns with `branch: null`.
8a. **Endorsement branches already on the remote for this skill** (rulings walk R2, 2026-09-06, superseding the abandoned-attempt reuse rule of D2): before anything is committed, one `git ls-remote --heads origin refs/heads/publish/<name> refs/heads/publish/<name>-*` lists them — the fresh form and the pre-R2 `publish/<name>` / `publish/<name>-2` a team may still carry; with gh on a GitHub remote each branch's open PR URL comes from `gh pr list --head <branch> --state open`, and a branch whose PR is no longer open is dropped as a leftover. Each is printed as `An endorsement of <name> is already open: <url|branch> (by <handle>).`, then `Open another pull request for <name>? If both merge, GitHub will flag the second as conflicting.` (y/N). No → `Nothing pushed; <name> keeps its open endorsement.`, `changed: false`, `prUrl` = the open one. Yes → a fresh branch exactly as in step 8. Nothing is ever refused on a branch's account and nothing is ever force-pushed; the push is create-only. The mutation also re-reads `policy.publish` from the reset tree and throws if it differs from the policy the branch was chosen under.
9. **Under `'pr'`, after the branch is pushed** (`result.pushedTo` is the fresh branch):
...
1. **`pr` policy with gh present (the parent's §12 case):** `originSha(bare, 'main')` is identical before and after; the freshly minted `refs/heads/publish/<name>-<handle>-<id8>` (read the actual name from `result.value.branch` — the id8 is random) exists on the bare and `git show <branch>:team.json` lists the ID in `global`; `gh pr create` was called once with `--base main` and `--head <branch>` and `-R acme/team`; the result carries the PR URL the fake gh returned; no `git push` in `runner.calls` targeted `refs/heads/main`. (Rewritten 2026-09-07 walk D3 — the pre-R2 `publish/<name>` shape.)
2. **`pr` policy with gh absent** (`mappedRunner` with no gh handler): same fresh branch pushed, `origin/main` unchanged, `ok: true`, the printed lines contain the compare URL `https://github.com/acme/team/compare/main...<branch>?expand=1`, and **no push to `main` ever happened** (assert on `runner.calls`). Also cover **gh installed but logged out** (`fakeGh('me', {}, false)`): identical outcome, and `pr create` never called.
3. **Create-only, no reuse, no fallback (R2):** an existing endorsement branch for the skill (any form — fresh-shape or legacy `publish/<name>`) is surfaced by the §8a vet with its PR URL and a y/N; on yes, a **second, independently named** fresh branch is pushed create-only — the existing branch's commit is untouched, nothing is force-pushed, and no `-2` (or any derived) fallback name exists anywhere in `runner.calls`. On no, nothing is pushed and the result points at the open endorsement. A push race (the fresh name appearing on the remote between mint and push) surfaces as a refused create-only push and a retried publish mints a different id8 — never a clobber.
...
1. Alice (`aStore`, bound as `seed` — reuse the seed people file the fixture already has) `share`s a skill (as `m2-walkthrough.test.ts` does), then `publish`es it under the default `pr` policy → the fresh `publish/<name>-<handle>-<id8>` from `result.value.branch` exists, `origin/main` unchanged.
2. Simulate the merge from the seed clone: `git fetch origin`, `git push origin refs/remotes/origin/<branch>:main` (a fast-forward, using the branch name from step 1's result), and assert `git show main:team.json` on the bare now lists the ID.
