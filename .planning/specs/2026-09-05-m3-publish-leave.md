...
3. **Read policy and skill.** `readTeam(clone)` → `policy.publish` (`'pr' | 'push'`; the schema already restricts it). `findSkill(clone, team, reference.name)` → the `SkillRecord` (`id`, `name`); Missing → return failure with the complete recovery message specified for issue 7. A readable, eligible local candidate under either the global or nearest-repository project Claude Code skills root supplies an absolute-path `share` command (both paths, scope-labelled, for a duplicate name; unreadable counts summed across roots; build spec §6, Ryan 2026-09-06); otherwise supply team-qualified `ls` and explicit-path `share` guidance. Preserve the resolved team and requested project in recovery commands. Discovery is best-effort and never imports a skill, edits a source, records tracking, or prompts.
