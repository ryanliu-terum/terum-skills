...
**Status:** PLAN (rev 3, 2026-09-06). Rev 3 rebases the plan on finalized phase-1
...
Authoritative designs stay in `2026-09-04-eval-engine.md` (**rev 9**) and
...
- **`publish`** — LANDED (PR #2 `765032c`). Its `safeWrite` writes only `team.json`,
  ...
  site regardless: `commands/publish.ts` calls the same hygiene function on the
  clone-resident `skills/<name>/` tree after the record resolves and **before** the
  branch/PR path is chosen and before `safeWrite`, so one call gates both
  `policy.publish === 'pr'` and `'push'` and no endorsement branch is minted for a
  skill that fails. Hygiene's fail-closed refusal wins over the softer
...
- **Receipt check on publish PRs [default — veto cheap].** On `publish/`-prefixed
  ...
  so the trigger matches the `publish/` prefix, not a literal `publish/<name>` — the
  PR must carry a committed receipt for the endorsed skill at its current tree-hash
  version that parses against `receiptSchema` and includes a candidate-vs-incumbent
  comparison; missing or stale receipt blocks (the incumbent regression gate per
  eval spec §6.1, in evidence form). The receipt is produced locally by the
  publisher on their own subscription auth (IE2/IE3) — CI validates evidence, it
  never generates it. This replaces the former key-gated blocking eval gate.
...
someone with push access. The CLI never writes `.github/workflows/**`; the guard's
...
scaffold; a test asserts no verb stages a `.github/workflows/**` path.
