...
**Status:** DRAFT for Ryan's approval (2026-09-09). Not locked; §7 holds the questions that must be answered before an implementer starts.
...
- **Dialog** (`components/ui/Dialog.tsx`, the shape `InstallDialog` already uses): title *New project*; field 1 *Project name* (required, validated live against `PROJECT_NAME_RULE`, and against the names already in `catalog.projects` case-insensitively, so the collision is caught before a round trip); field 2 *GitHub repository* (optional, placeholder `https://github.com/org/repo`, helper text *Its skills place when a teammate syncs inside this repo*); Cancel and Create. Create is disabled until the name is valid. A CLI failure is shown in the dialog verbatim and the dialog stays open with the values intact.
...
- The empty project card must render: `skills 0`, `members 0`, remote `—` when none. `catalogModel` (`tauri/index.ts:218`) already computes `installed` as `project.skills.length > 0 && …`, so an empty project reads as not-installed — correct, and the detail page must not offer *Install 0 skills*; it offers **Add skills** (§4).
