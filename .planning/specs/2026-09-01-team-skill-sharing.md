...
- Scope is not a property of the skill and is not derived from any folder (D17): it is which `team.json` list references the skill's ID. **Scope is the package**: which team.json list the install came from (Global, or a project). The folder is machine-local and never shared.
...
| D17 | A skill has no inherent scope; scope is which `team.json` lists reference its ID. **Scope is the package**: which team.json list the install came from (Global, or a project). The folder is machine-local and never shared. (Revised 2026-09-03; supersedes position-derived scope.) | DECIDED |
