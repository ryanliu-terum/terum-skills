...
- **Stale recovery:** a lock is stale if `started` is more than 10 minutes old, or if `host` is this machine and `pid` is not alive. A stale lock is reclaimed (moved aside atomically, removed only while it is still the record that was judged; a live record displaced instead is re-created with an exclusive create, never written over a holder that took the freed path meanwhile) and re-acquired once; a lock held by another host is always respected. Without this rule a crashed process disables sync forever.
