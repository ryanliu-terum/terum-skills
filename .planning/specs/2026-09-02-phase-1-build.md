...
src/
  ...
  lib/
    ...
    prompt.ts           Prompter interface (confirm / text / select / print) — the only channel a verb
                        uses to talk to a human; a non-interactive variant for --hook (rev 7)
...
- **`login`** — admins. `gh auth status` → use gh. **`gh` installed but logged out → offer (y/N) to run `gh auth login` as a child process with inherited stdio — gh's own interactive flow, not the `--with-token` form §5.4 forbids — then re-check `gh auth status`** (rev 7; V10). Declined, or no `gh` on the machine → prompt for a **per-team** fine-grained PAT (repo Administration + Contents), verify with a probe, store per-team. Collects handle (§5.4 rules), name, and email for `metadata.author` on first run (any user, `gh api user` defaults). **Joiners are never asked for a PAT** (D8): `setup` runs only the detection half and the gh offer for them, and a logged-in `gh` merely lets `team join` auto-accept the invitation. The gh offer is skipped whenever stdin is not a TTY.
...
38. **`login` offers gh's own interactive login** when `gh` is installed but logged out (child process, inherited stdio, TTY only); the PAT fallback is unchanged; joiners are never asked for a PAT (D8).
