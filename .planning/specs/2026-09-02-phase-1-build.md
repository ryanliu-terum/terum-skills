...
**In:** `setup` (the onboarding wizard, §6.1), `login`, `team create|join|leave|remove`, `invite`, `share`, `publish`, `ls`, `search`, `status`, `install`, `uninstall-skill`, `uninstall` (machine teardown), `sync`; the metadata write guard; the version cache; the native Placer (§7; vendored skillhub modules under Apache NOTICE); the README generator + repo GitHub Action; the session-start hook; Apache-2.0 LICENSE + NOTICE; npm packaging.
...
Tests are **collocated** under `src/**/__tests__/` (vitest); bare-repo fixtures drive E2E and concurrency, no network. Named suites at minimum: `lib/__tests__/{guard,schema,version,readme,teamRepo,auth,placer,hook,config,launch}.test.ts` and `commands/__tests__/{join,share,install,uninstall,sync,publish,remove,setup,search,leave,uninstallMachine}.test.ts`. Adversarial cases each suite must carry:
