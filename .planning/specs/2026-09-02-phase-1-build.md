...
**In:** `setup` (the onboarding wizard, §6.1), `login`, `team create|join|leave|remove`, `invite`, `share`, `publish`, `ls`, `search`, `status`, `install`, `uninstall-skill`, `uninstall` (machine teardown), `sync`, `update`; the release-state file; the metadata write guard; the version cache; the native Placer (§7; vendored skillhub modules under Apache NOTICE); the README generator + repo GitHub Action; the session-start hook; Apache-2.0 LICENSE + NOTICE; npm packaging.
...
Node **22+** (Node 20 is EOL), TypeScript, ESM. npm package `terum-skills`; every printed command uses `npx -y terum-skills@latest …`, except recovery hints and the push guard, which pin the running package's version so a retry runs the code that printed it (`-y` so a first-run machine doesn't stall on npx's install prompt). Deps: `commander`, `zod`, `yaml`, `proper-lockfile` **[default — veto cheap]**. Everything else shells out to `git` and `gh`; when the `github-teams` policy permits (at least one configured GitHub team), release discovery adds one bounded (10 s) `git ls-remote --tags` of the approved upstream `https://github.com/ryanliu-terum/terum-skills.git` from interactive `sync` at most once per 24 h and from explicit `update`. Results are **release advertisements**, never npm availability; issue 10's release workflow (PR pending) keeps the tag set truthful by advertising only verified publications. The narrow printed-command exception is that `update` and the end-of-command notice may print `npm install …@latest` advice for this copy's own location; product invocations keep the `npx -y terum-skills@latest …` spelling. No HTTP client, no server, no daemon, no third-party CLI on the install path.
...
src/
  ...
  lib/
    package.ts          single adjacent package metadata reader; approved package/upstream constants
    launch.ts           canonical entry + filesystem evidence, shared with uninstall
    update.ts           locked release-state, bounded tag probe, source selection and notices
...
### 4.2 Local state (`~/.terum/skills/` — never committed; `quarantine/` and `backups/` are recovery data, removed only by the teardown policy in §6 `uninstall`)
...
{
  "schema": 1,
  "package": "terum-skills",
  "upstream": "https://github.com/ryanliu-terum/terum-skills.git",
  "running": { "version": "0.1.0", "at": "2026-09-06T21:10:00Z", "entry": "/copy/dist/index.js" },
  "registry": { "version": "0.1.1", "at": "2026-09-07T09:00:00Z", "source": "npx-latest-cache", "entry": "/cache/_npx/hash/node_modules/terum-skills" },
  "advertisement": { "version": "0.1.1", "at": "2026-09-07T09:00:03Z", "source": "git-tags" },
  "attempt": { "at": "2026-09-07T09:00:03Z", "ok": true, "error": null },
  "ack": { "version": "0.1.1", "at": "2026-09-07T09:00:05Z" }
}
```

Each observation and `attempt`/`ack` can be null when absent. `running` is informational only; it never establishes publication. `registry` is recorded only from an npx cache with exactly the request `terum-skills@latest`; its timestamp is the installed package manifest's mtime. Every source replaces its own observation; a successful probe may lower or clear `advertisement`, and a cache rollback may lower `registry`. There is no version ratchet. A failed probe preserves the advertisement and records the first sanitized stderr line in `attempt.error`. Only stable `^\d+\.\d+\.\d+$` versions compare; pre-release tags are not compared. Timestamps more than 60 s ahead are stale.

Read-modify-write takes `proper-lockfile` **before reading**, on the file with `realpath: false`, `stale: 5_000`, `retries: 0`; contention skips the write without delaying the verb. Writes use temp + fsync + rename, mode 0600. Unparseable state, unsupported schema, wrong package, or wrong upstream reads as absent and is **never overwritten**. It is safe for a user to delete this file; no reset flag ships. Acknowledgments merge under the same lock and cannot overwrite a later advertisement. The upstream is a constant, not derived from mutable repository metadata; package name and normalized repository URL must match the approved pair before probing. npm's `git+https` metadata prefix is normalized for that comparison.
...
`share` injects `license`, `metadata.id`, and `metadata.author` (from config; email collected at first run, `gh api user` when available) **into the user's own source file, in place** — it shows the added lines and takes a y/N before writing. A SKILL.md that declares no `metadata.terum-category` — every off-the-shelf skill, which has no `metadata:` block at all — gets `misc`, the starter list's catch-all, as a fourth injected line shown with the other three; a declared category is never overwritten, and `sync` restores a missing one the way it restores the other managed fields (rulings walk R1, 2026-09-06: the tool generates the whole block; nobody hand-edits a skill to share it). The repo copy is then a byte copy of the source, which is what makes `sync`'s reconciliation a raw byte compare instead of a normalizing re-render. It also means a skill carries its own identity if `~/.terum/` is ever deleted. This satisfies SkillEvaluator's author and license schema findings — no broader Tier-1 claim (Tier 1 also runs security/PII/quality checks that injection cannot guarantee; that's phase 3's concern).
...
**`update` (rev 11):** prints this copy's running version, the latest advertised release when applicable, and the command that updates this copy, **never runs it**. Exit 0 means advice was produced; a probe failure is reported and still exits 0 with advice. Failure to produce the output contract exits 1. `--version` uses the same metadata reader; missing metadata prints `version unknown`, never `0.0.0`. There is no `status` verb on this base. Exact per-mode output templates are the issue-8 R.8 contract, enforced byte-for-byte in `src/commands/__tests__/update.test.ts`.

The classifier explicitly realpaths the module entry, then checks npx cache evidence before the nearest `node_modules` manifest. A declaration retains `dependencies` versus `devDependencies`. Only exact `node_modules/terum-skills/dist/index.js` shape may be called global-shaped without a declaration; other hoisted layouts are unknown. An adjacent `terum-skills` manifest with a non-node_modules `dist/index.js` is a source checkout. Environment variables and cwd never establish the installation method. Advice remains conditional: a manifest does not establish npm ownership. `This copy:` names the package directory for npx/local/global, the entry file for source/unknown. Existing uninstall advice is unchanged.

**Discovery:** `RELEASE_PROBE_POLICY: 'github-teams' | 'everyone' | 'nobody'` selects who may probe; the selected policy is `github-teams` (D40). `everyone` permits every machine, `nobody` permits none; local running/npx evidence is recorded under every policy. On sync, probing occurs only after the body has returned and all team locks have released, with `!hook && !prune && io.interactive && 'confirm' in io`, no opt-out, and an attempt older than 24 h. Both successful and failed attempts back off. Hook, prune, non-interactive and opted-out runs record only local evidence. Tests exercise the path against local bare repositories through deny-by-default runners; the advertisement channel is honest because issue 10's release workflow (PR pending) tags only verified publications. Explicit `update` probes every time policy permits. Automatic failures are silent. Probe command: `git -c url.<upstream>.insteadOf=<upstream> ls-remote --tags -- <upstream>` (an identity rewrite: git applies the longest matching `insteadOf` prefix, so it outranks any user rule with a shorter prefix such as `https://github.com/`; only a user rule naming the exact full URL ties, and a tie goes to the earlier config source), 10 s deadline, 64 KiB output cap; SIGTERM then SIGKILL after 2 s, settle on close with code 124. No existing git call acquires a deadline. Only stable `v<x.y.z>` refs with 40-character hex object IDs count; peeled refs and prereleases do not.

**Notice:** the highest newer stable `registry`/`advertisement` observation is the candidate, labelled **observed**/**advertised** respectively. After every eligible command's complete success/failure/throw output, one line goes to **stderr**, never stdout; a failing tail never changes the exit code. Excluded: `sync --hook`, `guard-push`, `readme`, `update`, help, version and usage errors. Non-TTY stderr, `CI`, `NO_UPDATE_NOTIFIER` and `TERUM_SKILLS_NO_UPDATE_NOTIFIER` suppress the tail; the environment gates are read only in `index.ts`. Cadence is once per candidate version per 24 h; a different newer candidate is eligible immediately. Record `ack` under the lock after showing the line; an ack failure does not fail the command. Exact text is the issue-8 R.6 contract in `src/lib/__tests__/update.test.ts`, e.g. `Newer terum-skills release advertised: 0.1.1 (running 0.1.0). If installed globally with npm, run npm install -g terum-skills@latest.` Source/unknown/npx advice includes the canonical entry and literal `npx -y terum-skills@latest <command>`.
...
4. Commit, then push **to `refs/heads/<branch>`** — never to a ref the caller did not name. With the default `main` this is the ordinary path. With a `branch` — publish's fresh `publish/<name>-<handle>-<id8>`, unique per run — the commit is created on top of the freshly reset `origin/main` and pushed to that branch only, **create-only** (`--force-with-lease=<ref>:` with an empty expectation: the name must not exist); **`origin/main` is not a push target in this mode and must be byte-identical before and after**. A derived branch is never replaced and there is no fallback name: one publish is one fresh branch and one fresh PR, and competing PRs are two PRs that GitHub's conflict badge arbitrates (rulings walk R2, 2026-09-06, superseding the abandoned-attempt reuse and the `-2` fallback). On success `publish` runs `gh pr create` against `main`.
...
On a **generic-git remote** (a team created with `--remote` and joined by URL, §6) there is no portable admin predicate, no collaborator API, and no invitation to cancel. Both commands therefore **fail before any mutation**, naming the remote and saying access is managed on the host. `team remove <handle> --archive-only` remains available and does the part that is actually ours: append the handle to `team.json archived` via safeWrite, so the roster reflects the departure while the CLI makes no claim about access it cannot revoke. Its only enforceable predicate is push access to the repo — which the safeWrite push itself already proves — and the spec says so rather than implying a stronger check. Host adapters with real admin checks are DEFERRED past phase 1; nothing here forecloses them. A `pre-push` hook installed in the clone runs the same path check (M4, `guard-push`): it is armed on every clone and every join with the absolute path of the CLI that armed it — no registry on git's blocking path, and the rules that shipped with that CLI, never `@latest` — and it exits 0 with one warning line when that launcher is gone, so a push that was not checked says so; a non-zero exit is always the guard's own refusal. What the guard cannot attribute it refuses rather than waives: every ref deletion (`--prune`, `--mirror`, `--delete`, `:ref` — a teammate's pending publish branch exists nowhere else), every non-branch ref, and a new branch whose fork point off `main` cannot be resolved. A new branch is judged from that fork point (what a PR merge applies), an existing branch against the content it replaces; a rename is judged as its delete plus its add. Threat model unchanged: **prevents accidents, not abuse** — the actor identity the guard reads comes from local config the user controls, so raw git bypass remains possible and attributed.
...
- **`team leave <name>`** — uninstall this team's placements, drop clone + config entry; leaves no repo trace (you remain an active member until an admin runs `team remove`). A clone with uncommitted or unpushed work is moved to `quarantine/<ISO>/teams-<name>` instead of deleted; a placement that is also a `shared` source is dropped from the ledger and left on disk. **`team remove <handle>`** — admin-only (§6.0 authorization check, GitHub-only), refuses on self and on the last remaining admin. Revokes collaborator access, **cancels any pending invitation**, appends the handle to `team.json archived` via safeWrite. On a generic-git remote it fails before mutating unless `--archive-only`, which appends to `archived` and explicitly claims nothing about access (§6.0); their `people/` file and skills remain, so the member becomes inactive by the §4.1 predicate without losing history — **departures archive, never delete**. Org base permissions can still grant read; document it. Reversible by `team join` (rejoin).
...
- **`publish <name> [--project <proj>]`** — the deliberate team-endorsement act: adds the skill's ID to `team.json global` (default) or the project list. `policy.publish: "pr"` (default): `safeWrite(mutate, { branch: "publish/<name>-<handle>-<id8>" })` — a fresh branch every run, created never replaced — then `gh pr create`; the branch parameter is what keeps the generic algorithm off `main` (§6.0); merge is the review, and the Action comments on the PR. Before pushing, an endorsement branch already open for this skill is noted with its PR URL and a y/N asks whether to open another (never a refusal, never a force-push; R2). **A missing `gh` does not downgrade the gate** — under `"pr"` the command pushes the branch, prints the compare URL, and exits telling the user to open the PR; it never falls through to a direct push, because a policy any member can bypass by not installing `gh` is not a policy. Direct commit happens only under `policy.publish: "push"`, a team-level setting in `team.json`, after a y/N showing the skill. **[default — veto cheap: "pr"]** A missing team skill is a recoverable usage error: publish may inspect the global Claude Code skills root to provide explicit-path share guidance, but never imports or begins tracking a local source.
...
- **`status [--team <name>]`** — offline, read-only local team summary, with `terum-skills <version>` first (installed package version; `(version unknown)` if unavailable). All configured teams in object-enumeration order, separated by a blank line; `--team` selects via `selectTeam`, unknown names fail. Each block shows the config key and handle, credential-stripped repository URL (GitHub web URL when applicable), `From the local clone; GitHub access is not checked.`, active filename-checked roster count and first five members sorted by codepoint handle (then `… and N more`, with `(you)` on the configured handle), local inactive/missing membership, usable shared-skill count, and exactly `Evaluated skills: not yet available` (numeric evaluation awaits IE3). Unreadable people/skills qualify their counts and print reasons. A missing/old/unreadable sync stamp adds the shared `npx -y terum-skills@latest sync` stale notice. No teams prints the create/join setup hints on stdout and succeeds; exit 0 means the query succeeded, so `terum-skills status && …` is not a setup-readiness or membership test. Missing/incomplete/foreign clones show recovery advice (move aside before join for incomplete/foreign); unverifiable clones advise checking git, never moving the directory. Per-team failures retain printed fields and continue other teams, with one `<team>: local team details could not be read.` stderr line each and exit 1. No prompts or writes; only one `git remote get-url origin` probe per readable team, no host access or per-skill git calls.
...
- **`uninstall-skill <ref> | member <handle> | project <name>`** — write a `pending` entry with `op: "uninstall"` → Placer remove (under the target lock; refuses any path that is not in the `placements` ledger or not under the resolved skills root — skillhub's path-safety rule) → drop the `placements` entry → people-file update → clear `pending`. The entry is what makes an interrupted uninstall recoverable: without it, a removal that dies before the people-file write leaves a record with no placement, and any rule that re-places such a record would silently undo the removal the user asked for. `sync` reads the entry, finishes the removal, and clears it. Uninstalling **any** skill that an automatic path would otherwise place — team-endorsed global *or* project-list — records its ID in `declined`, which suppresses every automatic placement of it (§5.2). Re-installing explicitly clears the entry. The `approvals` record is left alone: it is consent for a grant set, not a request for the skill. The durable pending operation and the guard action remain `uninstall`.
- **`uninstall`** (no argument) — confirmed machine-local teardown through the shared team-leave implementation: writes `backups/uninstall.<ISO>.json`, removes the §8 hook command first when present (aborting on failure), tears down every configured team, removes `config.json` under its lock only when nothing remains configured, removes empty directories non-recursively, preserves `quarantine/`, `backups/` and every `shared` source, leaves remote membership and `installed` records unchanged, and prints — never runs — the package-manager step. Incomplete cleanup is reported as incomplete with what remains. An operand is refused (`To remove a skill, use terum-skills uninstall-skill <ref>`).
- **`sync [--hook]`** — refresh the clone (`fetch` + `reset --hard origin/main`, safeWrite's own recovery, shared with `publish` — never `pull --ff-only`, which wedges on a drifted clone; D5b, 2026-09-05); auto-update owned `shared` skills per the §5.3 reconciliation contract; **complete every `pending` entry first** — an `install` entry finishes placing and recording, an `uninstall` entry finishes removing and unrecording; both are idempotent, and only then is the entry cleared. Untracked placements in the ledger with no `pending` entry and no `installed` record are orphans (§6 `sync prune`), offered for adoption, never silently re-placed. Every `placements` entry is then classified — **up-to-date**, **update-available**, **local-changed**, **blocked**, or **orphaned** — by comparing the placed copy's current fingerprint against the ledger's and against the clone's (§7; the vocabulary is skillhub's). Then place, subject to one rule.
...
`setup [<org>/<repo> | <remote-url>]` is the single onboarding entry point, for a joiner holding a target (the invite block's `setup <org>/<repo>`, or the three-part `install` bootstrap), and, with no argument, for a fresh machine — which is asked `Create a team or join one?` (no default; the one wizard question without one, because a wrong create is a GitHub repository) — or a configured machine, which resumes its first team and is told how to reach another. "Join" without a target prints the owner hand-off and exits 0 having written nothing (a `success` with empty team/remote; only reached steps are recorded). It is a **sequencer over the verbs above and owns no write path of its own**: every write it causes is the underlying verb's own `safeWrite`, every placement is the Placer's, and every consent it collects is the underlying verb's own prompt — the hook y/N (§8), the endorsed-set y/N, the per-skill `allowed-tools` y/N (§5.4), the `share` frontmatter y/N. `setup` cannot skip, pre-answer, or batch any of them; it calls `run(args, io)` on each verb with the same terminal `Prompter` (§3) and lets the verb ask. A wizard that answered consent prompts on the user's behalf would be a bypass of §5.4, and §12 carries a test that proves it cannot.

The eight steps, in order (with the fresh-machine role question between steps 1 and 2):

1. **Welcome.** Prints one short paragraph: what the tool does (the ledger's three-sentence model), that everything lives in a git repo the team controls, and what the wizard is about to do. Print-only. Immediately afterward, read config. With no target and no configured team, print `Creating a new team creates a private GitHub repository under your account.` and ask `Create a team or join one?` with `Create a new team` / `Join an existing team`, no default and no extra creation confirmation. Choosing join prints the five-line owner hand-off and returns success before any gh/git call or local write; step 2 is never reached. Bare setup on a configured machine prints `Resuming setup for team <name>. To join another team, run the setup command its owner sent you.` before probing gh; an explicit target bypasses the question.
2. **GitHub.** Runs `login`'s detection (§6 `login`). **Creator:** a logged-in `gh` is required, exactly as `login` requires it, because `team create` and `invite` need the host API (no token fallback, rev 9). **Joiner:** nothing is required and no token is ever requested (D8) — with `gh` logged in, `team join` will auto-accept the invitation; with `gh` installed but logged out, the `login` gh offer is made once; without `gh`, the invitation accept URL is printed in step 3 and the wizard waits for a y before cloning. First-run identity (handle, name, email — §5.4 rules) is collected inside `team create` / `team join`, not this preflight. When login, handle, name and email are known, the existing `Identity: …` and `Use this identity?` confirmation replaces the four identity questions (06f0345).
3. **Team.** **Creator:** prompts for the team name, then the GitHub repository name (default: the team name; re-asked when GitHub says it is taken — Decision 5, rev 9), and an optional org, then runs `team create` with all three as plain fields. The name is asked here, ahead of any skill step, because the repo must exist before `share`, `install`, or `invite` can run — this is the one place the eight-step order bends: the name comes first, the invites come directly after the team exists, the actions after that. **Configured bare setup:** resumes the first configured team, with the pre-gh resume line above. **Joiner with an explicit target:** runs `team join` with the argument; the handle collision check, the endorsed-set y/N, and the per-skill `allowed-tools` prompts happen here, as `team join` defines them. In both branches the verb is called with `offerHook: false`, so the §8 offer lands at step 7 rather than mid-wizard; run directly, `team create` and `team join` still offer it themselves.
4. **Invite.** Creator only, GitHub remotes only; follows the complete clone check immediately after the team exists. Asks `Invite teammates by inputting their GitHub usernames (comma or space separated; blank to skip)` with default `''`; blank skips, otherwise runs `invite` and prints its Slack-ready block. Asked on every creator run; a successful invocation is `done`, including all-204 responses. A failed Result or thrown error stops here before share, hints, community and hook; `steps.invite` stays unset, and earlier invitations are not undone. On a generic-git remote the step prints the §6.0 host message and moves on. A joiner never sees this step.
5. **First actions.** **Creator:** lists the skill folders under `~/.claude/skills` that are not yet in config `shared`, offers to `share` one (`select`, with a "skip" choice; the verb's own frontmatter y/N follows), then prints one line each for `install`, `ls`, `search`, `sync`, and `publish`. **Joiner:** the endorsed set was already offered in step 3, so this step is the same five one-liners. `eval` is not mentioned until phase 3 ships and `ui` is not mentioned until phase 2 ships — a wizard step that says "coming soon" is a broken step, so the reserved steps are **absent, not stubbed**. **[default — veto cheap]** Offer local candidates that pass the shared source-validation checks, excluding tracked sources and placements; report omitted invalid folders. The publish hint explicitly describes endorsement of an already-shared skill.
...
**Resumable by outcome, not by a state file.** `setup` records nothing of its own. Each step that writes is skipped when its outcome is already present: step 2 when `gh auth status` succeeds (creator) or first-run identity is already in `config.json` (joiner); the role question when the machine already holds a team (resume) or a target was given; step 3 when `config.teams` already holds the team (creator) or the remote's `people/<handle>.json` already exists for this handle (joiner); step 4 (invite) is asked on every creator run — blank skips it; step 5's share offer when nothing is unshared; step 7 when the settings entry exists. Print-only steps (1, the one-liners in 5, 6, 8) always run. Re-running `setup` after an interruption therefore continues from the first unfinished step and can never create a second repo, a second people file, or a second hook entry. An interruption *inside* a step is the underlying verb's problem and is already covered — `safeWrite`'s `finally` reset (§6.0) and the `pending` log (§5.4).
...
An existing matching hook entry is **not refreshed** by `offerHook` (`src/lib/hook.ts`); it returns `present`. A hook-command change needs an explicit migration. Issue 8 does not change `HOOK_COMMAND`. Its `@latest` request asks for the registry's latest release on each startup; it updates no other installed copy.
...
- **Acquire** by creating `run/<team>.lock` with `O_CREAT|O_EXCL` (atomic on every platform we target) containing `{pid, host, token, started}`. One lock **per team**, so unrelated teams never serialize behind each other. The `token` makes the record this acquire's own: the release, and the stale reclaim below, act only on the exact record they hold or judged, so a lock reclaimed as stale mid-run stays with its new holder and two reclaimers of one stale lock cannot both end up holding it.
...
- **Stale recovery:** a lock is stale if `started` is more than 10 minutes old, or if `host` is this machine and `pid` is not alive. A stale lock is reclaimed (moved aside atomically, removed only while it is still the record that was judged; a live record displaced instead is put back by a hard link, never written over a holder that took the freed path meanwhile, and left aside as `<team>.lock.stale-<uuid>` — swept by `team leave` — when it cannot be put back) and re-acquired once; a lock held by another host is always respected. Without this rule a crashed process disables sync forever.
...
- **Idempotency key** is a hook **command** containing `terum-skills` inside any `SessionStart` group. Present → replace that command in place when its group runs at `startup`; a group that held only our command becomes the canonical group; a mixed group under another matcher keeps its other commands and the canonical group is appended; absent → append the canonical group. Joining five teams therefore yields exactly one command, because `sync` already loops over every configured team — the hook is per machine, not per team.
...
- **Removal** (`team leave` of the last team, and `uninstall`, which runs it before any other deletion) deletes only the matching command; a group emptied by the removal is dropped; other commands and fields in the group, other groups and events survive; removes nothing if the key is absent.
- `hook.test.ts` carries the adversarial cases: an existing unrelated SessionStart hook survives; repeated installs never duplicate; malformed JSON refuses; an interrupted rename leaves a valid file; leave-with-other-teams-remaining keeps the entry; a mixed group (our command beside an unrelated one) keeps the unrelated command on removal and on re-install.
...
- **V8 (new, rev 4):** does an `async: true` SessionStart hook's `hookSpecificOutput.reloadSkills` get honored at all, and if so on which turn? This decides whether §8 keeps emitting the directive or drops it. Measure before M4; the hook ships async either way. **Measured 2026-09-06** (Claude Code 2.1.263, print mode; `.planning/reviews/2026-09-06-v8-reload-measurement.md`): the hook fires in print mode; a skill it places is **absent in the turn that fired it** and **present on the next turn of a resumed session** — "no later than the next session" holds. Whether the directive reloads a *live* session's skill index (one process, two turns) is still unmeasured; the directive stays, best-effort, until that run says it is never honored.
...
Tests are **collocated** under `src/**/__tests__/` (vitest); bare-repo fixtures drive E2E and concurrency, no network. Named suites at minimum: `lib/__tests__/{guard,schema,version,readme,teamRepo,auth,placer,hook,config,package,launch,update,runner,execute}.test.ts` and `commands/__tests__/{join,share,install,uninstall,sync,publish,remove,setup,search,leave,uninstallMachine,update}.test.ts`. Adversarial cases each suite must carry:
- **release state / metadata / launch** — source versions cannot advertise publication; missing/invalid metadata stays null; realpath and Windows paths; npx latest/pinned/bare/missing evidence; dependency kind, source and unknown layouts; corrupt/new-schema/wrong-package/wrong-upstream records preserved byte-identically; future timestamps stale; stable comparison only; removed-tag/registry rollback replaces downward; upstream mismatch refuses before any runner call; locked interleavings and ack merges preserve independent fields; lock contention skips writes.
- **release execution** — sleeping git alias cancellation and child cleanup; spawn rejection; output cap; local tagged bare fixture selecting 0.10.0 over 0.1.1 while ignoring prereleases/peels; failed attempt backoff at 23/25 h; post-team-lock probe placement; hook/prune/noninteractive/policy/opt-out exclusions; silent automatic failure versus explicit advice; exact per-mode templates and observed wording; tail last after success/failure/throw, unchanged exits, daily/per-version cadence, failed ack; built dist/package.json layout, --version, source classification, and non-vacuous TTY/CI/opt-out suppression tests. All new remote-capable tests use deny-by-default runner allow-lists and local bare fixtures, never public network probes.
...
- **status** — version first in every state; no-team hints succeed; explicit unknown selection fails; numeric team keys follow object enumeration; roster sizes 0/5/6, `(you)` without reordering, archived/missing self, filename mismatch, malformed people and codepoint ordering; partial skill counts and evaluation availability even with receipts; missing/incomplete/foreign/unverifiable clones and per-team failures preserve other output and fail the query; missing/fresh/old/future/unreadable stamps; no prompts, config bytes and clone HEAD/worktree unchanged, and exactly one origin probe per readable team; setup/join share the corrected roster and search shares the runnable stale line.
...
26. `safeWrite` takes a `branch`; PR-policy `publish` passes a fresh `publish/<name>-<handle>-<id8>` (create-only) and provably never writes `origin/main`.
...
35. **`setup` is the single onboarding entry point** for a joiner with a target, a fresh machine that is asked create-or-join, and a configured machine that resumes; it sequences existing verbs, owns no write path, and answers no consent prompt. Resumable by detecting each step's outcome, never by a state file.
...
39. **`COMMUNITY_URL`** is one exported constant in the package, printed by `setup` step 6 and never opened. **DECIDED 2026-09-05 (close-out walk, D6): the repository's Issues page, `https://github.com/ryanliu-terum/terum-skills/issues`** (`src/lib/community.ts`) — not Slack (the 2026-07-14 guest-scoping ruling keeps outsiders out of the internal workspace) and not Discord (an empty room to monitor), gated: a Discord server replaces it when outside users need to talk to each other. Step 6 is still skipped whenever the constant is empty.
...
41. **Reserved steps are absent, not stubbed:** the phase-1 wizard prints nothing about `ui` or `eval`; phase 2 replaces step 8's last line with opening the UI, phase 3 adds an eval action to step 5. The phase gates from walk Decision 2 are unchanged.
42. **Team name, then invites, then actions:** the team name is collected at step 3 because the repo must exist first; invitations follow immediately (Ryan, 2026-09-06 — supersedes the 2026-09-03 order); the first actions come after.
