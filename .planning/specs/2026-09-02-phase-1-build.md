...
**In:** `setup` (the onboarding wizard, §6.1), `login`, `team create|join|leave|remove`, `invite`, `share`, `publish`, `ls`, `search`, `status`, `install`, `uninstall`, `sync`; the metadata write guard; the version cache; the native Placer (§7; vendored skillhub modules under Apache NOTICE); the README generator + repo GitHub Action; the session-start hook; Apache-2.0 LICENSE + NOTICE; npm packaging.
