...
> **Partly superseded 2026-09-11 by `.planning/specs/2026-09-11-library-marketplace-refactor.md` §6.5:** the incumbent is no longer a materialized tree hash (`git archive <tree> | tar -x`, §7.1; `lib/version.ts`) but an immutable version folder read directly from the clone, and the candidate is identified by content digest rather than by tree. The **ordering rule is unchanged** — the most recently receipted version by run-id order, excluding the candidate's own bytes, none → the no-incumbent path (§11, the 2026-09-07 hygiene/publish-gate Decision 2).
