...
2. Eval assets live inside `skills/<name>/evals/` — **but they no longer version with the
   tree hash (rev 19 — Ajay, 2026-09-13, overriding this default's second clause).** A version
   is the skill tree with its top-level `evals/` entry REMOVED, minted as a real tree object and
   kept resolvable by a local `refs/terum/content/<skill-tree>` ref (`src/lib/content-tree.ts`),
   so a hash alone still pins, materializes and resolves. The deciding argument: a version drives
   update prompts, placements, pins and the publish gate, and an eval that only adds cases was
   moving all four — "this skill changed" has to mean the skill changed. The dataset therefore
   needs the separate digest this clause denied it: `provenance.eval_assets` (receipt rev 9)
   records the `evals/` subtree hash, null when the skill has none. Consequences taken
   deliberately: a skill that already carries committed eval assets gets ONE new version the day
   this lands and needs one re-eval to satisfy the publish gate; placements stop shipping eval
   datasets, which settles §16.15's open phase-1 question in the same direction; and a content
   tree is local-only, so every consumer re-mints one it does not have from the skill's recent
   history (bounded, anchored by ref, at most once per machine).
...
13. `--working` evaluates authored sources but can never `--commit`. **Rev 19 (Ajay,
    2026-09-13): a finished run PUBLISHES its receipt by default** — running the eval is the
    sharing step, `--no-commit` keeps it local, and an explicit `--commit` still refuses what the
    default merely declines (a person who typed the flag asked for a receipt and must not be told
    afterwards that they did not get one). `--working`, a forced `--gen`, and a missing joined
    handle downgrade a default run to local with one printed sentence naming what would publish
    it, and refuse an explicit `--commit` with the unchanged refusal text.
