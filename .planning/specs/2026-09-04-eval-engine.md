...
```jsonc
{
  ...
  "per_case": [                        // rev 20, optional: one entry per (case × rep), execution
    { "case": "missing-env", "rep": 0,  //   order. Display only — no statistic reads these rows.
      "arms": {                         //   passed = §5.1 all-or-nothing; null when the arm run
        "candidate": { "passed": true,  //   failed twice or the case has no checks. [name, passed]
                       "checks": [["file_exists:preflight.log", true],           // per check; the
                                  ["no_command_matching:rm -rf", true]] },      // engine's detail
        "baseline":  { "passed": false, //   string never enters a receipt.
                       "checks": [["file_exists:preflight.log", false],
                                  ["no_command_matching:rm -rf", true]] } },
      "outcomes": { "candidate-vs-baseline": "win" } }
  ],
  "case_runs": {                        // rev 20, optional: passed / decidable case-runs per arm —
    "candidate": { "passed": 7, "total": 9 },   //   the §12 card's Quality number, shown verbatim.
    "baseline":  { "passed": 4, "total": 9 }    //   Same keys as arm_scores; null verdicts are
  },                                    //   neither passed nor counted in total.
...
All free-text fields (`attribution`, the check names inside `per_case` — a check name carries
its argument from the case file — and any judge `reason` surfaced later) pass through
redaction (§8) before the receipt is built. Receipts written before rev 20 carry neither
`per_case` nor `case_runs`; a surface says so ("Per-case rows are not in the committed
receipt.") and never derives them from `arm_scores` or a local run tree.
...
- **`eval <name|id> [--k N] [--triggers-only|--execution-only] [--case <stem>]
  ...
  Defaults: `--k 1` (rev 18; `--k 3` for a receipt meant to gate, `--k 10` for depth when it matters), `--model sonnet`
...
| Quality, side by side (rev 20; was arm scores) | `case_runs.candidate` vs `.baseline` — "1 / 3 passed cases" when `provenance.k` is 1, "2 / 9 case-runs" otherwise, one pip per (case × rep). Absent on a pre-rev-20 receipt → "—". `arm_scores` remains in the receipt and one click deeper in the report's §2 paragraph. |
...
6. **Default k=1 (rev 18 — Ajay, 2026-09-10, overriding the 2026-09-07 "keep default
   k=3" ruling, Terum 5aa9a4b2).** The band alone gates at every k; sign p is reported
   but never gates below k=10. The deciding argument was cost, measured 2026-09-10 on
   sonnet: k=3 → k=1 takes a 3-case run from ~$4.40 to ~$1.50. Measured caveat, now
   sharper (2026-09-04 probe): two identical k=3 runs flipped PASS↔NEUTRAL with a net
   lift sitting on the +1/3 boundary — verdicts were already one wobbly rep from
   flipping at three reps, and at one rep every case is a single draw, so a default
   receipt carries correspondingly less weight than one at k=3. Append-only receipts
   (rev 5) remain the designed answer — re-runs accumulate rather than overwrite — and
   `--k 3` / `--k 10` stay available for a receipt meant to gate.
