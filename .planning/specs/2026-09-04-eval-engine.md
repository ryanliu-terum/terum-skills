...
half their rows were checks-decided, else to "judge calls". Ties say why they tied
(rev 20): when every baseline row is a `checks-equal-no-judge` tie the line reads
"all N rounds drew — neither version passed every check in any case, and a partial
lead doesn't win a round; with no rubric, no judge ran"; other all-tie runs name the
`decided_by` reason(s) ("the judge split across the two orderings", …, with per-label
counts when mixed); a mixed record appends the tie count and its reason(s). The
wording must never say the checks "came out level" — a `checks-equal-no-judge` row can
hide a partial lead — and never call such a row "unscored" (that word is reserved for
`both-arms-failed`). Display only: `net_lift`, `sign_p`, and the §16.5 band are
unchanged.

Report-only check-quality diagnostics (rev 20): the printed report (§6) lists every
check that failed in every arm on every rep ("dead") or passed in every arm on every
rep ("vacuous"), keyed `<case> › <check name>`, under a `check quality:` line. Results
from an arm that failed to run are left out (they were scored against the empty
transcript and say nothing about the check), and a check observed fewer than twice is
not classified. Such checks cannot tell the versions apart, so a tie over them is not
evidence the skill made no difference. These diagnostics change no score, no verdict,
and never enter the receipt.
