...
- §1.1 `target` comment and §1.3 role rule (original lines 50, 61): a target joins directly; absent a target, a configured machine resumes the configured team, otherwise ask `Create a team or join one?` with `Create a new team` / `Join an existing team`. Before the question print `Creating a new team creates a private GitHub repository under your account.` This defaultless select is the one exception to “a question with a default”; there is no extra creation confirmation. Read config and resolve this fork before probing gh. Configured bare setup prints `Resuming setup for team <name>. Terum Skills keeps one team per machine; to move this machine to another team run team leave <name> first.` before gh. A target-less join returns success with empty team/remote and only the reached steps (`welcome`, `role`, `team`); no gh/git call, verb call or local write occurs. It prints exactly:
  ```text
  ...
  ```
- §1.3 WELCOME line 3 (original line 71) is replaced by: `This wizard helps you create a team, join an existing team, or resume setup. It checks GitHub, sets up your team, invites teammates, shares a first skill, and offers the session hook; re-run it any time to continue, and leave the invitation question blank to skip it.` Lines 1–2 stay unchanged.
- §1.3 steps 4–5 (original lines 80–94): step 4 is Invite, immediately after the complete clone check; step 5 is First actions. The prompt is `Invite teammates by inputting their GitHub usernames (comma or space separated; blank to skip)`, passed to `io.text` with default `''`. Creator only, GitHub remotes only; a re-run asks again, blank skips, successful invocation (including all-204) records `done`. Failed Results and thrown errors stop the wizard there with `steps.invite` unset, without undoing earlier successful invitations. GitHub decides whether the caller can administer the configured repository. Generic-git prints the host hand-off; joiners skip. The five command hints stay after share. `joinCommand(target)` in `invite.ts` formats the setup command used by the invitation block, generic-git message and owner hand-off.
- §1.4 cases 1–2 (original lines 103–104): fresh creator scripts start with the explicit `Create a new team` answer; invite follows the repository question, then share. Interrupt using `verbs.invite`; resume with `['bob', 'starter']`, no role question and no second repository. The configured resume copy above replaces the creator's old configured-team line; the targeted joiner's line is unchanged.
- §1.4 case 5 (original line 107): the wizard's own questions are role, invite, share, in that source order (`['io.select(', 'io.text(', 'io.select(']`); it still never calls `io.confirm`. Consent questions remain in their existing verbs.
- §1.4 case 7 (original line 109): generic-git skips step 4, not step 5, with the host message. Case 12 explicitly selects create before the logged-out creator failure. Add the no-write join hand-off test and a built-bin non-TTY test: exit 1 at the role selector, no gh/git spawn and no HOME writes. Preserve terminal select's blank re-ask, three-invalid-answer failure, EOF and broken-pipe behavior. Add question-aware invitation order and exact-copy tests, plus API failure, syntax failure after an earlier success, blank recovery, all-204 and non-admin/org refusal cases.
- §1.5 walkthrough (original line 118): Alice answers `Create a new team`, creates the team, invites `bob`, then shares one skill; her script is `['Create a new team', 'team', '', '', 'Alice', 'alice@example.com', 'team', 'bob', 'sample']`. Bob's targeted setup is unchanged.
...
5. **`setup` is never the answerer:** stub `verbs.share`, `verbs.invite`, and `verbs.offerHook` with functions that record the `io` object they received and ask one question on it; assert each received **the very same `io` instance** `setup` was given (`toBe`), that their questions appear in `io.asked`, and that `io.asked` contains no entry `setup` asked other than the invite-logins question. Assert share receives the injected home and no path; `success(undefined)` records `skipped`, a shared result records `done`, and failure stops at actions. Empty inventory and explicit `Skip` both continue to invite. Also assert, on a real run, that the strings `Approve`, `Install`, `Share`-with-a-question-mark, and the hook question were asked by their verbs (present in `io.asked`) and that `setup.ts` contains no call to `io.confirm` (read the source in the test, or assert structurally through the stub).
