...
4. **First actions** (`done` | `skipped` for the share offer; the one-liners are always printed).
   ...
   - **Both roles, always:** print exactly
     ...
     Next, from any terminal:
       ...
       npx -y terum-skills@<version> publish <skill> — endorse a skill already shared with the team
     ```
     with `<team>` replaced by the carried team name and `<version>` by the running package's version, or `latest` when unknown. **The strings `eval` and `ui` must not appear anywhere in the wizard's output** (§6.1: reserved steps are absent, not stubbed).
