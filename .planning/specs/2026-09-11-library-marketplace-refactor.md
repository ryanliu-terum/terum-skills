...
- Category, printed before the confirmation, one of four states (auto-category spec R1): `Category: <name> (declared)` · `(from --category)` · `(suggested)` · `(could not reach the model — using <DEFAULT_CATEGORY>)`. Never silent: a silent fallback is how the catalogue drifts back to all-`misc`.
...
- **Identity at run time is the content digest.** `receiptSchema` gains `content_digest: z.string().regex(/^sha256:[0-9a-f]{64}$/)`, required at `schema_version: 2`, computed by the *same* `skillContentDigest` as the publish comparison (§3.3), over the folder exactly as it is on disk — `eval` never injects managed fields. The one legitimate divergence is a run made before the folder's first publish on a `SKILL.md` that declared no category (§1.2): publish **reports** those runs as not attached (§5.2) rather than silently skipping them, and every run after the first publish matches because publish wrote the category back (§5.1 step 5).
...
| `.planning/specs/2026-09-10-auto-category.md` | locked, **amended** | Survives with its wiring moved: the suggestion happens once, at the **first publish** (§5.1 step 4), with the sibling's precedence, its `--category` flag and its four-state disclosure, and is written into the user's local folder (§5.1 step 5). Its §4 ("Wiring into `connect`") and §7 ("at connect") are amended by this spec — see the note at its head. |
