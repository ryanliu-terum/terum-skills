...
**Status:** DRAFT rev 11 (2026-09-14).
**Rev 11 (2026-09-14, Ryan)** adds **D77**: `publish` writes its `profile[]` entry with **no question**. Typing `publish` is the endorsement — the publisher chose the skill, the team and the project by hand — so "Add <name> to your profile?" asked for consent already given, and its default-no left the ordinary publish path writing nothing at all (a piped publish could not even ask: `io.confirm` throws off a TTY, and publish reported "could not add it to your profile"). `install` keeps the prompt, default no: installing someone else's skill is a copy onto a machine, not a statement about the skill. D77 also **builds `profile --remove <skill>`**, which §9.1 has promised since rev 2 and which no batch ever shipped — with publish writing unasked, the inverse is owed, not optional. Amends D5 (§2) and §6 step 10; §9.1's install half is unchanged.
**Rev 10 (2026-09-13, Ajay)** overrides two clauses of **D9** and the matching sentence of **D2**: eval assets travel with a skill but no longer *identify* it. `skillContentDigest` skips `evals/`; publish writes them to the mutable `skills/<name>/evals/` under new guard row **a″** instead of into the immutable `v<N>/`; install stops carrying them; and a finished `eval` whose bytes already match a published version publishes its own receipt (§6.3). The accepted cost D9 recorded — *"regenerating cases mints a new skill version"* — is precisely what this removes, restoring rev 2's §3.1 position. **B5/B6 are in flight against this spec: D2, D9, §4.2, §6.3 and the §3.4 layout table all moved.** Ryan has not seen this override; Ajay took it and is squaring it with him. §2 records twenty-four rulings. §15 is empty — every open question was walked to a resolution.
**Rev 8** closes the five blockers B3 was waiting on. Two needed a human and were walked with Ryan on 2026-09-11 (`.planning/decisions/2026-09-11-library-marketplace-refactor-decision-walk.md`, D28 and D29): the category suggestion and HYG7 leave B3 for a new batch **B9** after B5, and `eval --gen` is deleted so generation only ever fills in what is missing. Three were mechanical and are applied here: OF-2 moves publish's local write behind every refusal-capable question and states the boundary against a failed `safeWrite` (§5.1 steps 5, 6, 6a, 6b); OF-3 makes guard row a′ require the whole version prefix to be absent, not just each path (§4.2); OF-4 corrects §3.4's claim that the config preprocess covers `personSchema.installed[]` — it never did — and gives §13's migration an explicit legacy people-file reader. **B1 is built and merged-pending as [PR #179](https://github.com/ryanliu-terum/terum-skills/pull/179).**
**Rev 9 (2026-09-13) is the B5/B6 spec resolution** — the open findings those two batches were waiting on, walked on 2026-09-13 (`.planning/decisions/2026-09-13-refactor-unblock-decision-walk.md`, D62 and D73–D76; A3 of `.planning/decisions/2026-09-12-unapproved-autonomous-decisions.md`) and applied here before either batch is handed to Codex: the D6 seam namespace becomes `backend.skillFile.*` because `Backend.skill()` and `Backend.library()` are already methods (OF-10 — §7.5, §11.4, D24); §7.5's "delete is undoable" promise is rewritten to the real three-way removal contract with no behaviour change (OF-11, D74); an interrupted `skill rename` is repaired by the skill's stable `metadata.id`, the fingerprint deciding only `edited` (OF-12, D75); the library-side `⋯` gates are recorded as B5's, the dialog gate having landed on B3 and the marketplace rows on B4 (A3 — §11.4); the lifted destination picker carries B2's `checkouts` → `libraryProjects` rename (OF-14 — §9.1.1); the deletion of `declined[]` names its three live readers and is B6's (D62 — §3.5, §9.1); install seeding keys by the receipt's `content_digest` and prints a line for a receipt without one, and `team migrate` stamps the digest on every receipt it re-keys (OF-15, D76 — §9.1, §13 step 3). The sibling edits landed in the same revision: phase-1 build §6 and §12 (OF-13's scoped supersession note in D51's shape; OF-16/OF-17's install grammar, destination rule, collision contract and walkthrough), auto-category §6 (its caller list rewritten to publish-only for B9, gated on B5 merged per D28), and OF-10–OF-17 struck in `.planning/reviews/2026-09-11-refactor-open-findings.md`. B4's three resolutions (OF-7–OF-9, D50–D52, 2026-09-12) were applied in place without a revision bump and are recorded here for completeness.
Rev 2 applied 27 findings that survived an adversarial verification pass (6 lenses, 49 findings adjudicated). **Rev 3 applies the six decisions of `.planning/decisions/2026-09-11-library-marketplace-refactor-decision-walk.md`**, three of which change sections rev 2 had locked — read §2 D9–D14 before anything else. **Rev 4 triages the 53 medium/low findings the rev-2 pass never adjudicated** (`.planning/reviews/2026-09-11-refactor-spec-unadjudicated.md`): 30 applied here, 15 were already present in rev 3, 4 were stale under D9/D13, and 4 were forks walked to D15–D18. **Rev 5 closes `/harden` round 1** (report: `.planning/specs/reviews/2026-09-11-library-marketplace-refactor.codex-spec.r1.review.md` — Codex `gpt-6-astra` found 24, a 3-vote Claude panel confirmed 10, refuted 11, split on 3): the eight BLOCKER/DRIFT findings are applied here — guard row j defined over the whole migration diff (removals and every member's people file), the category resolved and written back into the local folder at first publish, the D16 filter list named file by file, the Library's `installs` neutral value, and `installed[]` keyed by id **and** scope. The round's two AMBIGUITY/GAP findings (the incumbent rule in §6.5) are outside the loop's apply set and were adjudicated separately in the same revision, to the 2026-09-07 hygiene/publish-gate ruling; the three contested findings (HYG7 under publish, implicit roots in the local inventory, `newestReceiptAt`'s return shape) are left for Ryan in the report. **Rev 6 closes `/harden` round 2** (report `…codex-spec.r2.review.md`; 53 Codex reviewers, 24 raw, 14 confirmed, 6 refuted, 4 split): the seven BLOCKER/DRIFT findings are applied here — hygiene for never-published local eval candidates, project membership written on both publish branches, the local trigger catalog, the eval-engine spec's receipt and gate contracts recorded as superseded, `login`'s and `status`'s stale sync promises, the category disclosure deferred to the sibling's table, and versioned `validate` keeping the skill name; the fork on the publish receipt gate is D19, and six GAP/NOTE/AMBIGUITY findings were adjudicated in the same revision. **Rev 7 is the build-ready revision:** §16.1 is replaced by the eight-batch build order that actually typechecks between batches (`.planning/reviews/2026-09-11-refactor-implementation-batches.md`), and D20–D24 close five things earlier revisions left unplaced. `/harden` round 3's Codex half found 21 more findings but its Claude verify half aborted on a spend limit, so the loop stopped at two counted rounds; every finding it left open is tracked per batch in `.planning/reviews/2026-09-11-refactor-open-findings.md` and resolved at that batch's start (Ryan's call, 2026-09-11), not before implementation begins.
...
This matters for a second reason. `injectManagedFields()` (`src/lib/skills.ts:178`) **rewrites `SKILL.md` frontmatter on the way out** — inserting `license`, `metadata.id`, `metadata.author` and, when absent, `metadata.terum-category`. Under this refactor publish writes that rewritten file back into the local folder (§5.1 step 5), so from the first publish on local bytes and published bytes are the same bytes. Before it they differ by exactly those fields — and `canonicalSkillMd` normalizes the three identity-neutral ones (`license`, `metadata.id`, `metadata.author`), so a never-published folder and its first published version still share a digest. `terum-category` is **not** normalized: a category is content (auto-category spec §7, "ordinary content thereafter"), so a run made before the folder's first publish on a `SKILL.md` that declared no category digests differently from `v1` — §6.1 says how publish reports that. `canonicalDigest` is the right primitive precisely because it compares *post-normalization*.
...
- `moveDirectory()` (`src/lib/placer.ts:184`) already handles the cross-volume case with copy-then-remove — an interruption between the copy and the remove leaves both copies; §7.5 states the re-run rule.
...
**D2 — Byte identity: content and path only.** Compared: every file under the skill folder **except `evals/`** (D9, as overridden 2026-09-13 — eval assets travel with the skill but do not identify it). Ignored: `.DS_Store`, `Thumbs.db`, `.git/**`, `.skillhub/**`. Normalized: file mode and mtime — identity is **content + relative path** only. `SKILL.md` is compared through `canonicalSkillMd`, which normalizes `license`, `metadata.id` and `metadata.author` and leaves `metadata.terum-category` in the stream — a category is content (auto-category spec §7), so changing it mints a version, and a never-published folder digests equal to its first published version only once it declares one.
...
**D5 — `people/<handle>.json` carries both lists.** `installed[]` stays automatic (it is the sole source of install counts, Top-rated ordering and the member facepiles — `src/lib/readme.ts:40 installCounts`). `profile[]` is the new curated list. **Amended by D77 (rev 11):** `install` writes it only on an explicit yes; `publish` writes it unconditionally, because publishing *is* the yes. `profile --remove <skill>` is the only way to empty it.
...
**D9 — Eval cases are part of the skill.** They are generated into the user's **local** folder, ~~digested as content identity, published inside `v<N>/`, and copied on install~~ — **two of those four clauses are overridden (Ajay, 2026-09-13); see the override note below.** They are generated into the user's local folder and they travel with the skill, but they are **not** content identity, they are published **beside** the version folders at `skills/<name>/evals/`, and they are **not** copied on install. *Grounding:* 88 local skill folders on the author's machine contain **zero** `evals/` directories, while the team repo's `decision-walk` carries four eval asset files — cases are generated straight into the repo today and have never been on disk, and this refactor deletes both routes that put them there. ~~**Accepted cost:** regenerating cases mints a new skill version.~~ **This accepted cost is exactly what the 2026-09-13 override removes**, restoring rev 2's §3.1 derived decision that D9 had reversed.

**Ajay, 2026-09-13 — eval assets leave version identity.** D9's first clause stands — cases are generated into the local folder and reach the team when the user publishes — and the evidence it rests on (88 local skill folders carrying zero `evals/` directories) is untouched, because that evidence is about where cases are *authored*, not about what a version *means*. What is overridden is the other half:

- **Not content identity.** `skillContentDigest` skips `evals/` (`src/lib/skills.ts`, `isEvalAsset`). A version number answers one question — *did the skill change* — and it drives the marketplace's stale-copy disclosure (D4), install, the publish comparison and the local eval score. An eval that only added or corrected a case was moving all of them.
- **Published beside the versions, not inside one.** `skills/<name>/evals/**`, mutable, under new guard row a″ (§4.2). This is forced: row a′ makes a version folder add-only as a whole prefix (OF-3), so a case committed into `v3/` could never be corrected without minting `v4` — the very churn being removed. Add and modify are permitted, removal is not, so a folder that happens to be missing a case cannot delete the team's.
- **Not copied on install.** An installed copy is the version folder, which no longer contains `evals/`; a dataset is authoring material, not runtime skill content. This also settles the question eval-engine §16.15 left open for the phase-1 Placer.
- **Legacy `v<N>/evals/**` stays.** Those versions are committed and immutable; row a′ still admits them and nothing rewrites them. There is no migration.

*Consequence, stated plainly:* a publish whose only change is an eval asset now mints **no** version — it writes the assets and attaches any matching receipts to the version that already holds those bytes. That is the outcome the override exists for.
...
**D19 — The publish regression gate is relocated into `publish`, not retired.** The eval-engine spec gated a publish PR on a not-FAIL receipt; §12 deletes PR creation and with it that CI check, and nothing in this spec replaced it. Two walks ruled that a gate should exist (the 2026-09-06 publish-gate ruling; the 2026-09-07 hygiene/publish-gate walk), so the refusal survives as a local question: when the newest local receipt for the exact candidate bytes carries a candidate-vs-incumbent **FAIL**, publish asks before proceeding (§5.1 step 6a), `--allow-regression` pre-answers it, and the absence of a receipt never blocks. It reads only the local store — nothing moves — and it is asked, not silently enforced or silently dropped.

**D20 — Setup's `actions` step is deleted; setup is twelve steps.** Its only body offered `connect` (`setup.ts:291-295`, "Offering local skills"), which §12 deletes. Under the North Star nothing is published unless the user asks, and the Library's publish button is the ask — an onboarding step that offers to publish local skills is auto-share with a confirm on it. The list becomes `welcome, app, role, github, team, invite, projects, evals, community, hook, wrapper, done`; `SETUP_STEP_KEYS`, `SETUP_STEP_TO_BOARD`, `SetupBoot.tsx:10`, `setup-session.ts:15` and `e2e/routes/setup-evals.spec.ts:12` follow in the same batch (B1). §9.2's "thirteen steps" becomes twelve.

**D21 — `readme --pr-comment` becomes a no-op-with-notice shim for one major.** The mode reads `team.global` (`src/commands/readme.ts:20-26`) and is the committed Action's `publish-comment` job (`team.ts:620`), which runs on pull-request events the product no longer creates; a hand-made PR would still trigger it against a layout-3 repo. Same rule as `receipt-check` (§12): the flag stays registered, prints one line, exits 0.

**D22 — The Inbox's `update` and `review` item kinds are deleted and the surface stays dark.** They encode the two mechanisms this refactor removes (update-available placement, PR review); `surfaces.inbox` is already false and the route redirects. `InboxScreen.tsx:18-49`, `settings-data.ts:7`'s `inboxLabels`, the `InboxUpdate` fidelity board (`boards.ts:258-259`, `FIDELITY.md:40`) go, and `boards.test.ts`'s pin moves with the board count in the same batch (B1). §11.5's open ruling is closed by this.

**D23 — `policy.publish` leaves the desktop's `Settings` type; the fixture schema loosens.** `desktop/src/fixtures/schema.ts:64` requires `TEAM_POLICY.publish` in `design.json`, which `export:check` byte-locks; the schema file is hand-editable, `design.json` is not, so `publish` becomes optional there and the deviation is recorded in `desktop/FIDELITY.md`. The three projections in `tauri/index.ts` go with the CLI field (§11.5).

**D24 — Frame verbs.** `prune` and `skill move|rename|delete` are `FRAME_VERBS` entries (each has a seam: `backend.prune()`, `backend.skillFile.*` — `skillFile`, because `Backend.skill(q)` is already the detail reader, OF-10, §7.5); `team migrate` is **not** — it is run once per team by a human from a terminal, never from the app, and the inventory tests (`frames.test.ts:192`, `frames-cli.test.ts:67-74`) pin that.

**D18 — `skill move` is the only Move, and it lives in the Library.** The detail page's install-then-uninstall re-place (`SkillScreen.tsx:85-87`, Ryan 2026-09-09) and the marketplace card's Move are deleted; that ruling's "placed twice rather than nowhere" guard is superseded, because a filesystem move never fetches the clone (a local edit is never silently replaced by the team's copy) and, being copy-then-remove across volumes, an interrupted move is finished by a re-run rather than by a second placement (§7.5's write contract). (§7.5, §11.4)
...
skills/<name>/v<N>/**                              immutable. never modified after commit.
                                                   (NOT evals/ — overridden 2026-09-13;
                                                    legacy v<N>/evals/** stays where it is)
skills/<name>/evals/**                             mutable. the skill's eval dataset, beside
                                                   its versions. guard row a″.
people/<handle>.json
...
The record stream is unchanged and stays frozen: `` `${digestKey(relative)}:${sha256hex(content)}\n` ``, ascending by unescaped path, `SKILL.md` passed through `canonicalSkillMd` first (which normalizes `license`, `metadata.id` and `metadata.author` — three fields, not four; `terum-category` stays, D2), result prefixed `sha256:`. `digestKey` is the existing prefix-free escape (`src/lib/skills.ts:147`) — do not invent a second one. Mode and mtime are **not** in the stream (D2).
...
| `receiptSchema.skill_id` (`src/lib/evals/receipt.ts:52`) | `z.uuid()` | `z.union([z.uuid(), z.null()])` — null = a local run not yet attached (§6.1); a committed receipt is never null |
...
Model the preprocess on the existing `teamsSchema` one (`src/lib/schema.ts:104-112`), whose comment already documents this exact failure class. Object-level `z.preprocess`: map any 40-hex `version` in `placements`, in `pending`, and in `personSchema.installed[]` to `null` ("placed before versioning"). The same preprocess normalizes `scope.project` in `placements[]` and `pending[]` to `Global` when it case-insensitively equals it — the migration commit runs on one machine and cannot reach another member's config, and §10 deleted the sync pass that used to reconcile placements. Test that an untouched pre-upgrade `config.json` parses, that a `preserveUnchanged` update afterwards does not trip `patchConfig`'s throw, and that the stale bytes are dropped by the next plain `update()`.
...
- `declined[]` — retained in the schema so existing files parse, but **no longer written**: its only job was suppressing the endorsement-driven auto-install that §12 deletes. The `decline` verb goes. The write and its three readers are deleted by **B6** (§9.1, D62) — until that batch the field is still written by `uninstall` and read in three places, and that is §16.1's sequencing, not drift.
...
- `[modify] src/commands/validate.ts` — a ref that is not a local directory resolves via `listVersions` to the **pair** `{ name: <the ref as typed, i.e. the skills/<name> segment>, directory: <clone>/skills/<name>/v<max>/ }`, not to `<clone>/skills/<name>`; `assessHygiene` is called with that `name`, explicitly **not** with `basename(directory)` (`validate.ts:38`), because under layout 3 the basename is `v<max>` and HYG1 (`hygiene.ts:47-48`) would reject every valid published skill — the invariant being checked is §3.1's "`<name>` must equal the `name` frontmatter of the latest version", and `v<max>` is not that name (the same rule the `skills.ts` bullet states for `skillRecords`). The local-directory arm is unchanged and keeps `basename(directory)`, where the basename *is* the skill folder; `ValidateResult.name` is the skill name on both arms; a name with no version folder fails with the same `skills/<name> holds no v<N> folder.` message. Test: `src/commands/__tests__/validate.test.ts` — `validate deploy-check` against a clone holding only `skills/deploy-check/v2/**` passes hygiene and reports `deploy-check`, not `v2`; the same ref with no `v<N>` folder fails with the `holds no v<N> folder.` message; a local folder ref still takes its name from its basename.
...
- `PEOPLE_ACTIONS` becomes `['join','install','uninstall','profile','publish']`. **Edit this list in exactly one place and re-list every verb that writes the file in the same change** — a mismatch here throws `Write guard refused people/<handle>.json for install` on every install. `'migrate'` is deliberately absent: its people-file writes (every member's, §13 step 5) are admitted by row j, which runs first, because row b only ever admits the actor's own handle.
- Row c (`guardTeam`): `publish` may append to `projects[].skills`; the `global` branch is deleted.
- **New row j — migrate.** It must be the **first** clause of the loop, before the `team.json` branch at `:51`: `if (context.action === 'migrate' && permitsMigration(tree, path)) continue;`. Placed after the `team.json` branch it is unreachable, because `guardTeam` claims `team.json` first and refuses a migrate diff. `permitsMigration` **must not call `parseTeam`** — the pre-image is layout 2 and the layout-3 `teamSchema` rejects it (use `anyLayoutTeamSchema`, §4.1, or validate structurally). It is defined by the diff §13 actually produces, and the guard sees every move **twice** — `MutableTree.remove()` stores `undefined` in the overlay (`teamRepo.ts:324`) and `changedPaths` (`:320-321`) lists removals as well as adds — so a destination-only admit list refuses the migration's own commit. Three direction-keyed arms: **(i) adds** (`tree.before(path) === undefined && tree.after(path) !== undefined`) matching `/^skills\/[^/]+\/v1\/.+$/` or `/^evals\/[0-9a-fA-F-]{36}\/(v1|archive\/[0-9a-f]{40})\/\d{8}T\d{6}Z\.json$/` — **the `archive/` arm is not optional**: D7's archive path is in the locked layout (§3.1) and is admitted by no other row, so without it the migration's own push is refused by the guard it just passed; **(ii) removals** (`tree.before(path) !== undefined && tree.after(path) === undefined`) matching `/^skills\/[^/]+\/(?!v[1-9][0-9]*\/).+$/` — every layout-2 file under a skill folder that step 1 moves: `SKILL.md`, `scripts/**`, the old `evals/**` assets, anything else a real repo committed — or `/^evals\/[0-9a-fA-F-]{36}\/[0-9a-f]{40}\/\d{8}T\d{6}Z\.json$/`, the tree-hash receipts step 3 re-keys or archives. A removal under `skills/<name>/v<N>/**` stays refused even under `'migrate'`: the migration row does not open immutability; **(iii) modifications** of `team.json`, `README.md`, and `people/<handle>.json` for **every** handle (`/^people\/[^/]+\.json$/`), because §13 step 5 rewrites all members' files while row b is keyed to `people/${context.handle}.json`, the actor's own — which is why `'migrate'` is deliberately **not** in `PEOPLE_ACTIONS`. The migration commit's diff is exactly the union of these classes and nothing else.
...
**Done means:** `publish <ref>` copies one local skill folder into the team repo as its next immutable version, refuses a byte-identical republish, attaches every local eval run against those exact bytes, records the chosen project whether or not it minted a version, offers the profile — and nothing else can write a skill.
...
4. **Managed fields, resolved.** `injectManagedFields` into the in-memory `SKILL.md` — `license` from `team.json.policy.skill_license`, `metadata.id` (existing, or minted at v1), `metadata.author`, and `metadata.terum-category`. The category is resolved with the auto-category spec's own precedence (`.planning/specs/2026-09-10-auto-category.md` §2–§5; its wiring moves from `connect` to here): declared in `SKILL.md` › `--category <name>` (the flag moves from `connect` to `publish`) › `suggestCategory(raw, team.categories, agent)` › `DEFAULT_CATEGORY`, and the disclosure line of auto-category §4 (§5.2) — silent for a declared category — is printed before publish's existing confirmation, so the auto-category walk's Decision 2 and R1 keep their rendering site. **This is the only surviving caller of `injectManagedFields`**, migrated out of `connect.ts` before that file is deleted.
5. Hygiene checks on the **injected** map — `assessHygiene(name, { files, executable }, policy.skill_license)` (`src/lib/evals/hygiene.ts:120`) already takes exactly this shape — including the privileged-grants confirmation. **Never before step 4:** `skillFrontmatterSchema` is strict and requires the managed fields (`schema.ts:142-148`), so a folder that has never been published fails HYG1 on fields publish is about to write. `connect.ts:225→:228` already runs in this order; publish keeps it. **Once hygiene passes, publish writes the injected `SKILL.md` back into the local skill folder** — the same write `connect.ts:243` makes today, and the **only** local write publish ever makes; a refused publish leaves the folder untouched. From the first publish on, the bytes on disk, the bytes digested and the bytes in `v<N>/` are one set: a later publish finds a declared category and makes no model call, and a later `eval` digests the same bytes publish compares (§6.1).
6. `candidate = skillContentDigest(files)` — after step 4, so the comparison is post-normalization (§1.2).
6a. **The regression gate, local (D19).** If the newest local receipt whose `content_digest` equals `candidate` carries a candidate-vs-incumbent **FAIL** verdict, publish stops and asks: *Your latest eval of these exact bytes failed against Version K. Publish anyway? [y/N]* — `--allow-regression` pre-answers it for the desktop and for scripts. Absence of a receipt never blocks (a first or unevaluated publish proceeds), and the gate reads only the local store: nothing moves. This relocates the publish-gate ruling from the deleted PR check into the verb itself.

6b. **The local write-back, after the last refusal (OF-2).** Publish writes the injected `SKILL.md` into the local skill folder — the same write `connect.ts:243` makes today, and the **only** local write publish ever makes. It happens here, after step 6a, so a publish refused at *any* question leaves the folder byte-identical; the earlier ordering wrote at step 5 and then asked at 6a, so answering **N** left injected fields behind on a publish that never happened.
**The boundary against a failed `safeWrite`:** the local file is written *before* the team write, and a `safeWrite` failure therefore leaves the injected `SKILL.md` on disk with nothing published. That is the safe side of the boundary and it is deliberate — every injected field is re-derived identically on the next attempt (`metadata.id` is reused from the file, `author` from identity, `license` from policy, and the category is now declared), so the retry is idempotent and mints no extra version. The opposite order would be worse: a successful push whose local write then failed would leave the two mirrors disagreeing with nothing on screen to say so.
...
8. **If a version's digest equals `candidate`:** do not mint. `identicalTo = versionFolderName(K)` (the string `'v2'`, matching §5.3). Still attach any local receipts whose `content_digest` equals `candidate` and that are not already committed under `v<K>` (D from the refactor's republish rule). The mutation is empty (and `safeWrite` returns `changed: false`) only when no version was minted, no unattached matching receipt exists **and** the uuid is already in `projects[<project>].skills` (step 9a).
9. **Otherwise:** `N = highest + 1` (never `count + 1`; on a first publish the highest is 0, so `N = 1`). Write every entry of `files` under `skills/<name>/v<N>/`, eval cases included. Attach matching receipts under `evals/<uuid>/v<N>/` — *attach* means: write a **copy** of the local receipt at `evals/<uuid>/v<N>/<runId>.json` with `skill_id` set to the publish uuid (existing, or minted at v1) and `version` set to `v<N>`, every other field byte-preserved, `content_digest` included; the run under `~/.terum/skills/evals/local/<digest>/<runId>/` is **not** rewritten — it is local state, and rewriting it would move bytes the user did not ask to move. The stamping is what makes §8.1's `newest.receipt.skill_id != skillId` misfiled check safe; attach is never a plain file copy. Step 8's attach is the same operation.
9a. **After either branch:** append the uuid to `team.json.projects[<selected project>].skills` if absent. This is the only writer of project membership and it does not depend on whether a version was minted — the project list records where the team exposes a skill, the version records its bytes — which is what makes §11.6's *"adding a skill to a project is now `publish --project`"* true for a teammate's unchanged skill.
10. Append/refresh `installed[]`? **No.** Publishing is not installing — D5 splits the two lists, and `installed[]` means *a copy is on a machine*. The prompt's sentence *"The person who published it has their .json changed to include that skill"* is satisfied by writing `profile[] {via:'publish'}`. Only that entry writes the people file here, and under **D77** it is written with no question — publish prints `Your profile now lists <name> at <label>.` and a failure to write it still leaves the publish successful.
...
- Category, printed before the confirmation in the three cases the auto-category spec's §4 table names, **verbatim from that table** (it owns the strings; do not re-copy them here): `metadata.terum-category: X (from --category; edit SKILL.md any time)` · `metadata.terum-category: X (suggested from your SKILL.md; edit any time)` · `metadata.terum-category: misc (couldn't reach the model; edit SKILL.md any time)`. A `SKILL.md` that already declares a category prints **no line and makes no model call** (auto-category walk D2). The fallback is never silent: a silent fallback is how the catalogue drifts back to all-`misc`.
- Not attached, when local runs exist whose digest matches no version: `<n> local eval run(s) were not attached — they evaluated this folder before its first publish.`
- Identical but newly added to a project: `Added <name> to <project>. It is identical to Version 2 of the skill already in the team repository.` — the pinned clause intact. Identical and already in that project, nothing else to do: `Nothing to publish: <name> is identical to Version 2 and already in <project>.`
...
PublishResult = { team; id; name; project: string;
                  ...
                  projectAdded: boolean }      // this publish appended the uuid to projects[project].skills
```

`prUrl`, `compareUrl`, `branch` and `policy` are deleted from the DTO and from every reader — named: `cliPublish` (`desktop/src/backend/tauri/index.ts:51`) becomes `{ name, project, version: z.string().nullable(), created: z.boolean(), identicalTo: z.string().nullable(), attachedEvals: z.number(), profileAdded: z.boolean(), projectAdded: z.boolean() }`; the mapping at `:853` maps `version` straight through (today it reads `value.prUrl ?? value.branch`, so the rename is **not** a no-op); `PublishResult` (`types.ts:113`) drops `prUrl` and gains `created`/`identicalTo`/`projectAdded`; the mock at `mock/index.ts:193` stops minting a PR URL.
...
- **Identity at run time is the content digest.** `receiptSchema` gains `content_digest: z.string().regex(/^sha256:[0-9a-f]{64}$/)`, required at `schema_version: 2`, computed by the *same* `skillContentDigest` as the publish comparison (§3.3), over the folder exactly as it is on disk — `eval` never injects managed fields (so HYG1 treats them as optional for a local run, §6.3). The one legitimate divergence is a run made before the folder's first publish on a `SKILL.md` that declared no category (§1.2): publish **reports** those runs as not attached (§5.2) rather than silently skipping them, and every run after the first publish matches because publish wrote the category back (§5.1 step 5).
...
- The candidate is identified at run time by digest, never by ordinal; incumbent selection (§6.5) is therefore digest-and-receipt based, not ordinal.
- **`skill_id` is `null` for a local run on a folder with no `metadata.id`** (the folder's declared `metadata.id` when it has one), and is filled in by publish when it attaches the run. A *committed* receipt's `skill_id` is never null: `skill_id: z.union([z.uuid(), z.null()])`, null permitted only at `schema_version: 2`. At schema 2 a local run is identified by `content_digest` alone (§6.2's store key), never by `skill_id`.
...
- Resolve `ref` to a **local folder**, not a team skill. The ref is a skill name **or a folder path** (`~/…`, absolute, or anything with a separator — the desktop passes the Library card's path for a skill the team has never seen); a path outside every Library root misses like an unknown name. Miss message for a name: *"No local skill folder named `<ref>` in your library; install it from the marketplace first, or pass the folder's path."* — there is **no `--path` flag**; rev 9's mention of one was never built, and a message that promised it was a lie (fixed 2026-09-13).
- **Hygiene at local-eval time.** The hygiene tier still runs first and fail-closed (eval-engine §6.0; `eval.ts:131`), but on the folder **as it is on disk** — eval never injects (§6.1) — so `assessHygiene`/`inspectHygiene` gain one input flag, `managedFieldsAbsent: true` (the local-eval mode), under which HYG1's strict parse treats `license` and the three managed `metadata.*` fields as **optional** while every other HYG1 clause (no unknown top-level keys, folder name = `name`, `allowedTools()` grants) and every HYG2–HYG6 predicate stay unchanged and fail-closed; eval never sets `allowExecutable`; hygiene reads the in-memory `files` map and writes nothing, so the folder's bytes and its `skillContentDigest` are untouched. Publish never uses this mode — it injects first (§5.1 step 4) and assesses the finished bytes.
- **The trigger catalog.** `endorsedCatalog` (`src/commands/eval.ts:453-464`) read `team.json.global` and `endorsedCandidates`, both deleted. It is replaced by a purely local set: the direct child skill folders of the Library root that contains the folder under eval (`<project>/.claude/skills` when the folder sits under a registered `config.projects` root, `~/.claude/skills` otherwise), rendered `- <name>: <description>` from each folder's own `SKILL.md`, with `rejected`/`failed` folders (D16) excluded and the skill under eval always included even when it lives outside every root. No team read, no clone: a teamless run has the same catalog rule as a team one. Eval-engine §7.2's "endorsed set" is superseded by this bullet.
- Add `content_digest` where the receipt is built (`:295-324`), from the folder exactly as it is on disk through `skillContentDigest`, after the same ignore list — no injection, ever.
- **Publishing a matching receipt (Ajay, 2026-09-13).** A finished run whose bytes are ALREADY a published version writes its receipt to `evals/<id>/<vN>/<runId>.json` itself, stamped with the id and version it resolved to, and reports `publishedTo`. This is the case the request named: you install a teammate's skill, evaluate it, and they see the result without knowing to type `publish`. It is deliberately narrow — one path, guard row g, append-only, no version minted, no skill byte moved, no question asked — so §6.3's "publish is the only writer" stays true where it matters: publish remains the only thing that can change what a skill *is*. A folder with no matching version is not a published state and falls through to `shareHint` unchanged. `--no-commit` opts out; a failed write is reported and never fails the run, because the eval already ran and its local receipt is on disk. (`shareReceipt`, `src/commands/eval.ts`.)
...
- **Eval assets are written to the LOCAL folder, and only when they are missing (D9, D29).** Today generation has two routes and neither survives: with a connected source it calls `saveGeneratedAssets(shared.source, …)` then mirrors up through `reconcileShared`; without one it commits straight into the team repo under the `eval-assets` guard row. Both go. The new behaviour is `saveGeneratedAssets(<the local skill folder>, generated)` — and **`--gen` is deleted (D29)**. The rule is: *use the assets that are there; generate only the ones that are missing*, **per asset** — a folder holding cases but no `triggers.yaml` generates the triggers alone and leaves the cases untouched. This is already what `plannedGeneration` (`eval.ts:262-263`) does apart from its `Boolean(args.gen) ||` limb, whose only job is forcing generation over existing files; deleting the limb means nothing can ever be overwritten, so both `pathExists` refusals in `saveGeneratedAssets` (`:315-316`) become unreachable and go with it — their `--save refused:` text already names a flag deleted in B1. `--no-gen` survives as the honest opposite ("use what is there, spend no model call"), and `--gen`'s conflict guard against `--case` (`:75`) and its rejection in queue modes (`:441`) simplify away. **Regenerating is deleting `evals/cases/` and re-running** — say so in the miss message for anyone who goes looking for the flag. Generation still runs into `runDir/generated` for the run itself; only the write-back into the skill folder is governed here. The cases then reach the team the same way every other byte does: the user publishes.
  ...
  **Consequence to state in the UI:** ~~generating cases changes the skill's content digest, so the next publish mints a new version *and* the existing local eval score blanks (§7.3).~~ **No longer true as of the 2026-09-13 D9 override:** eval assets are outside content identity, so generating cases mints no version and blanks no score. The printed line still names the path and the fact that files were written into the user's folder without their asking — that disclosure is the reason it exists, and it stands. The generate flow **must** say so before it writes, and under D29 that line matters more, not less: with no flag to ask for generation, the first eval of a skill writes into that skill's folder without the user having requested it, so the printed line — naming the path and the consequence — is the only signal they get. It stays a **print, not a prompt**: nothing leaves the machine here, publish still asks before anything does, and publish already writes into the same folder (§5.1 step 6b).
- `EvalResult.id` becomes `string | null` and `team` becomes `string | null` — the local-folder resolve has neither (§6.6's queue item already carries `team?`); `eval.ts:284`'s `writeRunTree` header follows.
...
- `materializeIncumbent` (`src/commands/eval.ts:466`) becomes `incumbentDir(clone | null, name, versions, candidateDigest)`. The candidate has **no ordinal** at run time — it is a local folder identified by its content digest (§6.1) — so the incumbent is chosen by the rule the 2026-09-07 hygiene/publish-gate walk locked (Decision 2) and `.planning/specs/2026-09-04-eval-engine.md` §11 states: **the receipted version whose newest run-id is most recent**, excluding any version whose `skillContentDigest` equals `candidateDigest` (the version-folder form of "excluding the candidate's own hash"), read directly from `join(clone,'skills',name,'v<K>')`. When there is no clone, no team, **no version carrying a receipt**, or nothing left after the exclusion — an unpublished folder, a first publish — there is **no incumbent** (the legacy `origin/main^` fallback at `eval.ts:472-476` is deleted with `materializeVersion`, not re-expressed): the run is single-arm and the receipt records the no-incumbent path exactly as eval-engine §7.1 already names it. No `materializeVersion`, no cache, no `git rev-parse origin/main^:…` — a version folder is already an immutable checkout inside the clone, and the no-clone arm returns no incumbent rather than reaching for a tree, which is what makes §4.1's deletion of `materializeVersion` safe.
- `latestReceiptedTree` (`:479`) becomes `latestReceiptedVersion(clone, id, exclude)`, filtering directory names with `parseVersionFolder(d) !== null` and returning the `v<N>` whose newest run-id wins — the same receipt-recency rule as the bullet above. Ordinal order is **not** the tie-breaker: a `v2` re-evaluated today is the more current comparison than a `v4` evaluated last month (the locked rule; §14.1 pins it).
...
- **`SkillCard` stays ONE shared type** (`desktop/src/backend/types.ts:33`); the Library and the Marketplace are two *builders* over it, not two types — splitting it would fork every card component, the `⋯` menu and the router. The type gains `localEval: ReceiptSummary|null`, `localEvalStale: boolean`, `installedVersion: string|null`, `latestVersion: string|null`, `evalVersion: number|null`, `evalStale: boolean`, `latestEvalState: 'ok'|'none'|'invalid'|null`, `profileVersion: string|null` — all **required-nullable**, so neither builder can forget one.
  The Library builder sets every team-derived field to a fixed neutral value, named here once: `installs: '—'`, `installsN: 0`, `teamState: 'unknown'`, `latestVersion: null`, `installedVersion: null`, `evalVersion: null`, `evalStale: false`, `latestEvalState: null`, `profileVersion: null`. `installs` is the display string on the shared type (`types.ts:33`) and `installsN` the number; the Library reads no team source, so the chip makes no claim — `'—'` — and the number exists only to satisfy the type, never rendered, sorted or summed on a Library row. `localCard`'s two-case `local ? '0 installs' : '—'` (`tauri/index.ts:144`) collapses to the dash because §12 deletes `connect` and `config.shared`, the predicate it keyed on. It sets `localEval`/`localEvalStale`; the Marketplace builder leaves those null/false. `teamed` stays on the type and is set by both — `detailPath()` (`skill-card-actions.ts:11`) routes on it.
...
- **Every direct child folder of a Library root is a card (D16) — and every filter between a folder and its card is deleted, named here:** (a) `[modify] src/lib/local-skills.ts` — delete both `if (!tracked) return null` early returns (`:152` non-directory, `:159` directory with no `SKILL.md`), so every direct child becomes a `LocalEntry` with a `rejected` inspection; today those folders reach neither `rows`, `notOffered` nor `problems`, so "`ls --local` already emits rejected rows" is only true of folders that reached the frontmatter stage. (b) `[modify] src/commands/ls.ts` — the `kind:'failed'` arm (`:266`) pushes to `local.notOffered` as well as echoing to `problems`; `LocalSection.notOffered[].reason` (`:48`) widens to `SourceProblem | 'failed'` with the error text in `detail`; `localSkillCounts` (`:193-195`) becomes a plain count of direct child directories and `FRONTMATTER_PROBLEMS` (`:192`) is deleted. (c) `[modify] desktop/src/backend/tauri/index.ts` — delete `countable` (`:123`), the desktop's own seven-reason filter, and its three call sites — the grid loop's skip (`:688`), `localDetailByName` (`:162`) and the path detail route (`:711`) — so a card that exists can also be opened; delete `visibleSkillFolders` (`:129`) and have `rootOf` (`:104`) and `status` (`:286`) read `section.counts.skillFolders`, which is what makes the sidebar count and the grid agree. (d) Consequence, accepted: the bundled `/terum-skills` manual (`managed-wrapper`) draws a card like any other folder. `candidate` renders normally; `rejected` and `failed` render with the name, the path and the inspection reason as the body, excluded only from eval and publish. **Amended 2026-09-14 (Ryan):** one carve-out, for the bundled manual alone. `managed-wrapper` is not a fault — setup placed that folder and `connect` refuses it by its frontmatter marker, so no user action can ever make it a team skill — and rendering it as a red `broken` card told every user something was wrong with their machine. It keeps its card and its own SKILL.md description, and carries a fourth, neutral indicator, `bundled` (`box` glyph, `text3`, "Bundled with terum-skills — placed by setup, not a team skill."): a muted chip on the card, the same sentence always visible under the description on the detail page, and the reason on the disabled Run eval / Publish rows — `localActionReason` reads `broken || bundled`, so the exclusion D16 already required is unchanged. It is also out of the attention tile, which counts `broken` only. Every other `rejected`/`failed` reason keeps the red flag, the path-and-reason body and its place in that count.
...
**The safety rail** (replacing the ledger-fingerprint check, which does not exist for a folder Terum never placed): the target's parent must satisfy `isSkillsRoot()`, the target must sit **directly** under it, and the parent must be one of the Library's own roots (`~/.claude/skills` or a `config.projects` entry's skills root). Symlinks are refused. **The typed-name confirmation is `delete`'s alone (Ryan, 2026-09-14, superseding rev 9's "and the user must confirm by typing the skill's name" for all four):** move, copy and rename are each undone by running the same verb the other way, and none of them overwrites anything — a folder already at the destination is kept in `old-skills` — so the typed name was friction, not a rail. Delete is the mode that ends in quarantine, or in the team's copy being the only copy, and it keeps the ask. Both halves move together: the CLI stops asking (`src/commands/skill.ts`) and the dialog stops drawing the field, because a dialog that answers a question it does not draw is the thing the 2026-09-09 remove-dialog ruling forbids. **The destination picker offers no "Choose destination" row and no root the folder already sits in** (the CLI refuses that as the same folder): it opens on the first other root, and with none it says so and leaves the action disabled.

- **delete** is a three-way removal contract, not one promise (OF-11, D74 — rev 8's wording, *"undoable, and `prune` is the only thing that ever hard-deletes"*, was true in substance and false in wording; **no behaviour changes** and `src/commands/uninstall.ts:178-241` is untouched). A folder that is **not a placement** is always quarantined: it goes through `moveToQuarantine()` — the existing "moved, never deleted" primitive (`src/lib/placer.ts:170`) — and is undoable until `prune`. A placement that is **unmodified** (its current fingerprint equals the ledger's) is **removed outright** by `uninstallMany` → `removePlacements` → `placer.remove()`: the team repository holds its bytes, so the undo is a reinstall, and quarantining it would fill `quarantine/` with copies the repo already has. A placement that is **edited** (fingerprint differs) is **quarantined** by that same path — `remove()` reports `quarantined` and `uninstallMany` prints where it went (the branch at the end of `uninstall.ts:178-241`) — and the dialog repeats the line. `prune` is what hard-deletes the quarantine directory, and nothing else does.
...
- **`skill move` is the only Move (D18).** The detail page's `moveTo()` — install into the destination, then uninstall from the old scope (`SkillScreen.tsx:85-87`, Ryan 2026-09-09) — and its `dialog=move` are deleted, and the marketplace card offers no Move (§11.4). A filesystem move never fetches the clone, so a local edit is never silently replaced by the team's copy; across volumes it is copy-then-remove (`placer.ts:184-190`), so an interruption can leave both copies — the re-run rule below finishes it, which is what that ruling's guard becomes here. The 2026-09-09 ruling is superseded.

**The write order, and what an interruption leaves.** `[new] src/commands/skill.ts` (the verb), `backend.skillFile.{move,rename,delete}` on the seam (OF-10, D73: `Backend.skill(q)` at `desktop/src/backend/Backend.ts:28` is the detail reader and `library(q)` at `:19` the Library reader, so neither `skill` nor `library` can also be a namespace on the seam — a method and a namespace cannot share a name; `skillFile` names what the three modes act on, the folder) with zod DTOs in `desktop/src/backend/tauri/index.ts` and `mock/index.ts`, and `skill move|rename|delete` in `FRAME_VERBS` (`src/lib/frames.ts:36`), `CliVerbs` and `src/cli.ts` — the same shape §7.1 uses for L-PROJ. Ordered writes per mode: **rename** = `rename()` → `SKILL.md` `name` rewrite → ledger re-key; **move** = destination check → `moveDirectory()` → ledger re-key; **delete** of a non-placement = `moveToQuarantine()` only; delete of a placement = `uninstallMany`, unchanged (the three-way contract above). Every mode is safe to re-run, and a `config.placements` row whose recorded path no longer exists is **repaired, never stranded** — **by the skill's stable `metadata.id`, never by fingerprint** (OF-12, D75): the row's `id` is matched to the sibling folder under the same root whose `SKILL.md` `metadata.id` equals it, and the row is re-pointed at that folder; the recorded `fingerprint` decides **only** whether the card reads `edited` (§7.4), never whether the row survives. Rev 8's rule — re-point when the fingerprint matches, drop otherwise — dropped the row at exactly the interruption it was written for: after the folder rename the fingerprint has changed (the frontmatter `name` is part of it), so a fingerprint match can never repair a rename. The two interruption points, and how each repairs forward on the re-run: **(i) folder renamed, frontmatter stale** — `SKILL.md` still declares the old `name`; the re-run rewrites the frontmatter to the folder name and continues to the ledger; **(ii) frontmatter and folder renamed, ledger stale** — a ledger row whose path is missing is matched to the sibling folder whose `metadata.id` equals the row's id and re-pointed. A row whose id matches no sibling is dropped, and only then. This is the local twin of §13's `pending` self-drain, and what makes the stale-ledger-key hazard above survivable. **B5 implements it in `[new] src/commands/skill.ts`.** A move whose destination already holds a folder of that name follows §9.1.1's one rule: the existing folder is kept at `<root>/.claude/old-skills/<name>` and the user is told. Tests: `src/commands/__tests__/skill.test.ts` — interrupt after the directory change, after the frontmatter write and after the ledger write for each mode, plus an `EXDEV` move that leaves both copies and is completed by a re-run — and **one test per interruption point above (D75)**: (i) a renamed folder whose frontmatter still carries the old `name` is re-run and ends with the frontmatter rewritten and the row re-pointed; (ii) a renamed folder with rewritten frontmatter and a stale row ends with the row re-pointed by `metadata.id` although its fingerprint no longer matches the ledger's; and a placed folder edited before the rename keeps `edited: true` after the repair, because the fingerprint was never the join key.

They are `FRAME_VERBS` entries with a seam each (D24). Register the three modes in `SERVE_READ_VERBS`? **No** — they are writes; §1.5. Note `serve` gates on `argv[0]`, so `project` cannot be added there either: `project add|remove` are writes even though `project list` is a read.
...
selectCardEval(clone, skillId, versions /* DESC by n, non-empty */) ->
  ...
  for v in versions:                                            // descending
      ...
      if newest.receipt.skill_id != skillId or newest.receipt.version != v.folder:
...
- **`uninstall` is the inverse of the automatic half only.** It removes the `installed[]` entry for the last scope removed, exactly as today (`uninstall.ts:140-141`), and **leaves `profile[]` untouched** — a profile entry is a curated statement about a skill, not a claim that a copy is on this machine (D5); only `profile --remove <skill>` empties it (built under D77, rev 11), and the uninstall notice says so. Its `declining` computation is deleted with `declined[]` (§3.5) — **and that deletion is B6's, under §16.1's build order (D62).** `declined[]` is still written and read on `main` until then, deliberately: the writer is `uninstall.ts:207-212` (the `declined.push` inside `uninstallMany`'s people-file mutation) and its three live readers are `src/commands/uninstall.ts:145` (the `declining` preview computation), `src/commands/install.ts:170` (the `person.declined.filter` that un-declines a skill on install) and `src/commands/ls.ts:217` (`member.declined` on `ls member`'s result). B6 deletes the writer and all three readers together, keeps the schema key readable so existing people files parse (§3.5), and drops the field from `ls member`'s DTO and its desktop mirror in the same change. No earlier batch touches them — B3's review flagged the surviving write as contradicting §3.5, and the code was right; the record was wrong.
- **Seeds the local eval store (D11).** Copy every committed receipt for the version being placed — `evals/<uuid>/v<N>/*.json` in the clone — to `~/.terum/skills/evals/local/<digest>/<runId>/receipt.json`, where `<digest>` is **the receipt's own `content_digest`** (OF-15, D76) — the store is keyed by the digest the receipt records, never by one install computes from the placed folder, so a receipt can only ever be filed under the bytes it evaluated. A receipt that carries no `content_digest` — a schema-1 receipt that §13 re-keyed before step 3 stamped digests, or one copied in by hand — is **not seeded**, and install prints exactly one line for it: `Skipped <runId>: no content digest (pre-migration receipt).` — `<runId>` being the receipt's file stem. Not an error, not a silent skip. No other transformation: the receipt already carries `version: 'v<N>'` and `provenance.runner_handle`. This is a file copy of data already in the clone; it moves no bytes between machine and team. **B6 builds it** — nothing seeds today, `install.ts` has no seeding step — and §13 step 3's stamp (B8) is what makes a migrated receipt seedable at all.
...
**The destination picker is lifted from closed PR #167**, not rewritten — `install-destinations.ts` and its test are exactly this builder — **and the lift carries one rename (OF-14, D73; B6):** the lifted file reads `features.checkouts` (`destinationsFor`'s `features: { checkouts: boolean }` parameter at `:4` and `addRow: features.checkouts` at `:10`), the feature key B2 renamed to `libraryProjects` (§7.1 — `FEATURE_KEYS` in `desktop/src/backend/types.ts` and `FRAME_FEATURES` in `src/lib/frames.ts` both carry the new key on `main`, and `checkouts` exists on neither). Pasted as-is, a caller handing it the app's `Features` fails typecheck on the missing key, and a caller that hand-builds `{ checkouts: … }` reads `false` forever and hides the Add-project row. Both occurrences and the test's fixture become `libraryProjects` in the lift commit; nothing else in the file changes. They stay retrievable from the intact branch:
...
New `sync` description: *"Fetch each team clone and reset it to origin/main. Nothing on this machine is changed: no placement, no upload, no edit to your skills."* The one exception is Terum's own `/terum-skills` manual, refreshed by `sync --hook` with a printed line (§11.1, D17). The two surviving user-facing promises that sync writes — `login`'s identity notice and `status`'s pending guidance — are re-worded in §11.5.
...
`[modify] desktop/src/backend/tauri/index.ts` — **the `sync` seam is rewritten under the app and must be named.** `sync` becomes `run(['sync', ...(args.team ? ['--team', args.team] : [])], cliSync, …)`. `SyncArgs` loses `auto`, `freshMs` and `prune`; `SyncResult` loses `placed` and `deferred` and keeps `{ changed, teams[], notices }`. `--prune` moves to a new `backend.prune()` seam (a `FRAME_VERBS` entry, D24), and the Prune button and its hint in Settings move with it. `features.autoSync` retires from `FEATURE_KEYS` (`desktop/src/backend/types.ts:12`) and every `hello.features.autoSync` branch collapses to the `refresh` path.
...
- **Move** (D18): `moveAction` is offered on Library cards only and routes to `backend.skillFile.move` — the `skill move` verb (§7.5; OF-10's namespace); it is removed from marketplace cards, together with the detail page's `moveTo()` and `dialog=move`.
- **Run eval** gates on `skill.path`: `reason = skill.path ? null : 'Install it first — evals run against the copy on your machine.'` Same gate on the detail page's Evals-tab button and the `EvalsEmpty` primary (`SkillScreen.tsx:135`). **Who builds which (A3, 2026-09-12):** B3 put the gate inside `RunEvalDialog` itself — with no local copy the dialog shows that sentence and offers no Run eval — because the dialog is the one choke point every entry path reaches (card row, Evals-tab button, `EvalsEmpty` primary, a pasted `?dialog=run-eval`), which made B3 safe standing alone without touching this file; it does **not** discharge the disabled rows. The **marketplace half landed on B4** (`skill-card-actions.ts`'s `run-eval` and `publish` rows with their reasons). The Library card needs nothing more: its Run eval and Publish rows are the same `cardActions` rows (`skill-card-actions.ts:24`, `:52`) that the one `SkillCard` renders for both screens, so they landed with the marketplace half on B4. The **library-side disabled rows still owed are B5's**: the Evals-tab button and `EvalsEmpty` primary on the detail page (`SkillScreen.tsx`) — each gated on `skill.path`, each with the reason string above, each a disabled row with a reason and never a hidden one.
...
- **`src/commands/login.ts:42`'s identity notice** still promises that *the next sync writes* the author line into *the skills you have connected on this machine* — false three times over once §12 deletes `connect` and `config.shared`, §10 deletes every sync write, and §5.1 moves `metadata.author` injection to publish. Rewrite it in the same shape (printed and returned as `notice`, inside the `store.update` callback so it renders the post-write author line): *This changes the author line (`<display_name> <email>`) that `publish` writes into the skills you publish from this machine; versions already published keep their recorded author.* `src/commands/__tests__/login.test.ts` and the desktop surface that renders `notice` move in the same commit. The sibling's contract for this string (`.planning/specs/2026-09-02-phase-1-build.md` §6, the `login --set` notice) is superseded by this bullet.
- **`status`'s pending-work guidance** (`.planning/specs/2026-09-02-phase-1-build.md` §6: *"its only remedy is `sync`, which finishes or unwinds the entry"*) is superseded by §13: the remedy for a `pending` row is to re-run the matching `install` / `uninstall-skill`, which drains its own pre-existing row, and a row that will never be retried is cleared when its team is torn down. `syncedAt` keeps its field and its null semantics but reads the redefined stamp (§10) — *fetched*, never *synced*, in every string derived from it. More broadly, every phase-1-build §5.4/§6 sentence that describes `sync` as the reconciler (the `pending` replay, install/uninstall ordering "for the next sync to finish", the orphan/local-changed/blocked rules, requirement 25) is superseded by §10.1's table and §13; D17's manual refresh is the single carve-out.
...
- **The Inbox's `update` and `review` item kinds** encode exactly the two mechanisms this refactor deletes (update-available, PR review). **Ruled (D22):** delete both kinds; with `surfaces.inbox` false the route already redirects and the surface stays dark. `InboxScreen.tsx:18-49`, `settings-data.ts:7`'s `inboxLabels`, the `InboxUpdate` fidelity board and the `boards.test.ts` pin move in the same batch.
- **`readme --pr-comment`** (`src/commands/readme.ts:20-26`) reads `team.global` and is the committed Action's `publish-comment` job (`team.ts:620`): it becomes a no-op-with-notice shim for one major, like `receipt-check` (D21).
- **`desktop/src/fixtures/schema.ts:64`** requires `TEAM_POLICY.publish` in the byte-locked `design.json`: the schema loosens `publish` to optional and the deviation is recorded in `desktop/FIDELITY.md` (D23).
...
One verb, one target shape, one version number. `terum-skills team migrate`, guard action `'migrate'`. It is **not** a frame verb (D24): a human runs it once per team from a terminal, never the app.
...
2. Fold `team.json.global[]` into `projects.Global.skills[]`; create `projects.Global` if absent; delete `global`; delete `policy.publish`; set `layout_version: 3`. Parse the pre-image with `anyLayoutTeamSchema` (§4.1) — the strict layout-3 schema rejects its own input. **Check for an existing project whose name case-insensitively equals `Global`** before creating one: `projectNameSchema` does not reserve the name today, so a team may already have `global` or `GLOBAL`, and blind creation either collides or silently orphans that project's skills. If one exists, adopt it and normalize the key to `Global`. The adoption is a **rename**, not a new project: record the adopted old key, merge its skill list with the folded `global[]`, and re-key every reference to it — step 5 rewrites `installed[].scope.project` in every people file, because a project scope names a registry key and is matched by exact equality (`src/lib/schema.ts:43`; phase-1-build §5.2).
3. Re-key eval receipts: the receipts under the tree hash that `HEAD:skills/<name>` resolved to at migration time move to `evals/<uuid>/v1/`, with their `version` field rewritten to `'v1'`, a new `version_tree: '<40-hex>'` field preserving the original so a re-keyed receipt still provably describes specific bytes, **and `content_digest` stamped as `skillContentDigest(<the v1 files from the tree>)` (OF-15, D76; B8)** — the digest of the bytes step 1 just moved under `v1/`. The stamp is faithful by construction: a receipt is re-keyed **only** when its tree hash equals the skill's current tree (the `hashes.get(id) === hash` branch), so the `v1` folder's bytes are exactly the bytes that were evaluated. It is what lets §9.1's install seeding file the receipt under the digest the Library looks it up by; a re-keyed receipt left without one is skipped there with a printed line. Test: the stamped digest equals `skillContentDigest` of the migrated `v1/` folder. **Every other receipt moves verbatim to `evals/<uuid>/archive/<40-hex>/<runId>.json`** (D7) — archived receipts are **not** stamped: their bytes are no version's bytes.
4. Re-arm the clone's pre-push guard (§4.3).
5. Rewrite each `people/<handle>.json`'s `installed[].version` (and, when step 2 adopted an old key, every `installed[].scope.project` equal to it becomes `Global`): a 40-hex value becomes `'v1'` when it equals the tree hash step 3 re-keyed, and `null` otherwise ("installed before versioning"). §3.4's read-time preprocess protects *local* config **and covers `configSchema` alone** (OF-4); **the people files are committed team state and no preprocess touches them**, so without this step the first layout-3 CLI to read the repo fails `personSchema` on every member. These writes are admitted by guard row j (§4.2), not by row b.

**The migration parses the people files with its own legacy reader (OF-4).** Layout 3's `installedSchema.version` is `v<N> | null` (§3.5), so the layout-3 schema *cannot* read an un-migrated file — and no preprocess may quietly null those 40-hex values, because they are exactly what step 5 re-keys from. `team migrate` therefore reads each `people/<handle>.json` through an explicit legacy schema that accepts `version: 40-hex | v<N> | null`, holds the original hashes through the re-key, and then validates the rewritten object against the real `personSchema` before it is staged. A file that fails that final validation fails the migration loudly rather than being written half-converted. Test: a people file carrying a 40-hex `installed[].version` that matches a re-keyed tree hash becomes `'v1'`, one that matches nothing becomes `null`, and both results parse as layout-3 `personSchema`.

**No case rename is needed** (D1), so this runs cleanly through `safeWrite` — provided row j admits the removals and the people files (§4.2): the commit's diff is exactly the union of the classes row j names.

**`config.pending` is stranded by §10.** Today sync replays and clears it; §10.1 deletes sync's replay. `install` and `uninstall` must self-drain their own matching rows (filter out a pre-existing row before writing a new one), or a crashed install leaves a row nothing can ever clear and `status` keeps advising a sync that no longer does anything. A row for a skill that is never retried is cleared when its team is torn down: `teardownTeam` (`src/commands/leave.ts:63`) filters that team's rows out at `:119`, and `uninstall --machine` runs that teardown for every remaining team (`uninstallMachine.ts:126`) before its `store.remove` predicate at `:135`, so the `Kept … pending: N` branch is not reachable on the normal path and no new clearer is needed — §11.3's `teardownTeam` rewrite must keep that filter, the only remaining clearer. The residual is honest: with sync's replay gone a crashed row survives until the user leaves that team, which is why the self-drain above stays a requirement and why `status` must stop advising a sync that no longer drains anything (§11.5).
...
- `src/lib/__tests__/skills.test.ts` — a literal-hex pin on `skillContentDigest` for a fixed fixture (§3.3); `.DS_Store` does not change the digest; **editing `evals/cases/*.yaml` DOES** (D9); a change to `license`, `metadata.id` or `metadata.author` alone does not change it, and a change to `metadata.terum-category` alone **does** (a category is content); a `SKILL.md` with no `metadata:` block digests equal to the same file with `metadata: {}` and with `metadata:` null.
- `src/commands/__tests__/publish.test.ts` — republish of an unchanged folder returns `created:false, identicalTo:'v2'` with the verbatim message; republish after an eval attaches the receipt under `v2` and mints nothing; a changed byte mints `v3`; a repo whose highest version is `v5` with `v3` deleted mints `v6`, not `v5`; **an edit to `evals/cases/*.yaml` alone mints a new version** (D9 — cases are content identity), and a republish with byte-identical cases does not; a first publish of a folder whose `SKILL.md` carries only `name` and `description` succeeds (hygiene runs on the injected map, §5.1 step 5); after a successful publish the local `SKILL.md` carries the managed fields and a second `publish` makes no model call; a run of `eval` **before** the first publish of a category-less folder is reported as not attached, and a run **after** it attaches under `v1` and mints nothing; `--category <name>` wins over the classifier and a declared category wins over both; a declared category prints no line, a `--category` publish prints the `(from --category…)` line verbatim, and a second publish of the same folder prints no line (step 5 wrote the category back); an identical republish with `--project <other>` and no new receipts appends the uuid to that project, returns `created:false, identicalTo:'v2'` and is not `changed:false`, and the same publish repeated is `changed:false`; an id-less folder with a declared category is evaluated then published, and its receipt is attached under `v1` carrying the minted uuid and `v1` with the local copy unchanged; a FAIL receipt for the exact bytes stops publish with the D19 question and `--allow-regression` proceeds, while a folder with no receipt publishes without a question. **The first of these is the gate on §5.1 step 7's prefix stripping** — get that wrong and it is the only test that fails, so it must exist before publish does. **OF-2:** a matching FAIL receipt plus changed managed fields plus a declined regression gate leaves the local `SKILL.md` **byte-identical** — the case that would have caught the write-then-ask ordering.
- `src/lib/__tests__/guard.test.ts` — `skills/x/v3/SKILL.md` admitted as an add under `publish`; a *modification* of `skills/x/v3/SKILL.md` refused; `evals/<uuid>/v3/<run>.json` admitted; the old 40-hex receipt path refused; `v03` refused; a multi-path publish commit (version folder + team.json + receipt) admitted; `skills/x/v3/evals/cases/a.yaml` admitted under `publish` as an ordinary version byte, and `skills/x/evals/cases/a.yaml` (no version segment) **refused** — row h is gone; **row j** — the **complete** layout-2→3 diff is admitted under `migrate`: `skills/x/SKILL.md` and `skills/x/scripts/run.sh` removed with `skills/x/v1/SKILL.md` and `skills/x/v1/scripts/run.sh` added, `evals/<uuid>/<40-hex>/<run>.json` removed with its `archive/` counterpart added, `team.json` modified, and `people/another-member.json` modified; the same diff under `publish` is refused, a removal of `skills/x/v1/SKILL.md` is refused even under `migrate`, and the migration test (below) drives that full diff through `safeWrite` with at least two members; `guardRawPush` refuses `skills/x/v3/SKILL.md` with the publish-naming message (D15). **OF-3:** adding a previously absent file to a version that already exists in the pre-image is **refused**, while many files added together for a new version are admitted — the prefix clause of row a′, not the per-path one.
...
- `src/lib/__tests__/schema.test.ts` — an untouched pre-upgrade `config.json` with 40-hex versions parses and yields `null`s; a `preserveUnchanged` update afterwards does not throw; a `placements[]` row scoped to project `global` parses as `Global`.
- `src/commands/__tests__/ls-receipt.test.ts` (rewrite) — every branch of §8.1 including the invalid-newest-receipt descent, and a `skill_id: null` receipt under `evals/<uuid>/v1/` reported misfiled and not rendered.
- `src/commands/__tests__/skill.test.ts` — the D6 interruption matrix (§7.5).
- `src/commands/__tests__/leave.test.ts` / `uninstallMachine` — a config carrying a `pending` row for the torn-down team ends with `pending: []` and `config.json` removed in one run.
- `desktop/src/backend/tauri/__tests__/index.test.ts` — `catalog()` spawns exactly **three** CLI children (`status`, `ls --team <team>`, `ls --local`) for a five-member team — the point of the gate is that the count does not grow with team size (OF-9).
- `src/commands/__tests__/eval.test.ts` — the incumbent rule (§6.5): with receipts on `v2` and `v4` where `v2`'s receipt is newer, `v2` is the incumbent (receipt recency, the locked rule); a candidate byte-identical to `v4` excludes `v4` and picks the next receipted version; a folder with no team, or a name with no receipted version, runs single-arm with no incumbent and still writes a `version: null` receipt; a folder whose `SKILL.md` carries only `name` and `description` passes hygiene and produces a `version: null`, `content_digest`-bearing receipt, while a secret (HYG3) or a mixed-script token (HYG2) in the same folder still hard-stops; a two-folder root with one `rejected` folder yields a two-line trigger catalog, and a teamless folder still produces one; a published skill with two digest-unequal version folders and zero receipts, evaluated from a digest-unequal folder, runs single-arm with `version: null`; the no-team receipt carries `skill_id: null` and re-parses under schema 2. **D29:** a folder holding cases but no `triggers.yaml` generates the triggers alone and leaves the cases untouched; a folder holding both generates nothing and is never overwritten; `--gen` is not a recognised option.
- `desktop/src/backend/tauri/__tests__/index.test.ts` and an `ls --local` test — a root holding an untracked folder with no `SKILL.md`, an unreadable folder, a symlink and one good skill produces four cards, each detail route resolves, and the sidebar count reads 4 (D16).
- A migration test on a seeded layout-2 repo with at least two members, driving the complete layout-2→3 diff through `safeWrite`, asserting the exact layout-3 output, the archived receipts, every member's re-keyed `installed[]`, a pre-existing `global` project holding one skill plus a member `installed[]` row scoped to it ending as `projects.Global.skills[]` with the scope re-keyed, and that an un-migrated repo read by a layout-3 CLI produces the upgrade sentence rather than a zod dump.
...
**Open findings are tracked, not closed.** `/harden` stopped at two counted rounds when round 3's verify half hit a Claude spend limit; its Codex half had already found 21 more. Ryan's call (2026-09-11): implement against a tracked list rather than wait for a third clean round. Every unresolved finding from all three rounds — 3 + 4 panel splits, 6 confirmed-but-untriaged, 7 unverified — is in `.planning/reviews/2026-09-11-refactor-open-findings.md`, mapped to the batch that must resolve it at that batch's start. Three of them recurred across rounds with different verdicts each time and are flagged as probably-real regardless of their last vote. So the acceptance bar "three rounds with zero BLOCKER/DRIFT" was **not** met, and this spec is build-ready under that stated condition rather than unconditionally.
...
| `.planning/specs/2026-09-10-auto-category.md` | locked, **amended** | Survives with its wiring moved: the suggestion happens once, at the **first publish** (§5.1 step 4), with the sibling's precedence, its `--category` flag and its disclosure table unchanged (declared → no line), and is written into the user's local folder (§5.1 step 5). Its §4 ("Wiring into `connect`") and §7 ("at connect") are amended by this spec — see the note at its head. **Built as batch B9, after B5 (D28):** none of its machinery exists (`src/lib/categorize.ts`, `suggestCategory`, `askJson settingSources`, HYG7 and hygiene's fifth `categories` parameter), and B3 ships the precedence without the suggestion limb. Its §6 caller list named `connect.ts`, `publish.ts:64/:118`, `eval.ts:98` and `validate.ts:42` — all written against pre-B1 files — and **was rewritten to publish-only on 2026-09-13 (rev 9)**: publish passes `team.categories`, `eval` and `validate` pass nothing and HYG7 no-ops there, and B9 is gated on B5 merged (D28). |
...
| `.planning/specs/2026-09-04-eval-engine.md` | locked, **partly superseded** (see the note at its head) | M3 replaces its receipt and gate contracts: §5.3's receipt JSON (now `schema_version: 2` with required `content_digest`; `version` = `v<N>` or `null` for a local run; `skill_id` nullable for a local run — §3.4, §6.1); §5.4/§4.1's keying (committed at `evals/<id>/v<N>/<runId>.json` per §6.4, migrated at `evals/<id>/archive/<40-hex>/…` per §13 step 3, local runs at `~/.terum/skills/evals/local/<digest>/<runId>/` per §6.2 — the digest is a first-class field because a local run has no version); `--commit` (deleted, §12: publish is the sole receipt writer); §10's guard regex (replaced by row g's `RECEIPT_PATH` under `'publish'`, §4.2); §7.1's tree materialization and `origin/main^` fallback (§6.5); §7.2's endorsed-set trigger catalog (§6.3); §6.0/§9's hygiene contract for `eval` (§6.3's local-eval mode); and the publish-PR receipt gate of its §2/§6.1/§11/§14/§15/§16, which is **relocated into `publish` itself, not retired** (D19). Its precedence clause does not extend to anything this row names. `schema_version` stays `1 \| 2` and the 40-hex `version` arm is retained read-only, so no historical receipt becomes unparseable. |
| v0.14.0 | merged on main, **not tagged** | A release is mid-flight. Do not branch across the tag. |

### 16.1 Build order

The milestones above are the spec's *reading* order. The **build** order is eight batches, derived from the importers of every module this spec deletes or rewrites (`.planning/reviews/2026-09-11-refactor-implementation-batches.md` holds the file:line evidence, the desktop zod-mirror strictness table, and the test files that pin deleted behaviour); after **each** batch the root gates (`lint`, `typecheck`, `test`) are green, and the desktop gates too where `desktop/` is touched.

```
B1  M7 sync collapse (§10) + §12's verb deletions — connect, decline, receipt-check, eval --commit,
    config.shared and every reader, auto-sync.ts, Settings ▸ Sharing, AddSkillsDialog — + §11.2, §11.3,
    §11.5, §11.6, D17, D20, D22
B2  L-PROJ config half (§3.6, §7.1, §7.2, D13) + §9.2's onboarding step  (discover.ts imports checkouts.ts)
B3  the layout-3 keystone: M1 (§4) + M2 (§5) + M3 (§6) TOGETHER, plus §9.3's profile-entry.ts, §13.1(a)'s
    layout precondition and never-blank invariant, install's v<max> source path, and every `unresolved` deletion
B4  M5 marketplace (§8) + the SkillCard fields of §7.4
B5  M4 library (§7.3–§7.5, D6, D16, D18)
B6  M6 install rest (§9.1, §9.1.1, D11, D12, the #167 picker lift)
B7  §11.1 the bundled manual + docs/frame-protocol.md
B8  §13 the migration verb — BUILT here, RUN only after the release containing B1 has propagated
```

**Why M1 → M2 → M3 is not three green batches.** §4.1 deletes `src/lib/version.ts` and forbids leaving it beside `versions.ts`, but `eval.ts:29,105,124,229,231,472,477`, `evalReport.ts:11,33` and `install.ts:20,105,109` import it; `teamSchema` dropping `global` and `policy.publish` breaks `publish.ts:64-125`, `status.ts:27,116`, `eval.ts:454`, `readme.ts`, `src/commands/readme.ts:25-26`, `team.ts:527` and `guard.ts:198` at once; `skillVersions`' new signature is consumed at `ls.ts:87` and `eval.ts:519`; publish's attach needs schema-2 receipts (§6.1) and the content-keyed store (§6.2); and `skillRecords` reading `v<N>/SKILL.md` invalidates every flat test fixture (`src/lib/__tests__/fixtures.ts:72` and ~34 files seeding `skills/<name>/SKILL.md`). So M1, M2 and M3 are one batch. The only defensible split — M1+M2 first, M3 second — needs a throwaway shim (eval's candidate resolved to `<clone>/skills/<name>/v<max>/` with no `content_digest`); rule 0.2 tolerates a single path, but nothing else recommends it.

**Why M7 goes first, not "with M2".** Landing B1 before B3 removes every call site of the guard actions M1 deletes (`connect.ts:197,251,366,385,444,446`, `sync.ts:355`, `eval.ts:220,335`, `decline.ts:30`) before `guard.ts` is rewritten, so the tree compiles between the two; the rows for the deleted actions sit with zero callers for exactly one batch, which is dead code, not a second active path. Three couplings the milestone order missed also land in B1: `sync.ts:19,502` imports `endorsedCandidates` and `sync.ts:22,282,428` imports `materializeVersion` (both M1 deletions), and `src/lib/execute.ts:4,52-59` imports `SyncResult` and special-cases hook results. B1 also carries the desktop half of M7 — `cliSync`'s `placed`/`deferred`, `cliStatus.ledger.shared` and `cliLocalRow.shared` are required keys, and `features.test.ts:26` pins the `['sync','--auto',…]` spawn — because the desktop suite feeds hand-written CLI JSON through its zod mirrors and never runs the CLI: a CLI shape change without its mirror is gate-green and app-broken.

**Every batch that changes a `terum-skills` literal or a verb** updates `src/lib/__tests__/invocation-catalog.ts` and `FRAME_VERBS` in the same batch (`invocation-tripwire.test.ts:7-24`, `frames.test.ts:192`, `frames-cli.test.ts:67-74`); `versions.ts` must stay import-free or `desktop/src/backend/tauri/__tests__/cli-tree-imports.test.ts` fails, not only the bundle. M5 is **not** run in parallel with M2 on one branch: both edit `ls.ts`, `tauri/index.ts` and the `index.test` fixtures.

**Two release-ordering constraints stand.** (1) §13's migration is *run* only after the release containing B1 has propagated (§13.1(b)): an un-upgraded teammate's auto-share fails closed on a layout-3 repo (`teamSchema`'s `z.literal(2)` refuses it before any local write), but until they upgrade every session start reports a skipped auto-share for every local skill. (2) §13.1(a)'s layout precondition and the `applyReadme` never-blank invariant ship with **B3**, the first layout-3 CLI — so from that release until each team runs `team migrate`, every un-migrated team repo's `readme` job (pinned to `terum-skills@latest`, un-updatable) refuses and is red. That window is the migration debt the release notes must name; batch order cannot remove it.
