...
9. **Otherwise:** `N = highest + 1` (never `count + 1`; on a first publish the highest is 0, so `N = 1`). Write every entry of `files` under `skills/<name>/v<N>/`, eval cases included. Attach matching receipts under `evals/<uuid>/v<N>/`.
...
- A migration test on a seeded layout-2 repo with at least two members, driving the complete layout-2→3 diff through `safeWrite`, asserting the exact layout-3 output, the archived receipts, every member's re-keyed `installed[]`, and that an un-migrated repo read by a layout-3 CLI produces the upgrade sentence rather than a zod dump.
