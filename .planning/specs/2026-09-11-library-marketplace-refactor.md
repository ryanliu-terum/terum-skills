...
**Status:** DRAFT rev 8 (2026-09-11). §2 records twenty-four rulings. §15 is empty — every open question was walked to a resolution.
...
**D24 — Frame verbs.** `prune` and `skill move|rename|delete` are `FRAME_VERBS` entries (each has a seam: `backend.prune()`, `backend.skill.*`); `team migrate` is **not** — it is run once per team by a human from a terminal, never from the app, and the inventory tests (`frames.test.ts:192`, `frames-cli.test.ts:67-74`) pin that.
...
**The write order, and what an interruption leaves.** `[new] src/commands/skill.ts` (the verb), `backend.skill.{move,rename,delete}` on the seam with zod DTOs in `desktop/src/backend/tauri/index.ts` and `mock/index.ts`, and `skill move|rename|delete` in `FRAME_VERBS` (`src/lib/frames.ts:36`), `CliVerbs` and `src/cli.ts` — the same shape §7.1 uses for L-PROJ. Ordered writes per mode: **rename** = `rename()` → `SKILL.md` `name` rewrite → ledger re-key; **move** = destination check → `moveDirectory()` → ledger re-key; **delete** of a non-placement = `moveToQuarantine()` only; delete of a placement = `uninstallMany`, unchanged. Every mode is safe to re-run, and a `config.placements` row whose recorded path no longer exists is **repaired, never stranded**: re-pointed at the new path when the recorded `fingerprint` matches what is there, dropped otherwise — the local twin of §13's `pending` self-drain, and what makes the stale-ledger-key hazard above survivable. A move whose destination already holds a folder of that name follows §9.1.1's one rule: the existing folder is kept at `<root>/.claude/old-skills/<name>` and the user is told. Tests: `src/commands/__tests__/skill.test.ts` — interrupt after the directory change, after the frontmatter write and after the ledger write for each mode, plus an `EXDEV` move that leaves both copies and is completed by a re-run.
