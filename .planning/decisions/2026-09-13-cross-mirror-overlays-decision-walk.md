...
- **Prerequisite proof:** `showLocal` digests with `canonicalDigest(path)`; `publish` and `evalReport` with `skillContentDigest(files)`. §3.3 says one function. The implementer must show equality on a fixture with managed fields present, or the overlay matches nothing.
...
- **Implementation prerequisite carried on D1:** prove `canonicalDigest(path)` and `skillContentDigest(files)` agree on a fixture with managed fields present.
