...
| 4a | Setup auto-launches the app | LOCK | Supersedes D4's question (Teddy, 2026-09-09: "The wizard should also auto boot the application"). Where an app exists for the machine and a person is at an interactive terminal, `setup` installs and opens it without asking and continues there; `--no-app` keeps the wizard in the terminal; frames, pipes and `install`'s quiet bootstrap still skip it. The `app` verb keeps its `offer` knob for other callers. Same PR: the Windows launch is detached so the CLI no longer waits for the app window to close. | — |
...
### Technical
- **Files / code paths:** `.github/workflows/release.yml` gains a `desktop` job matrix (macOS arm64, macOS x64 cross-compiled, Windows arm64 NSIS on `windows-11-vs2026-arm`, Windows x64 NSIS on `windows-latest` since 2026-09-09) between `validate` and the npm-environment approval, uploading to a draft release that the finalize step publishes; `contents: write` scoped to that job. Template: the scratch workflow proven green on 2026-09-08 in `ryanliu-terum/terum-tauri-ci-probe` (ad-hoc signing, codesign verify, artifact-exists gate, no updater JSON).
- `app` downloads from `ryanliu-terum/terum-skills` release `v<version>`; asset names `terum-skills-desktop_<version>_{aarch64.app.tar.gz,x64.app.tar.gz,arm64-setup.exe,x64-setup.exe}` each with `.sha256`. Verified before unpack. (The `x64-setup.exe` leg, NSIS on `windows-latest`, was added 2026-09-09 in PR #98; the first matrix had left x64 Windows machines without a build.)
