...
- **Explain every bug and design decision in the `/single-fix` escalation-briefing style** (`.claude/skills/single-fix/SKILL.md` Phase 3B): What it is (plain English first) → Why (walk the real code path) → Options + trade-offs (Fit 0-4 against the governing spec + ratified North Star, then Bug risk 0-4, how likely the change is to introduce a new defect, never summed; highest Fit wins, then lowest Bug risk; effort is a one-line footnote that never decides; each with a "Wins if" line) → Recommendation. Conceptual first, then technical.
