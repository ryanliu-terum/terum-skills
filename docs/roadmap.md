...
- **Generating a suite from a skill.** Planned. A suite is one sandbox and one session per arm with a row per planted defect, which is how a skill that reviews or audits gets measured on whether it found the bug instead of on the words in its output. The engine accepts a hand-authored `evals/suite.yaml` today and runs it; generating one from the skill under test is specified and not built, so `eval` generates cases and trigger prompts only.

- **Category-specific generated tests.** Planned. The generator is specified to choose a shape from what the skill does: a small repository with planted defects for a skill that reviews or fixes code, a document with planted contradictions for one that audits a spec, a fabricated session log for one that summarises, and prompt cases for one whose behaviour depends on how it is asked. Today every generated set has the same shape, described in [generated evals](evaluating/generated-evals.md): three to seven cases across the five buckets, plus should-trigger and should-not-trigger prompts.

- **`k` and cost controls.** Under consideration. `--k`, `--parallel`, `--batch` and a queue ship today, and the app and setup's evals step price a run from your earlier receipts before it starts. A skill that calls subagents is detected and the run asks `Use the one-session heavy evaluation mode?`; your answer is written to the run log and changes nothing else, because the one-session shape comes from an authored `evals/suite.yaml` and from nothing else. The numbers behind the controls are provisional and recorded as such in the source: `k` defaults to 1 on cost, arms run `sonnet`, and the escalation judge runs `opus`.
