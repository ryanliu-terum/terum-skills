...
|---|---|---|
| The eight skills | `~/.claude/skills/<name>/`, and `~/.codex/skills/<name>/` for Codex | Run terum-skills for you and show the result as a board; the manual among them says which verbs to hand to your terminal |
...
The manual divides the CLI in two, and the dividing line is a question. Claude Code's Bash tool has no TTY, so a verb that needs an answer fails with:

```
Cannot ask "…": this command needs an interactive terminal (stdin is not a TTY).
```

A refusal does not prove nothing happened. A verb can clone, record intent or finish a write before it reaches its first question, so the manual tells the agent to read the whole board before reporting an outcome.
...
`unpublish`, `reconcile`, `project add` with no path, `skill move`, `skill copy`, `skill rename`, `skill delete`, `skill enable`, `skill disable`, `prune`, `uninstall-skill`, `uninstall`, `team leave`, `team remove`, `team move`, `team project create`, `team project delete`, `team migrate`, and the wizards: `setup`, `team create`, `team join`, bare `login`.
...
It places nothing, uploads nothing and edits none of your skill folders. The exceptions are Terum's own artefacts described on this page: its own `SessionStart` entry, the eight skills where they are outdated or missing, and an outdated edit-hook script.
