...
`team join` still ends with one offer of the session-start hook, after the join itself is durable. It never places the eight skills or offers the edit hook; only `setup` does.
