...
`<n>` is the number of authored case files, or the word `each` when the folder has none.
...
One-session mode is selected by one thing only, the presence of an authored `evals/suite.yaml` in
the skill folder. A skill with a suite runs one session per arm whether you answer yes or no; a
skill with ordinary cases runs one session per case per arm either way. See
...
| --- | --- |
| `evals/cases/*.yaml` | `--triggers-only` not given, `--no-gen` not given, no authored case file, and no `evals/suite.yaml` |
...
Assets land by rename, cases first, so each one is absent or whole and never partial. A copy of the
same bytes is also written to the run tree under `generated/`. See
