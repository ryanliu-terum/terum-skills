...
Execution cases come from `evals/cases/*.yaml` in the skill folder and from `evals/suite.yaml`, and a folder holding both runs both. When it holds neither, a generator writes execution assets into the folder, either a set of cases or one suite, whichever shape it judges the skill measurable in. Trigger prompts come from `evals/triggers.yaml`, generated the same way when absent. See [test assets](test-assets.md) and [generated evals](generated-evals.md).
