...
| | `eval <skill>` | Run the skill's evals on your own Claude Code login and file the receipt to the team; `--no-commit` keeps it local. `eval --drain [--parallel n] [--window overnight] [--max n]` runs queued evals; `eval --queue-list` lists them; `eval --dequeue <team>/<skill>` removes queued versions |
...
**`eval`** runs the skill for real, on your own logged-in Claude Code, in throwaway sandboxes. It reads the team clone and writes its local run tree plus one receipt file — running the eval IS the sharing step, so teammates see the result without a second command. `--no-commit` keeps the run to yourself. It never touches `skills/`, `people/`, or your placed copies.
...
Where the files go depends on what you answer:

- **Declined, or no terminal to ask:** generated assets live only in the local run tree. The run prints a verdict and the review path; it writes nothing to the team repo and files no receipt — a receipt must name eval assets the team can read.
- **Confirmed, one command:** `eval <skill>` shows you the generated files and asks one question — *Commit generated eval assets for `<skill>`? [y/N]*. **y** commits them into your skill through the ordinary sync commit, re-runs against them, and files the receipt, which records the exact asset tree it read. **N** keeps everything local: the eval still runs and prints a verdict, and the files wait in the run tree for you to edit. This is the one place publishing reaches into your skill, and it never does so without asking.
- **Deliberately, your own source:** `eval --working --save` writes reviewed generated assets into your source folder without committing anything; the next `sync` carries them up, and the next ordinary `eval` receipts them like any authored case.

Flags: `--no-gen` restores the old empty-asset behavior (and is the escape hatch when you want publishing untouched by generation); `--gen` produces a fresh second-opinion set for one run without reading or touching your authored files — it never commits and can't be combined with `--case`.
...
A finished run writes one immutable file, `evals/<skill-id>/<version>/<run-id>.json`, through the same guarded write path as everything else. The version pins exactly which skill bytes were evaluated, and `provenance.eval_assets` pins the `evals/` tree that judged them — so adding or editing cases files a new receipt beside the old one instead of moving the skill to a new version. Re-running the same version appends a new receipt beside the old; nothing is overwritten or deleted. Anyone may evaluate anyone's skill; the receipt records who ran it, the engine and Claude Code versions, the models actually resolved, k, and the case list. Numbers from receipts with a different model or Claude Code version are never compared.

`--working` evaluates your local connected source instead of the store copy, for the authoring loop. It never publishes, because receipts pin committed trees only: the run says so and names the command that would share the source first. Passing `--commit` explicitly is refused there rather than quietly downgraded.
...
- **receipt-check** on `publish` PRs: the PR must carry a schema-valid receipt at the skill's exact version — the skill tree without its `evals/` folder, so a PR that only adds eval cases keeps the receipt it already earned. If it includes a candidate-vs-incumbent comparison, that comparison must not be FAIL. If it includes none, CI confirms no prior version of the skill exists and lets the first publish through.
